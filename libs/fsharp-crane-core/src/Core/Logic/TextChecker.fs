module CraneCore.Logic.TextChecker
...
/// compute that scaled O(N x M) per chunk.
...
let classifyMissing (segment: string) : string =
    let norm = normalize segment
    if norm.Length < 50 then "CRITICAL" else "HIGH"
