...
Deploy the OSE Application app group to **staging** by dispatching the scheduled
local-deploy-stag workflow. The workflow gates on the full local-stack test
suite, then force-pushes the `stag-ose-app-web` and `stag-ose-be` branches.

## Core Responsibility

Ship the OSE Application app group to staging via a gated GitHub Actions workflow:

1. **Trigger workflow**: `gh workflow run ose-app-test-local-deploy-stag.yml`
...
3. **Trigger Vercel build**: on success, Vercel detects the push to
   `stag-ose-app-web` and rebuilds the staging site

**Build Process**: Vercel listens to `stag-ose-app-web` branch and
...
OSE Application app group. The gated `ose-app-test-stag-deploy-prod.yml` workflow
currently runs the FE E2E gate against the staging URL and **stops on pass — it
does not promote to production**. Production continuous delivery is deferred to a
separate plan. Do not invent or invoke a prod-promotion workflow.
...
gh workflow run ose-app-test-local-deploy-stag.yml \
...
gh run list \
  --repo wahidyankf/ose-public \
  --workflow=ose-app-test-local-deploy-stag.yml \
  --limit=3
```
...
`stag-ose-app-web` (Vercel rebuilds the staging app) and
`stag-ose-be` (the backend GHCR build fires for the backend image).

## Staging Deployment, Protection Bypass, and Secrets

Vercel serves the `stag-ose-app-web` branch at a **private** staging URL behind
Vercel Deployment Protection. The staging E2E gate authenticates against it
using GitHub Environment values — never literals committed to the repo:

- **Environment**: `ose-app-staging`
- **Var** `WEB_BASE_URL`: the private `stag-ose-app-web` staging URL (Environment
...
```bash
git push origin main:stag-ose-app-web --force
```

This skips the GitHub Actions workflow entirely. It does not skip Vercel —
Vercel still builds from `stag-ose-app-web` on push.

## Vercel Integration

**Staging Branch**: `stag-ose-app-web`
...
development happens on `main`. The staging branches (`stag-ose-app-web`,
`stag-ose-be`) are CI-automated from `main` by
`ose-app-test-local-deploy-stag.yml`. Production promotion is deferred —
...
- The `ose-app-staging` GitHub Environment can carry protection rules
...
`stag-ose-app-web` or `stag-ose-be` may have diverged unexpectedly, or branch
protection may be misconfigured. Inspect the run logs.

## When to Use This Agent

**Use when**:

- Shipping the latest `main` to the OSE Application staging environment
...
- Deploying the OSE marketing site (use `apps-ose-www-deployer`)
...
- `swe-typescript-dev` - Develops ose-app-web Next.js code
- `swe-fsharp-dev` - Develops ose-be F# backend code
