...
- `docs-checker`, `apps-ayokoding-www-facts-checker` — validate existing repo content against facts; may invoke `web-research-maker` when they need current data before producing findings
...
2. **Check the repo first**: Before hitting the web, use `Read`, `Grep`, and `Glob` to see if the repo already contains the answer. Priority paths:
   ...
   - `apps/*/README.md`
   - `specs/apps/*/behavior/*/gherkin/` for behaviour specs
