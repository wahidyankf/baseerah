---
name: apps-ose-web-deployer
description: Deploys ose-web to production environment branch (prod-ose-web) after validation. Vercel listens to production branch for automatic builds.
tools: Bash, Grep
model: haiku
color: purple
skills:
...
Deploy ose-web to production by force pushing main branch to prod-ose-web.
...
2. **Force push to production**: Push main branch to prod-ose-web
3. **Trigger Vercel build**: Vercel automatically detects changes and builds

**Build Process**: Vercel listens to prod-ose-web branch and automatically builds the Next.js 16 content platform on push. No local build needed.
...
```bash
# Force push main to prod-ose-web
git push origin main:prod-ose-web --force

echo "✅ Deployed successfully!"
echo "Vercel will automatically build from prod-ose-web branch"
```

## Vercel Integration

**Production Branch**: `prod-ose-web`  
...
**Why Force Push**: Safe because prod-ose-web is deployment-only. We always want exact copy of main.
...
- `apps-ose-web-content-checker` - Validates content before deployment
