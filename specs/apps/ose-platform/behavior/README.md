...
Behavior specifications for `ose-web` — Gherkin scenarios that exercise the single
...
- `cli/` — CLI-semantic scenarios for `ose-cli`. Covers the `links check` command.

## Perspectives

| Perspective | Background                          | Step style                                       | Consumed by                                      |
| ----------- | ----------------------------------- | ------------------------------------------------ | ------------------------------------------------ |
| `web`       | `Given the app is running`          | `clicks`, `types`, `sees`, `navigates`           | `apps/ose-web-fe-e2e` (Playwright)               |
| `api`       | `Given the API is running`          | `sends GET/POST`, `status code`, `response body` | `apps/ose-web-be-e2e` (Playwright tRPC HTTP)     |
| `cli`       | `Given the CLI binary is available` | `runs`, exit code, output assertions             | `apps/ose-cli` (Go test + godog, `cli/gherkin/`) |
