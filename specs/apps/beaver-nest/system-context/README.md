...
graph TB
    ...
    FE["<b>beaver-nest-fe</b><br/>Next.js landing page<br/>port 19310"]
    BE["<b>beaver-nest-be</b><br/>F#/Giraffe REST API<br/>port 19320"]
...
- **Maintainer (browser)** — the only human actor in Phase 1. Navigates to `beaver-nest-fe` in a
  browser; there is no authentication and no other user role.

## Systems

- **`beaver-nest-fe`** — Next.js 16 landing page. Renders one page (`/`) that names the product and
  displays the greeting it fetches from `beaver-nest-be`.
- **`beaver-nest-be`** — stateless F#/Giraffe REST API. Exposes `GET /api/v1/health` (liveness),
  `GET /api/v1/hello` (the greeting `beaver-nest-fe` renders), and a 404 handler for any other route.
