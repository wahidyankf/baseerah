...
- `beaver-nest-be` — a stateless F#/Giraffe REST API exposing exactly two `GET` routes
  (`/api/v1/health`, `/api/v1/hello`) and a 404 handler for anything else
- `beaver-nest-fe` — a single Next.js landing page (`/`) that names the product and shows the
  greeting fetched live from `beaver-nest-be`
