...
- `src/generated-contracts/` — codegen output, gitignored, regenerated by `nx run organiclever-app-web:codegen`.
...
The Gherkin folder layout under `specs/apps/organiclever/behavior/organiclever-app-web/gherkin/` is reorganized from per-route to per-bounded-context. The mapping below is the authority that Phase 9 executes mechanically.
...
**Severity: ESLint boundaries (`boundaries/element-types`) at `error` severity** as of Phase 8 of the [DDD adoption plan](../../../../plans/done/2026-05-03__organiclever-adopt-ddd/delivery.md). Any forbidden cross-layer or cross-context import fails `nx run organiclever-app-web:lint` and blocks the pre-push hook + CI.

### Why a separate eslint pass alongside oxlint?

oxlint does not implement `eslint-plugin-boundaries` (the closest rule it ships, `import/no-cycle`, only catches cycles, not directional layer/context boundaries). Replacing oxlint with eslint repo-wide would force re-implementing every oxlint rule under eslint and slow lint significantly. The narrow scope chosen here — eslint enabled only for `apps/organiclever-app-web` and only for the boundary rule — keeps oxlint authoritative for everything it covers and adds eslint as a focused sidecar.
