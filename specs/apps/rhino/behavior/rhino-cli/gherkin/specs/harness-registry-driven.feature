...
Feature: harness commands are registry-driven
  ...
  As a developer
  I want harness naming validate, harness instruction-size validate, and harness duplication
  validate to derive their target sets from repo-config.yml
  ...
  Scenario: Every harness command is registry-driven, not hard-coded
    Given the repo-config.yml harness section lists an agent-bearing tier (Amazon Q) and a native
    instruction surface
    When harness naming validate, harness instruction-size validate, and harness duplication validate
    run
