...
graph TB
    ...
    FE["<b>baseerah-fe</b><br/>Next.js landing page<br/>port 19310"]
    BE["<b>baseerah-be</b><br/>F#/Giraffe REST API<br/>port 19320"]
...
- **Maintainer (browser)** — the only human actor in Phase 1. Navigates to `baseerah-fe` in a
  browser; there is no authentication and no other user role.

## Systems

- **`baseerah-fe`** — Next.js 16 landing page. Renders one page (`/`) that names the product and
  displays the greeting it fetches from `baseerah-be`.
- **`baseerah-be`** — stateless F#/Giraffe REST API. Exposes `GET /api/v1/health` (liveness),
  `GET /api/v1/hello` (the greeting `baseerah-fe` renders), and a 404 handler for any other route.
