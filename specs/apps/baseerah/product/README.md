...
Baseerah is a personal AI assistant and content operating layer, built
...
- `baseerah-be` — a stateless F#/Giraffe REST API exposing exactly two `GET` routes
  (`/api/v1/health`, `/api/v1/hello`) and a 404 handler for anything else
- `baseerah-fe` — a single Next.js landing page (`/`) that names the product and shows the
  greeting fetched live from `baseerah-be`
