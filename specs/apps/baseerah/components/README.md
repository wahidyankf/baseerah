# Baseerah — Components (C4 L3)

Component-level (inside-the-container) detail for the Baseerah hello-world quad.

Both `baseerah-be` and `baseerah-fe` are still hello-world stubs (Phase 1) — a single route/page
each, no internal module boundaries worth diagramming yet. Component-level `be/` and `web/`
subfolders land once the apps exist (Phase 6 and Phase 8) and have enough internal structure to
document. See [product/](../product/README.md) for the phase sequence.

## Related

- [overview.md](./overview.md) — status of component-level detail
- [containers/](../containers/README.md) — C4 L2
- [behavior/](../behavior/README.md) — Gherkin scenarios
