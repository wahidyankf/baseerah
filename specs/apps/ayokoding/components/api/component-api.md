# Component Diagram: tRPC API Perspective

Level 3 of the C4 model. Shows the logical components inside the `web` container's tRPC API
surface: tRPC router, procedures, content services, search index, and markdown pipeline.

The `api` perspective slug indexes HTTP-semantic behavior (procedure shapes, status codes,
locale-scoped responses). It does not represent a separate deployable container — tRPC procedures
execute inside the same `web` container's Next.js server.
...
| ------------------------------------ | ---------- | ---------------------- |
| content.getBySlug + Content Reader   | content    | content-api.feature    |
| content.listChildren + Content Index | content    | content-api.feature    |
| content.getTree + Navigation Schema  | navigation | navigation-api.feature |
| search.query + Search Index          | search     | search-api.feature     |
| meta.health                          | health     | health-check.feature   |
| meta.languages                       | i18n       | i18n-api.feature       |
