...
The component structure is presented as five views of one graph, split by hop so each view stays
readable. Together they carry every component and every relationship.

## View 1: Learner → Middleware → Pages

Every request passes through the locale middleware before it reaches a page. The sitemap and RSS
feed are standalone route handlers with no middleware hop.
...
## View 2: Pages → Layout Components

The content page composes the full chrome; the locale home composes only header and footer. Mobile
Nav belongs to the layout inventory and carries no page-level edge in this model.
...
## View 3: Content Page → Content Renderers

The content page mounts the markdown renderer, which in turn resolves shortcode elements into
dedicated renderer components.
...
## View 4: Header → Search and i18n

The header owns both the search trigger and the language switcher. Translations are consumed by the
language switcher and by the middleware.
...
## View 5: Components → tRPC API

Four components call the server-side tRPC API directly.
...
[`specs/apps/ayokoding/behavior/ayokoding-www/gherkin/`](../../behavior/ayokoding-www/gherkin/):
...
| Header + Footer + Mobile Nav     | app-shell  | Responsive layout, accessibility       |
...
- **API perspective component diagram**: [../api/component-api.md](../api/component-api.md)
- **Web gherkin specs**: [../../behavior/ayokoding-www/gherkin/](../../behavior/ayokoding-www/gherkin/)
