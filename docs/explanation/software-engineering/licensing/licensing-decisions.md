...
sharp is used in Node.js applications (primarily `ayokoding-www`) for server-side image optimisation. sharp loads libvips as a **native dynamic addon** via Node.js's N-API interface. The libvips binary is distributed as a platform-specific prebuilt package (`@img/sharp-linux-x64` etc.) and loaded at runtime with `require()`.
