...
One-line summary: give `pr-review-maker` a dedicated posting identity so blocking reviews land with a
real `REQUEST_CHANGES` STATE instead of a plain `COMMENT`.

> De-promoted 2026-07-21 from a full backlog plan (full detail preserved in git history).

## Problem / context

`pr-review-maker` structurally cannot post a `REQUEST_CHANGES` review: `gh` authenticates as the PR
...
`.claude/agents/pr-review-maker.md`. A documented workaround is not a fixed gate. Surfaced during the
`parallel-orchestration-shared-machine-governance` Knowledge Capture phase.

## Why now

The maker→fixer cycle is the default gate for every `*-to-pr` delivery, so this blind spot is exercised
...
- Wire `pr-review-maker` to authenticate as that identity so `REQUEST_CHANGES` becomes available for
  blocking findings.
...
In scope: the posting identity, the maker rewiring, and the workaround removal.
