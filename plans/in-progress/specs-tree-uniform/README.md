...
5. **Governance propagation** — delegate to `repo-rules-maker` to fold the new uniform state into `repo-governance/`, agent definitions, and `AGENTS.md`; then `repo-rules-checker` validates the propagation; then `npm run sync:claude-to-opencode` mirrors agent changes to `.opencode/`.
