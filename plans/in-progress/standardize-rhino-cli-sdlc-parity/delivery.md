...
- [ ] [AI] **GREEN**: implement `env staged-guard validate` (rhino-cli `src/`) — port `check-no-env-staged.sh`: list `git diff --cached --name-only --diff-filter=AM`, reject any path whose basename matches `.env*` except exactly `.env.example`, emit the offending paths + the "policy: guard-env-file-access" message, exit non-zero — command: `npx nx run rhino-cli:test:unit` — acceptance: new test passes.
  - _Suggested executor: `swe-rust-dev`_
- [ ] [AI] **REFACTOR**: expose it as the Nx target `env:staged-guard-validation`; document it in `docs/reference/sdlc-gate-standard.md` (pre-commit step 1) — command: `npx nx run rhino-cli:test:quick` — acceptance: `npx nx run rhino-cli:env:staged-guard-validation` exits 0 on a clean staged tree; staging a real `.env` makes it exit non-zero.
...
### 2a. Standardize rhino-cli target names (remove `fmt`/`format:check`, fold tool-lint into lint-staged, add `harness:bindings-generate` + `harness:bindings-validation`)
...
- [ ] [AI] **GREEN**: in `apps/rhino-cli/project.json` add targets `harness:bindings-generate` (`cargo run -- harness bindings generate` — regenerates `.opencode/`/`.amazonq/` from `.claude/`; the pre-commit hook auto-stages the result) and `harness:bindings-validation` (`cargo run -- harness validate bindings`) — acceptance: `npx nx run rhino-cli:harness:bindings-generate` and `:harness:bindings-validation` each exit 0.
  - _Suggested executor: `swe-rust-dev`_
- [ ] [AI] **REFACTOR**: tag the two new targets consistently with the existing validation targets in `project.json` (`harness:bindings-validation` cacheable; `harness:bindings-generate` writes files, so mark it `cache: false`) — acceptance: `npx nx show project rhino-cli --json | jq '.targets | keys'` lists both; re-running `:harness:bindings-validation` is a cache-hit.
...
- [ ] [AI] Edit `.husky/pre-commit`: **delete the inline `shellcheck` / `hadolint` / `actionlint` blocks** (they are now lint-staged entries, added above), **replace the inline `./scripts/check-no-env-staged.sh` line with `npx nx run rhino-cli:env:staged-guard-validation`** (step 1), **replace the opaque `rhino-cli git pre-commit` call with `npx nx run rhino-cli:harness:bindings-generate`** (step 3), and **remove the `nx affected -t test:quick` line entirely** (it moves to pre-push only — pre-commit must stay fast), preserving the tool-gated graceful-skip behaviour and the standard 3-step order (env-staged-guard → lint-staged [format + tool-lint] → harness:bindings-generate; **no `test:quick`**) per [tech-docs §1](./tech-docs.md#1-lifecycle-stage--exact-commands-post-implementation-identical-across-3-repos) — acceptance: `bash .husky/pre-commit` on a staged no-op runs without error; staging a real `.env` then committing aborts; `grep -c 'test:quick' .husky/pre-commit` returns 0; step order matches the standard.
...
- [ ] [AI] Add a new `.github/workflows/main-ci.yml` triggered on `push: branches: [main]` that runs the **same fast set as the PR gate** for affected projects: `nx affected -t test:quick` + the governance/spec validators (`lint:md`, `env:validation`, `specs:behavior:coverage` + structural, path-gated governance) — acceptance: `actionlint` passes; a merge to main runs test:quick + validators; `grep -n 'test:integration\|test:e2e' .github/workflows/main-ci.yml` returns nothing.
...
- [ ] [AI] Confirm the gate test-scope split: **pre-push, PR, and main-ci run `test:quick`** (+ validators); **pre-commit runs the fast file-type set only — no `test:quick`**; and **no gate runs integration/e2e** — acceptance: `grep -n 'test:integration\|test:e2e' .husky/pre-commit .husky/pre-push .github/workflows/pr-quality-gate.yml .github/workflows/main-ci.yml` returns no gate invocation; `grep -c 'test:quick' .husky/pre-commit` returns 0; `grep -c 'test:quick' .husky/pre-push` returns ≥ 1.
...
- [ ] [AI] `npx nx run rhino-cli:harness:bindings-generate` and `:harness:bindings-validation` — each exits 0; `npx nx run rhino-cli:fmt`, `:format:check`, `:shell:lint`, `:dockerfiles:lint`, `:actions:lint` all fail (targets removed); staging a `*.sh` with a shellcheck warning aborts the commit (lint-staged tool-lint).
...
- [ ] [AI] `actionlint .github/workflows/main-ci.yml` — exits 0; it runs the fast set (test:quick + validators) and `grep -n 'test:integration\|test:e2e' .github/workflows/main-ci.yml` returns nothing.
- [ ] [AI] `npm run lint:md` — exits 0.
...
- [ ] [AI] In primer `apps/rhino-cli/project.json`: **remove `fmt` + `format:check` targets** (formatting → lint-staged); **drop primer's `shell:lint`/`dockerfiles:lint`/`actions:lint` targets** (tool-lint folds into the lint-staged config — add the three linter entries there if missing); **add the `harness:bindings-generate` target** (parity with public, for the pre-commit regen step); update every `fmt`/`format:check` reference (`grep -rn 'rhino-cli:fmt\b\|rhino-cli:format:check' --include='*.json' --include='*.yml' --include='*.sh' --include='*.md' .`) — acceptance: `npx nx run rhino-cli:harness:bindings-generate` exits 0; `:fmt`/`:format:check`/`:shell:lint`/`:dockerfiles:lint`/`:actions:lint` all fail; staging a bad `*.sh` aborts the commit; zero stale `fmt`/`format:check` references.
  - _Suggested executor: `swe-rust-dev`_
...
- [ ] [AI] Add `.github/workflows/main-ci.yml` mirroring public's fast gate (`nx affected -t test:quick` + validators on push to main) — acceptance: `actionlint` passes; `grep -n 'test:integration\|test:e2e' .github/workflows/main-ci.yml` returns nothing.
...
- [ ] [AI] Confirm the primer gate test-scope split: pre-push/PR/main-ci run `test:quick`; pre-commit runs the fast file-type set only (no `test:quick`); no gate runs integration/e2e — acceptance: `grep -n 'test:integration\|test:e2e' .husky/pre-commit .husky/pre-push .github/workflows/pr-quality-gate.yml .github/workflows/main-ci.yml` returns no gate invocation; `grep -c 'test:quick' .husky/pre-commit` returns 0.
...
> `validate-env.yml`) + governance-vendor pre-push, but (like public) lacks the
> `harness:bindings-generate`/`harness:bindings-validation` Nx targets, folds shell/docker/actions
...
- [ ] [AI] In infra `apps/rhino-cli/project.json`: **remove `fmt` + `format:check` targets** (formatting → lint-staged); add `harness:bindings-generate` + `harness:bindings-validation` Nx targets (same defs as public); **fold shell/Dockerfile/workflow lint into lint-staged** (no tool-lint Nx targets); **remove the `test-coverage` target** (the command is gone from the propagated rhino-cli source); update references; rewire `.husky/pre-commit` to lint-staged (format + tool-lint) + `env:staged-guard-validation` + `harness:bindings-generate` (replacing the inline `check-no-env-staged.sh` and `shellcheck`/`hadolint`/`actionlint` blocks; drop the `test:quick` line) **and remove the git-identity guard** (`git rm scripts/check-no-env-staged.sh scripts/git-identity-check.sh` + drop both hook lines), and `.husky/pre-push` to `npx nx run rhino-cli:harness:bindings-validation` — acceptance: `jq -r '.targets|keys[]' apps/rhino-cli/project.json | sort` equals public's sorted key set (no `fmt`/`format:check`/`test-coverage`); `test ! -f scripts/check-no-env-staged.sh && test ! -f scripts/git-identity-check.sh`; each new target exits 0.
  - _Suggested executor: `swe-rust-dev`_
...
- [ ] [AI] Add `.github/workflows/main-ci.yml` (self-hosted) running the **fast gate** (`nx affected -t test:quick` + validators + IaC on push to main) — acceptance: `actionlint` passes; `grep -n 'test:integration\|test:e2e' .github/workflows/main-ci.yml` returns nothing.
...
- [ ] [AI] Confirm the infra gate test-scope split: pre-push/PR/main-ci run `test:quick` (+ validators + IaC); pre-commit runs the fast file-type set only (no `test:quick`); no gate runs integration/e2e — acceptance: `grep -n 'test:integration\|test:e2e' .husky/pre-commit .husky/pre-push .github/workflows/pr-quality-gate.yml .github/workflows/main-ci.yml` returns no gate invocation; `grep -c 'test:quick' .husky/pre-commit` returns 0.
