...
- **Symmetric per-project targets**: every project (direct child of `apps/`/`libs/`) in all three repos exposes the mandatory seven targets (`echo` where N/A). Observable check: the mandatory-seven `jq` loop prints no `MISSING` in any repo (delivery Phase gates). [Repo-grounded]
