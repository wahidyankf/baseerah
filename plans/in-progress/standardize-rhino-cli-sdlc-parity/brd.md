...
- **Symmetric per-project targets**: every project (direct child of `apps/`/`libs/`) in all three repos exposes the mandatory six targets (`echo` where N/A), plus native `test:coverage`, `specs:behavior:coverage`, and `specs:domain:coverage` on `*-be`. Observable check: the mandatory-six `jq` loop prints no `MISSING` in any repo (delivery Phase gates). [Repo-grounded]
...
- **Standardized GitHub CI for every project**: the plan completes only when every project across the three repos is covered by a GitHub CI named per the ose-public convention (`pr-quality-gate.yml`, `validate-markdown.yml`, `validate-env.yml`, `main-ci.yml`). Observable check: the four canonical workflows exist with those names in each repo and every project resolves into `main-ci.yml`'s affected matrix. [Repo-grounded]
- **Uniform test→deploy lifecycle**: `test:quick` (+ validators) runs at **pre-push, PR, and post-merge `main-ci.yml`**; **pre-commit stays fast** (format + tool-lint + guards, no `test:quick`); and **none** of the four gates runs `test:integration`/`test:e2e`. The heavy levels and the staging/prod deploy that depends on them run **only** in the scheduled CRON pipelines (`*-test-local-deploy-stag.yml` → staging; `*-test-stag.yml` → prod), per-app isolated — identical mechanism in every repo (deploy leg per each repo's actual deployables). Observable check: `grep test:integration\|test:e2e` finds no gate invocation in any hook or `main-ci.yml`/`pr-quality-gate.yml`; only the CRON pipelines run them. [Repo-grounded]
