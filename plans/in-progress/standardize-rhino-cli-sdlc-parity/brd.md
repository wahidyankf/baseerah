...
- **Uniform test→deploy lifecycle**: all four quality gates (pre-commit, pre-push, PR, post-merge `main-ci.yml`) run only `test:quick` (+ validators) — **none** runs `test:integration`/`test:e2e`. The heavy levels and the staging/prod deploy that depends on them run **only** in the scheduled CRON pipelines (`*-test-local-deploy-stag.yml` → staging; `*-test-stag.yml` → prod), per-app isolated — identical mechanism in every repo (deploy leg per each repo's actual deployables). Observable check: `grep test:integration\|test:e2e` finds no gate invocation in any hook or `main-ci.yml`/`pr-quality-gate.yml`; only the CRON pipelines run them. [Repo-grounded]
