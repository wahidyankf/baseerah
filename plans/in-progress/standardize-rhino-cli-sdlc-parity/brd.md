...
- **Uniform test→deploy lifecycle**: pre-merge gates run only `test:quick`; post-merge runs `test:quick`+`test:integration`+`test:e2e` per affected project and deploys passing deployables to staging individually — identical mechanism in every repo (deploy leg per each repo's actual deployables). Observable check: each repo has a `main-ci.yml` with the per-project matrix; pre-push/PR run no integration/e2e. [Repo-grounded]
