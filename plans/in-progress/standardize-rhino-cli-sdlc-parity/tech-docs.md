...
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| detect        | affected languages (matrix driver)                                                                                                                                                                                                                                                                                                                                                                                                                                          | other              |
| lint-staged   | `lint-staged --diff="origin/main...HEAD"` over the PR's changed files — **the exact pre-commit lint-staged set** at PR scope: formatters auto-fix and commit back to the PR branch; the tool-linters (`shellcheck`/`hadolint`/`actionlint`) **and the per-file markdown validators** (`markdownlint-cli2`, `md mermaid validate`, `md heading-hierarchy validate`, `specs gherkin-cardinality validate`) **fail** the job on findings (same config as the commit-time hook) | affected file-type |
| `<lang>` gate | `nx affected -t test:quick` (composes typecheck → lint → test:unit → test:coverage → test:specs — the latter runs all `specs:*` validators; rust also `deny:check`, `msrv:check`; Δ `format:check` REMOVED). The specs gate is folded in here — no separate specs-structural job                                                                                                                                                                                            | affected projects  |
| md-links      | `cargo run --release -- md links validate --exclude …` — the **cross-file** markdown validator, repo-wide (NOT `--diff`: a deleted/renamed file breaks links in untouched files, so the staged set is insufficient). The per-file md validators run in the lint-staged job above.                                                                                                                                                                                           | all file-type      |
| env           | `cargo run --release -- env validate`                                                                                                                                                                                                                                                                                                                                                                                                                                       | all file-type      |
| governance    | each pre-push path-gated validator, run when its trigger path is in the PR diff — `harness naming validate`, `harness parity validate`, `harness bindings validate`, `harness instruction-size validate`, `repo-governance vendor validate`, `repo-governance workflows naming validate` (so pre-push ∪ pre-commit is fully covered server-side)                                                                                                                            | other (path-gated) |
| quality-gate  | sentinel join job (all required jobs green)                                                                                                                                                                                                                                                                                                                                                                                                                                 | other              |
...
| Job               | Command / content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Scope         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| lint-staged-equiv | the **exact pre-commit lint-staged check set**, run over **_all_** files (not `--diff`): tool-lint — `shellcheck --severity=warning` over all `*.sh`, `hadolint --failure-threshold warning` over all `Dockerfile`/`*.Dockerfile`, `actionlint` over all `.github/workflows/*.{yml,yaml}` — **and** the per-file markdown validators — `markdownlint-cli2 "**/*.md"`, `md mermaid validate` (all), `md heading-hierarchy validate` (all), `specs gherkin-cardinality validate` (all `.feature` files). All fail on findings. **Formatters (the write-step) are the one carve-out — not run here: no PR branch to auto-fix-and-commit-back, and `format:check` is removed plan-wide.** | all file-type |
| `<lang>` gate     | `nx run-many --all -t test:quick` — composes typecheck → lint → test:unit → test:coverage (≥90% line) → test:specs (all `specs:*` validators) on **_every_** project (rust also `deny:check`, `msrv:check`); fanned with `--parallel`. Catches main-only / merge-skew breakage the affected graph misses. The specs gate is folded in here — no separate specs-structural job                                                                                                                                                                                                                                                                                                         | all projects  |
| md-links          | `cargo run --release -- md links validate --exclude …` — the cross-file markdown validator, repo-wide (same as pre-push/PR)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | all file-type |
| env               | `cargo run --release -- env validate` — every app's `.env.example` against the repo env contract (already repo-wide)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | all file-type |
| governance        | the full governance validator set, run **unconditionally** (not path-gated — post-merge re-verifies everything): `harness naming validate`, `harness parity validate`, `harness bindings validate`, `harness instruction-size validate`, `repo-governance vendor validate`, `repo-governance workflows naming validate`                                                                                                                                                                                                                                                                                                                                                               | other         |
| quality-gate      | sentinel join job (all required jobs green)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | other         |
...
| 8   | `md validate heading-hierarchy`              | `md heading-hierarchy validate`              | Validate heading hierarchy (one H1, no skipped levels) — **per-file**                                                                                                                                                                    | **wired**                   |   ❓    | Δ **lint-staged** (`*.md`, changed files; PR via `--diff`; main over all) — direct `cargo run`, was Nx `headings:hierarchy-validation`                     |
| 9   | `md validate links`                          | `md links validate`                          | Validate markdown links (relative paths + `#fragment` anchors resolve) — **cross-file**                                                                                                                                                  | **wired**                   |   ❓    | Δ **repo-wide gate** (pre-push/PR/main) — direct `cargo run`; never lint-staged (delete/rename breaks links in untouched files), was Nx `links:validation` |
| 10  | `md validate mermaid`                        | `md mermaid validate`                        | Validate Mermaid diagrams (label length, width/span, single-diagram) — **per-file**                                                                                                                                                      | **wired**                   |   ❓    | Δ **lint-staged** (`*.md`, changed files; PR via `--diff`; main over all) — direct `cargo run`, was Nx `mermaid:validation`                                |
...
| 29  | `workflows validate naming`                  | `repo-governance workflows naming validate`  | Validate workflow filename suffixes + frontmatter name consistency; Δ **domain moved `workflows`→`repo-governance`** (workflow docs live in `repo-governance/workflows/`)                                                                | **wired**                   |   ❓    | Nx `naming:workflows-validation` (target name unchanged — `:`-scheme) → pre-push (scoped) + PR gate                                                        |
...
| 37  | `specs validate gherkin-cardinality`         | `specs gherkin-cardinality validate`         | Audit `.feature` scenarios for repeated primary Given/When/Then keywords — **per-file**; `.feature` files only (unchanged — no markdown scanning)                                                                                        | **wired**                   |   ❓    | Δ **lint-staged** (`*.feature`, changed files; PR via `--diff`; main over all) — direct `cargo run`, was Nx `specs:gherkin-cardinality-validation`         |
