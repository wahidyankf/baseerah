...
inside `test:quick` (step 1) via `test:specs`, which runs, in order: `specs:adoption-validation`,
`specs:tree-validation`, `specs:counts-validation`, `specs:links-validation`, `specs:behavior:coverage`
(`--require-consumption`), and — **on `*-be` backends only** — `specs:domain:coverage`.
(`specs:gherkin-cardinality-validation` is **not** in this set — it is a per-file `.feature` check that
runs in lint-staged at pre-commit stage 1, not via `test:specs`.) These are the six `specs:*` columns in
the [§2 per-project matrices](#2-per-project-target-matrices).
...
| --------------------------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `specs:adoption-validation` | `cargo run --release -- specs adoption validate`                                | the app/lib has adopted BDD + DDD — no orphan project                                                                         | every project        |
| `specs:tree-validation`     | `cargo run --release -- specs tree validate`                                    | the canonical C4-aware five-folder spec tree                                                                                  | every project        |
| `specs:counts-validation`   | `cargo run --release -- specs counts validate`                                  | each required spec subfolder holds ≥ 1 spec file                                                                              | every project        |
| `specs:links-validation`    | `cargo run --release -- specs links validate`                                   | markdown links inside spec files resolve                                                                                      | every project        |
| `specs:behavior:coverage`   | `cargo run --release -- specs behavior-coverage validate --require-consumption` | every Gherkin step has a step def **and** every feature **+ scenario** is exercised by ≥ 1 eligible unit/integration/e2e test | every project        |
| `specs:domain:coverage`     | `cargo run --release -- specs domain-coverage validate`                         | every domain entity in `specs/apps/<domain>/domain/**` is exercised by ≥ 1 domain unit test                                   | `*-be` backends only |
...
extra columns (`test:coverage`, the **six `specs:*` validators**, `build`) are
required-where-applicable. The six `specs:*` columns — **adoption**, **tree**, **counts**, **links**,
**behavior:cov**, **domain:cov** — are the Nx targets `specs:adoption-validation`,
`specs:tree-validation`, `specs:counts-validation`, `specs:links-validation`, `specs:behavior:coverage`,
and `specs:domain:coverage` (`*-be` only); `test:specs` runs exactly this set (see
[§1](#1-lifecycle-stage--exact-commands-post-implementation-identical-across-3-repos)).
`specs:gherkin-cardinality` is **not** among them — it is a per-file `.feature` lint-staged check, not
...
| Project                    | Type             | typecheck | lint | test:unit | test:coverage |  test:integration  | test:e2e | test:quick | adoption | tree | counts | links | behavior:cov | domain:cov | build  |
| -------------------------- | ---------------- | :-------: | :--: | :-------: | :-----------: | :----------------: | :------: | :--------: | :------: | :--: | :----: | :---: | :----------: | :--------: | :----: |
