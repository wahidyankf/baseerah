...
| **Hook/gate step order**               | see [§1.2a](#12a-lifecycle-stage--exact-commands-post-implementation-identical-across-3-repos)                  | the normative per-stage command sequence (pre-commit, pre-push, PR, main) lives there — locked + identical across repos                                                                  |
...
gate — ever runs them.** Every gate runs only `test:quick` (plus, from pre-push onward, the
governance/spec validators). The heavy levels — and the staging/prod deploy that depends on them —
run **only** in the scheduled CRON pipelines
([§1.4](#14-post-merge-main-ci--per-project-staging-deploy)). The exact per-stage command sequence for
every gate is [§1.2a](#12a-lifecycle-stage--exact-commands-post-implementation-identical-across-3-repos);
the visual flow is [§5.1](#51-sdlc-gate-flow-target-standard-shared-mechanics).

The diagram below shows only what is local to this section — the `test:quick` composition and the
three levels sharing the same Gherkin (gate routing lives in §1.2a/§5.1):

```mermaid
flowchart TD
    Q["test:quick (parallel: false)"] --> Q1[nx run P:typecheck]
    Q1 --> Q2[nx run P:lint]
    Q2 --> Q3["nx run P:test:unit"]
    ...
    class Q,Q1,Q2,Q3,INT,E2E comp
    class F spec
```

### 1.2a Lifecycle Stage → Exact Commands (post-implementation, identical across 3 repos)
...
[§5.1](#51-sdlc-gate-flow-target-standard-shared-mechanics). [Repo-grounded — current hooks/CI;
...
| --- | ------------------------------------ | --------- | ----------------------------------------------------------------------------------------------------------------------- |
| 1   | `./scripts/git-identity-check.sh`    | all       | verifies the committer's git `user.name`/`user.email` are configured                                                    |
| 2   | `./scripts/check-no-env-staged.sh`   | file-type | aborts the commit if any real `.env*` file is staged                                                                    |
| 3   | `lint-staged`                        | file-type | auto-formats staged files by file type (formatter map = [§1.1](#11-nx-target-name-standard-targets-invoked-by-hooksci)) |
| 4   | `nx run rhino-cli:shell:check`       | file-type | shellcheck-lints staged `*.sh` at warning threshold (Δ was inline `shellcheck`)                                         |
| 5   | `nx run rhino-cli:dockerfiles:check` | file-type | hadolint-lints staged Dockerfiles at warning threshold (Δ was inline `hadolint`)                                        |
| 6   | `nx run rhino-cli:actions:check`     | file-type | actionlint-lints staged GitHub workflow YAML (Δ was inline `actionlint`)                                                |
| 7   | `cargo run … -- git pre-commit`      | all       | regenerates platform-binding artifacts (`.opencode/`, `.amazonq/`) and auto-stages them                                 |
| 8   | `nx affected -t test:quick`          | affected  | runs typecheck → lint → test:unit per affected project (`parallel: false`)                                              |

**Formatting and lint both live here**, not in a separate stage: step 3 formats staged files
(lint-staged, by file type — the formatter map is the SSOT in
[§1.1](#11-nx-target-name-standard-targets-invoked-by-hooksci)); step 8's `test:quick` runs `lint`
(plus typecheck + test:unit). Steps 4–6 are cacheable Nx targets (were inline shell) and stay
**tool-gated** (skip-with-hint when the linter is absent locally — CI is the hard gate).
...
| #   | Command                                                                                                                                 | Scope            | What it does                                                                                                                                                                                                                                                       |
| --- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `nx affected -t test:quick`                                                                                                             | affected         | runs typecheck → lint → test:unit per affected project                                                                                                                                                                                                             |
| 2   | `npm run lint:md`                                                                                                                       | all              | lints all markdown: mermaid + links + heading-hierarchy + gherkin-cardinality                                                                                                                                                                                      |
| 3   | `nx run rhino-cli:env:validation`                                                                                                       | all              | validates each app's `.env.example` against the repo env contract                                                                                                                                                                                                  |
| 4   | `nx affected -t specs:behavior:coverage specs:adoption-validation specs:tree-validation specs:counts-validation specs:links-validation` | affected         | checks every Gherkin feature/scenario is consumed by an eligible test, plus structural specs validators (adoption/tree/counts/links). Δ renamed from `specs:coverage`; `specs:domain:coverage` resolves here too but **only** on `*-be`; Δ `test-coverage` REMOVED |
...
Step 5 — `nx run rhino-cli:<target>`, each path-gated on changed files:

| Validator (`nx run rhino-cli:…`)     | What it validates                                                                                           | Trigger (changed paths)                                   |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `naming:harness-validation`          | agent/skill filenames match the harness naming convention                                                   | `.claude/agents/` or `.opencode/agent/`                   |
| `naming:workflows-validation`        | workflow-doc filenames match the workflow naming convention                                                 | `repo-governance/workflows/`                              |
| `governance:vendor-audit-validation` | governance docs stay vendor-neutral (no vendor leakage)                                                     | `repo-governance/**.md`                                   |
| `cross-vendor:parity-validation`     | agent behaviour is consistent across vendor bindings                                                        | parity surfaces (agents, `AGENTS.md`, `CLAUDE.md`)        |
| `harness:bindings-validation`        | generated bindings match source (`.claude/` → `.opencode/`, `.amazonq/`); Δ now an Nx target, was `npm run` | binding surfaces                                          |
| `instruction-size:validation`        | auto-loaded instruction files stay within their byte budget (reads `repo-config.yml` `instruction-size:`)   | any instruction surface (`AGENTS.md`, `CLAUDE.md`, rules) |
...
| format               | `lint-staged` on changed files, commit fix back to PR branch                                                                        | file-type |
| shell/docker/actions | `shell:check` / `dockerfiles:check` / `actions:check`                                                                               | file-type |
| `<lang>` gate        | `nx affected -t typecheck lint test:quick specs:behavior:coverage` (rust also `deny:check`, `msrv:check`; Δ `format:check` REMOVED) | affected  |
| markdown             | `npm run lint:md` (mermaid + links + heading-hierarchy + gherkin-cardinality)                                                       | all       |
| env                  | `nx run rhino-cli:env:validation`                                                                                                   | all       |
| specs-structural     | adoption + tree + counts + links validators                                                                                         | affected  |
...
| #   | Command                      | Scope    | What it does                                                                                                                                          |
| --- | ---------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `nx affected -t test:quick`  | affected | re-runs typecheck → lint → test:unit on each affected project (catches main-only / merge-skew breakage)                                               |
| 2   | governance + spec validators | mixed    | identical set to pre-push/PR: `lint:md` (all), `env:validation` (all), `specs:behavior:coverage` + structural (affected), path-gated governance (all) |

`test:integration`, `test:e2e`, and deploy run **only** in the scheduled CRON pipelines
([§1.4](#14-post-merge-main-ci--per-project-staging-deploy)) — never in this or any other gate.
...
non-symmetric `build`/`specs:behavior:coverage`; the mandatory six are NEVER absent). Target presence is
...
lifecycle, unchanged in mechanics — only renamed per [§1 CRON shape](#1-target-standard-best-of-three-synthesis)):
...
| lint-staged config (`package.json` `lint-staged` block / `.lintstagedrc`)                 | ensure one identical glob→formatter map (§1.1 SSOT) covering every shipped file type, incl. `*.rs`→`rustfmt`, `*.fs`→`fantomas`, and the **direct** `*.cs`→`dotnet csharpier format`, `*.clj`→`cljfmt fix`, `*.dart`→`dart format` (all 3); **delete `scripts/format-{csharp,clojure,dart}.sh`** (keep only `scripts/format-elixir.sh`); add **CSharpier** (local dotnet tool) + **cljfmt** (native binary) to `npm run doctor`                                                                                   |
