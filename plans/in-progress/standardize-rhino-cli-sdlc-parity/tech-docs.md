...
```yaml
# repo-config.yml (root, all 3 repos)
instruction-size: # ← former instruction-size-budget.yaml
  # per-surface byte budgets …
env-contract: # ← former env-contract.yaml
  # code↔config env surfaces …
env-injection: # ← former env-injection.yaml
  # CI/runtime injection map …
```

**Rules:**

- The rhino-cli config loaders (`convention validate instruction-size`, `env validate`/`init`/`backup`/`restore`, the `env injection` checker) read their section from `repo-config.yml` instead of the standalone file; a missing section is a hard error (no silent default).
...
- Identical structure across all 3 repos (values differ; the section schema does not). [Judgment call — consolidation; schema per former file is Repo-grounded]
...
| `test:unit`        | **BDD step tests + non-BDD unit tests**; mocks all I/O; consumes the project's Gherkin feature files. Coverage is gated by the sibling **`test:coverage`** target (below), not here                                                                                                                                                                                                                                                                    |
...
is folded into the `specs validate behavior-coverage` command (renamed from `specs validate coverage`;
Nx `specs:behavior:coverage`), which today asserts every Gherkin **step** has a matching step
definition; it is extended to **also** assert every feature **file** is referenced by ≥ 1 test
(`--require-consumption`, default on). Failure message names the orphan:
`orphan feature: <path> not consumed by any test`. See
...
**Domain-coverage enforcement** (`*-be` only) — the new `specs validate domain-coverage` command (Nx
...
| 2   | `repo-governance validate vendor`            | `repo-governance validate vendor`            | Scan governance markdown for forbidden vendor-specific terms                                                                                       | **wired**                   |    ?    | Nx `governance:vendor-audit-validation` → pre-push (scoped); **drift: not in primer pre-push**                               |
| 3   | `repo-governance validate layer-coherence`   | `repo-governance validate layer-coherence`   | Audit governance docs for layer numbering/naming coherence                                                                                         | not wired                   |    ?    | only via `repo-governance audit` (manual)                                                                                    |
| 4   | `repo-governance validate traceability`      | `repo-governance validate traceability`      | Audit governance docs for required traceability sections                                                                                           | not wired                   |    ?    | only via `repo-governance audit` (manual)                                                                                    |
...
| 6   | `md validate naming`                         | `md validate naming`                         | Validate markdown filenames are lowercase-kebab-case                                                                                               | not wired                   |    ?    | only via `md audit` (manual)                                                                                                 |
| 7   | `md validate frontmatter`                    | `md validate frontmatter`                    | Validate doc YAML frontmatter against area-specific schemas                                                                                        | not wired                   |    ?    | only via `md audit` (manual)                                                                                                 |
| 8   | `md validate heading-hierarchy`              | `md validate heading-hierarchy`              | Validate heading hierarchy (one H1, no skipped levels)                                                                                             | **wired**                   |    ?    | Nx `headings:hierarchy-validation` → pre-commit + markdown workflow                                                          |
| 9   | `md validate links`                          | `md validate links`                          | Validate markdown links (relative paths + `#fragment` anchors resolve)                                                                             | **wired**                   |    ?    | Nx `links:validation` → pre-commit + markdown workflow                                                                       |
| 10  | `md validate mermaid`                        | `md validate mermaid`                        | Validate Mermaid diagrams (label length, width/span, single-diagram)                                                                               | **wired**                   |    ?    | Nx `mermaid:validation` → pre-commit + markdown workflow                                                                     |
| 11  | `md validate frontmatter-dates`              | `md validate frontmatter-dates`              | Audit markdown for forbidden manual date metadata                                                                                                  | not wired                   |    ?    | only via `md audit` (manual)                                                                                                 |
| 12  | `md validate readme-index`                   | `md validate readme-index`                   | Audit directory README indexes against sibling markdown                                                                                            | not wired                   |    ?    | only via `md audit` (manual)                                                                                                 |
| 13  | `md frontmatter-dates` (alias)               | `md frontmatter-dates` (alias)               | Alias of `md validate frontmatter-dates`                                                                                                           | not wired                   |    ?    | manual                                                                                                                       |
| 14  | `md readme-index` (alias)                    | `md readme-index` (alias)                    | Alias of `md validate readme-index`                                                                                                                | not wired                   |    ?    | manual                                                                                                                       |
| 15  | `md audit`                                   | `md audit`                                   | Run all md validators in sequence; aggregate findings                                                                                              | not wired                   |    ?    | manual aggregate                                                                                                             |
| 16  | `convention validate emoji`                  | `convention validate emoji`                  | Audit forbidden file types for emoji codepoints                                                                                                    | not wired                   |    ?    | only via `convention audit` (manual)                                                                                         |
| 17  | `convention validate license`                | `convention validate license`                | Verify per-directory LICENSE files match the licensing convention                                                                                  | not wired                   |    ?    | only via `convention audit` (manual)                                                                                         |
| 18  | `convention validate instruction-size`       | `convention validate instruction-size`       | Audit all auto-loaded instruction surfaces against per-surface byte budgets (`instruction-size-budget.yaml`); legacy alias: `agents-md-size`       | **wired**                   |    ?    | Nx `rhino-cli:instruction-size:validation` → pre-push (changed-path gate) + PR gate                                          |
| 19  | `convention audit`                           | `convention audit`                           | Run all convention validators; aggregate findings                                                                                                  | not wired                   |    ?    | manual aggregate                                                                                                             |
| 20  | `harness validate naming`                    | `harness validate naming`                    | Validate agent filename suffixes + `.claude`↔`.opencode` mirror parity                                                                             | **wired**                   |    ?    | Nx `naming:harness-validation` → pre-push (scoped) + PR gate                                                                 |
| 21  | `harness validate duplication`               | `harness validate duplication`               | Detect verbatim duplication across agent + skill files                                                                                             | not wired                   |    ?    | only via `harness audit` (manual)                                                                                            |
| 22  | `harness validate claude`                    | `harness validate claude`                    | Validate Claude Code agent/skill format in `.claude/`                                                                                              | not wired                   |    ?    | npm `validate:claude` (manual script)                                                                                        |
| 23  | `harness validate sync`                      | `harness validate sync`                      | Validate `.claude/` and `.opencode/` are in sync                                                                                                   | not wired                   |    ?    | npm `validate:sync` (manual script)                                                                                          |
| 24  | `harness validate bindings`                  | `harness validate bindings`                  | Validate Amazon Q binding bridge files + catalog coverage                                                                                          | **wired**                   |    ?    | npm `harness:bindings-validation` → pre-push (scoped)                                                                        |
| 25  | `harness sync opencode`                      | `harness sync opencode`                      | Sync Claude Code agents → OpenCode format                                                                                                          | **wired** `[Unverified]`    |    ?    | pre-commit `git pre-commit` auto-sync (CLAUDE.md claims auto-sync; confirm in Phase 1)                                       |
| 26  | `harness emit amazonq`                       | `harness emit amazonq`                       | Emit Amazon Q Developer binding bridge files (idempotent)                                                                                          | **wired** `[Unverified]`    |    ?    | pre-commit `git pre-commit` auto-sync (confirm in Phase 1)                                                                   |
| 27  | `harness generate bindings`                  | `harness generate bindings`                  | Generate all platform bindings (sync OpenCode + emit Amazon Q)                                                                                     | **wired** `[Unverified]`    |    ?    | pre-commit auto-sync + npm `generate:bindings` (confirm in Phase 1)                                                          |
| 28  | `harness audit`                              | `harness audit`                              | Run all harness validators; aggregate findings                                                                                                     | not wired                   |    ?    | manual aggregate                                                                                                             |
| 29  | `workflows validate naming`                  | `workflows validate naming`                  | Validate workflow filename suffixes + frontmatter name consistency                                                                                 | **wired**                   |    ?    | Nx `naming:workflows-validation` → pre-push (scoped) + PR gate                                                               |
| 30  | `specs validate adoption`                    | `specs validate adoption`                    | Verify an app has adopted BDD + DDD practices (no orphan app)                                                                                      | **wired**                   |    ?    | Nx `specs:adoption-validation` → pre-push + PR gate                                                                          |
| 31  | `specs validate counts`                      | `specs validate counts`                      | Validate each required spec subfolder has ≥1 spec file                                                                                             | **wired**                   |    ?    | Nx `specs:counts-validation` → pre-push + PR gate                                                                            |
| 32  | `specs validate links`                       | `specs validate links`                       | Check markdown links in spec files resolve                                                                                                         | **wired**                   |    ?    | Nx `specs:links-validation` → pre-push + PR gate                                                                             |
| 33  | `specs validate tree`                        | `specs validate tree`                        | Validate canonical C4-aware five-folder spec tree                                                                                                  | **wired**                   |    ?    | Nx `specs:tree-validation` → pre-push + PR gate                                                                              |
| 34  | `specs validate coverage`                    | `specs validate behavior-coverage`           | Validate every Gherkin step has a matching step definition **and** every feature file is consumed by ≥1 test (`--require-consumption`, new — §1.2) | **wired**                   |   ✅    | Nx `specs:behavior:coverage` (renamed from `specs:coverage`) → pre-push + PR gate                                            |
| 34b | **— (new)**                                  | `specs validate domain-coverage`             | Validate every domain entity in `specs/apps/<domain>/domain/**` (bounded-context/ubiquitous-language registry) is exercised by ≥1 domain unit test | **new** → **wire (`*-be`)** |   ✅    | Nx `specs:domain:coverage` → pre-push + PR gate, **only on `*-be` backend projects**                                         |
| 35  | `specs validate bc`                          | `specs validate bc`                          | Validate bounded-context structural parity against the registry                                                                                    | not wired                   |    ?    | no Nx target; manual                                                                                                         |
| 36  | `specs validate ul`                          | `specs validate ul`                          | Validate ubiquitous-language glossary parity against the registry                                                                                  | not wired                   |    ?    | no Nx target; manual                                                                                                         |
| 37  | `specs validate gherkin-cardinality`         | `specs validate gherkin-cardinality`         | Audit `.feature` scenarios for repeated primary Given/When/Then keywords                                                                           | **wired**                   |    ?    | Nx `specs:gherkin-cardinality-validation` → PR gate (+ markdown workflow in primer/infra)                                    |
| 38  | `specs clean java-imports`                   | `specs clean java-imports`                   | Strip unused/same-package imports from generated Java contract files                                                                               | not wired                   |    ?    | dormant (primer-oriented)                                                                                                    |
| 39  | `specs scaffold dart`                        | `specs scaffold dart`                        | Generate Dart package scaffolding around generated contract types                                                                                  | not wired                   |    ?    | dormant (primer-oriented)                                                                                                    |
| 40  | `specs audit`                                | `specs audit`                                | Run all specs validators; aggregate findings                                                                                                       | not wired                   |    ?    | manual aggregate                                                                                                             |
| 41  | `lang java validate null-safety-annotations` | `lang java validate null-safety-annotations` | Check Java packages carry required null-safety annotations                                                                                         | not wired                   |    ?    | dormant (primer-oriented)                                                                                                    |
...
| `apps/rhino-cli/src/` (+ `specs/apps/rhino/`)                                             | extend + rename `specs validate coverage`→`specs validate behavior-coverage` with the `--require-consumption` orphan-feature check; **add `specs validate domain-coverage`** (`*-be` domain-model check); **remove the `test-coverage validate` command + its specs/tests** (all 3)                                                                                                                                                                                                                               |
