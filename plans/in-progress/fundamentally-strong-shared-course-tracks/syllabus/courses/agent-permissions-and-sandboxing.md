# Agent Permissions & Sandboxing (By Example, Python)

**Course ID**: `agent-permissions-and-sandboxing` · **Format**: By Example · **Language**: Python.
...
`agent-tools-and-mcp` (the tool boundary is where permissions live) and
draws on `security-essentials` forward-referenced from Phase 1 fundamentals.
...
- **Prior topics**: `agent-tools-and-mcp` (the tool boundary),
  `the-agent-loop`, the security fundamentals from Phase 1/earlier, and
  `just-enough-python`.
- **Tools & environment**: a macOS/Linux terminal; Python 3.x under `uv`; a container runtime (Docker/
  ...
  `the-agent-loop`/`agent-tools-and-mcp`; `pytest`; Neovim/VSCode.
...
- **Goal**: wrap the [coding agent](./the-agent-loop.md) in a complete **safety layer** — a
