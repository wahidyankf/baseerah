# Self-Hosting Essentials (By Example, — (ops/config, minimal app code))

**Course ID**: `self-hosting-essentials` · **Format**: By Example · **Language**: — (ops/config,
minimal app code). **NEW** — a light self-hosting on-ramp; strictly below the heavier
`containers-and-orchestration` and `cloud-and-iac` courses.
...
the full-depth bare-metal / hypervisor course — that is `bare-metal-virtualization` (Proxmox), which
sits in the deepening arc's ops & platform cluster, mirroring the two-altitude split. This course uses
minimal app code (a small service from earlier courses) and focuses on ops/config.
...
- **Prior topics**: `backend-essentials` (a service to host) and
  `just-enough-bash` (the shell, `ssh`, scripts). A small service from
  `async-python-and-fastapi-services` or `backend-essentials` is the thing being hosted.
...
- **This altitude vs the deep one**: this module deliberately stops at "one box, one service, a proxy, a
  PaaS." Clustering, hypervisors, and fleet management (`bare-metal-virtualization`, `self-managed-kubernetes-and-gitops`) are a
...
- The industry swung from self-managed servers to fully managed PaaS/serverless for good reasons (less
  ...
  deployment anywhere. This is the light on-ramp; `containers-and-orchestration`, `cloud-and-iac`, `bare-metal-virtualization`, and `self-managed-kubernetes-and-gitops` build the heavier altitudes on top of it.
...
- `job-seeking/software-engineer` — Phase 2 · Multi-Platform Productivity (web → cloud → mobile →
  desktop) — Cloud / backend-at-scale sub-phase.
