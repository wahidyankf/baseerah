...
| -------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| IR-SE    | Yes — all three, deepening tail | Donor courses shrink. If the two new eval courses are **not** added to this manifest, IR-SE **loses eval coverage outright** — it currently gets it only via the donors. This is the sharpest risk in S1. | Decide explicitly whether IR-SE gains the light gate. If not, record that IR-SE ships without eval coverage as an accepted, documented consequence. |
| IE-SE    | Yes — all three, deepening band | Same loss shape as IR-SE. The "immediately effective" framing arguably makes the light gate mandatory here.                                                                                               | Insert the light gate at minimum; place the deep course and prerequisite ordering.                                                                  |
| FS-SE    | Yes — all three, Stage 12       | Same loss shape. A fundamentals-first path losing measurement rigor is the worst outcome of the three.                                                                                                    | Insert both eval courses plus `statistics-for-evaluation`; verify Stage 12 ordering.                                                                |
