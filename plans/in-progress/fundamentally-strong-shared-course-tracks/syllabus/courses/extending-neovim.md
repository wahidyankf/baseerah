# Extending Neovim (By Example, Lua)

**Course ID**: `extending-neovim` · **Format**: By Example · **Language**: Lua.

**Short summary**: Neovim config, plugins, LSP, keymaps in Lua

**Scope note**: turn vanilla Neovim ([topic 1](./just-enough-nvim.md)) into a real IDE-grade forge
using the Lua learned in [topic 2](./just-enough-lua.md) — plugin management, LSP, Treesitter,
...
- **Prior topics**: [topic 1 Just Enough Nvim](./just-enough-nvim.md) (modal editing fluency) and
  [topic 2 Just Enough Lua](./just-enough-lua.md) (config is written in Lua).
...
> _Content originated in the now-closed FS-SE plan (topic 3); it now lives here in
