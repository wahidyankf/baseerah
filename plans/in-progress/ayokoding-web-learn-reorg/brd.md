...
The Hugo-to-Next.js migration is finished (Hugo artifacts removed on 2026-05-22 in commit `2cd845b05`). With no live Hugo, the directory tree is purely Next.js content and we can rename folders, move files, and add redirects without fighting two routing engines simultaneously. Every additional week of new content authored against the current chaotic shape multiplies the eventual migration cost.

## Cost of Inaction

- Each new topic added at the wrong shape costs ~2-4 hours of cleanup (proven by the last three weeks of refactors).
- The `apps-ayokoding-web-by-example-checker`, `apps-ayokoding-web-by-concept-checker`, and `apps-ayokoding-web-in-the-field-checker` agents cannot enforce structural rules they cannot define; the platform's three-track learning model is hollow without a tree to match it.
...
The Hugo-era cleanup proved this is mechanical, not creative. The 2026-05-22 link-fix commit (`9035b41d9`) demonstrated the cleanup pattern works at scale (50 links fixed in one pass).
...
| Coverage regression below 82% threshold                 | low        | high   | Track folder moves don't touch test files; pre-push hook gates push                                 |
