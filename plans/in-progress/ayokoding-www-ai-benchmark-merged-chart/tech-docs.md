...
flowchart TB
  ...
  Data --> UrlState["core/url-state.ts<br/>+4 sort params (NEW)"]
...
| DD-4 | Per-band sort state lives in the URL (`sortOpus`/`sortSonnet`/`sortLight`/`sortUnrated`) | Matches the page's existing "URL is the single source of truth for view state" architecture                                                                  |
...
| `apps/ayokoding-www/src/features/ai-benchmark/core/url-state.ts`                    | **Edited** — adds `sortOpus`/`sortSonnet`/`sortLight`/`sortUnrated` params + sanitizers                                              |
...
`shell/benchmark-filters.tsx`, `shell/how-to-read.tsx`, `shell/evidence-badge.tsx`,
...
in `prd.md` per the [UI Mockups in Plan Docs convention](../../../repo-governance/conventions/formatting/diagrams.md#ui-mockups-in-plan-docs).
