# 40 · Defensive Security (blue team, SOC/IR) (By Example, Python + shell †)

**prd row**: Pass 3 · Build for the Real World · By Example · Python + shell † · Learn 140 / Drill 240 ·
...
lab-generated data. The direct counterpart to [`39-offensive-security`](./39-offensive-security.md);
applies [`38-it-security`](./38-it-security.md). This topic **closes Pass 3** and anchors **three
inter-topic capstones** (Pass-3 boundary + two cross-cutting), specified at the end of this file.

## Prerequisites

- **Prior topics**: [topic 39 Offensive Security](./39-offensive-security.md) (the attacks to detect),
  [topic 38 IT Security](./38-it-security.md) (threats, OWASP, crypto), and
  [topic 05 Just Enough Bash](./05-just-enough-bash.md) (log wrangling).
- **Tools & environment**: a macOS/Linux terminal; the **same isolated local lab** from topic 39
...
- **Assumed knowledge**: the attack lifecycle + how a web attack looks on the wire (topic 39); the OWASP
  Top 10 + threat modeling (topic 38); shell + log filtering (topic 05).
...
**Weight**: `learning/_index.md` and the drilling mirror place this capstone at **505** (Pass-3 boundary,
after topic 40; ahead of Pass 4). Kind: **subject → full runnable** (DD-27). This is the Pass-3 graduation
...
- **Goal**: take the Pass-2 `capstone-solid-core` application and deliver it the way a real team ships:
  choose and justify a data layer beyond a single SQL DB (NoSQL/graph where it fits — topics 26/27),
  scale the backend (28) with an event-driven slice (33) modeled with DDD (31) under a documented
  architecture (30) and a system-design capacity plan (32), containerize + orchestrate it (34), describe
  its infrastructure as code (35), and secure it end to end (38) with red-team validation (39) and
  blue-team detection (40) — a complete, deployed-as-code, secured, observable system.
- **Integrates topics**: 26 NoSQL · 27 Graph (where it fits) · 28 Backend at Scale · 30 Architecture ·
  31 DDD · 32 System Design · 33 Event-Driven · 34 Containers/K8s · 35 Cloud/IaC · 38 IT Security ·
  39 Offensive (validation) · 40 Defensive (detection). (36/37 optional where the domain benefits.)
- **Concepts exercised**: [ ] a justified polyglot-persistence choice [ ] a scaled, event-driven backend
  ...
  [ ] containerized + orchestrated deployment [ ] infrastructure described as code [ ] a security
...
- **Ordered steps**:
  ...
  3. `.../capstone-real-world-delivery/deploy/` — Dockerfile(s) + K8s manifests + Terraform/OpenTofu (local
     provider). Verify the app deploys to the local cluster via the IaC and self-heals.
...
**Weight**: `learning/_index.md` and the drilling mirror place this capstone at **506** (cross-cutting,
...
- **Goal**: build (or take a prior) HTTP service and make it demonstrably secure end to end — apply the
  ...
  **prove** the security by attacking it from the red-team lab (39) and **detecting** those attacks from
  the blue-team stack (40) — a single service where the full security lifecycle is visible.
- **Integrates topics**: 14 Security Essentials · 28 Backend (auth surface) · 38 IT Security (OWASP/crypto/
  identity) · 39 Offensive (validation) · 40 Defensive (detection). (34 for a hardened container image
  where used.)
...
**Weight**: `learning/_index.md` and the drilling mirror place this capstone at **507** (cross-cutting,
...
- **Goal**: build an end-to-end data path — ingest raw data through a medallion pipeline (bronze/silver/
  gold) with data-quality gates (36), model it with advanced SQL for serving (23), optionally serve a
  read-optimized store (26/27 where the access pattern fits), and expose it through an AI-powered,
  RAG-grounded query interface (37) over a backend (28) — a complete "raw data → governed warehouse →
  intelligent interface" slice.
- **Integrates topics**: 08 SQL · 23 Advanced SQL · 26 NoSQL / 27 Graph (where the serving pattern fits) ·
  28 Backend at Scale (serving) · 36 Data Engineering (the pipeline) · 37 AI-Powered Apps (the interface).
...
- **Ordered steps**:
  1. `.../capstone-data-pipeline/pipeline/` — raw → bronze → silver → gold with quality gates (36). Verify
     idempotent re-runs, a bad batch caught by the gate, and a reconciling star schema.
  2. `.../capstone-data-pipeline/serve/` — advanced-SQL serving queries (23) + optionally a read-optimized
     store (26/27). Verify serving queries match hand-computed expected results.
  3. `.../capstone-data-pipeline/interface/` — a RAG-grounded (37) query interface over the gold data,
     served via a backend endpoint (28). Verify answers are grounded in the served data + cited.
...
← Previous: [39 · Offensive Security (red team, Kali)](./39-offensive-security.md) · Next: [41 · Just Enough Go](./41-just-enough-go.md) →
