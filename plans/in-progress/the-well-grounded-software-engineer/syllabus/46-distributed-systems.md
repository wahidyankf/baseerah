...
- Distributed-systems theory grew from trying to make many unreliable machines behave like one
  ...
  [`92-build-your-own-raft`](./92-build-your-own-raft.md) and the scaling context in
...
- **ex-85 · coordination-service-vs-diy-consensus** — a decision artifact (DD-20) contrasting leaning on a coordination service (ZooKeeper/etcd/Consul) against hand-rolling consensus (ties to [`92-build-your-own-raft`](./92-build-your-own-raft.md)) for locks/leader-election — verify the trade-off (operational dependency vs build cost) and each product's license are recorded. (co-35, co-19)
