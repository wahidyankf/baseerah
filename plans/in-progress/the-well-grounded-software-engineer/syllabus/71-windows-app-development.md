# 71 · Windows App Development ◆ (By Example, C# †)

**prd row**: Pass 4 · Concurrency & Systems · By Example · C# † ◆ · Learn 171 / Drill 271 ·
...
- **Prior topics**: [topic 70 Just Enough C#](./70-just-enough-csharp.md) (the language + `async`/`await`),
...
- **Assumed knowledge**: C# syntax + `async`/`await` (topic 70); MVVM + data-binding thinking (topics
...
- Scope: **native Windows desktop apps** — installed, windowed, OS-integrated (not a web page in a
  wrapper); the OS window, message loop/dispatcher, and packaging are first-class.
...
← Previous: [70 · Just Enough C#](./70-just-enough-csharp.md) · Next: [72 · Linux App Development](./72-linux-app-development.md) →
