...
- **CQRS ≠ Event Sourcing (keep independent)** — CQRS = "use a different model to update information than the
  ...
  teach both as intros here; deep in [`45-event-driven-architecture`](./45-event-driven-architecture.md).
...
- **co-30 · cqrs** — separate the write model from the read model (intro here; deep in [`45-event-driven-architecture`](./45-event-driven-architecture.md)).
- **co-31 · event-sourcing** — persist state as an append-only log of events and rebuild state by replay (intro here; deep in [`45-event-driven-architecture`](./45-event-driven-architecture.md)).
