# 52 · Bare-Metal Virtualization (By Example, HCL/YAML/shell †)

**prd row**: Pass 3 · Build for the Real World · By Example · HCL/YAML/shell † · Learn 152 / Drill 252 ·
...
_beneath_ Kubernetes ([`50-containers-and-orchestration`](./50-containers-and-orchestration.md)) and the
public cloud ([`51-cloud-and-iac`](./51-cloud-and-iac.md)). Hypervisor fundamentals (Type-1 vs Type-2,
...
[`53-self-managed-kubernetes-and-gitops`](./53-self-managed-kubernetes-and-gitops.md); here the hypervisor
...
- **Prior topics**: [topic 50 Containers & Orchestration](./50-containers-and-orchestration.md) (the
  workloads that will run on the VMs, and the immutable-image intuition), [topic 51 Cloud & IaC](./51-cloud-and-iac.md)
  (Terraform plan/apply/state, idempotency, declarative-vs-imperative — reused here against Proxmox instead
  of a cloud), and [topic 12 Networking Essentials](./12-networking-essentials.md) (DHCP, bridges, subnets —
...
- The arc runs mainframe time-sharing → per-app physical servers (the "snowflake" era, one workload one box,
  ...
  [`50-containers-and-orchestration`](./50-containers-and-orchestration.md) and declarative infra in
  [`51-cloud-and-iac`](./51-cloud-and-iac.md), now one layer lower.
...
- **Goal**: provision a reproducible on-prem virtualization substrate as code — a Packer-built golden image,
  ...
  ([`53`](./53-self-managed-kubernetes-and-gitops.md)) can sit on.
