...
- **Processes preview** — hexdocs `processes.html`: `spawn/1` ("takes a function which it will execute in
  ...
  [`67-actor-model-concurrency`](./67-actor-model-concurrency.md)). `[Verified]`
...
- **co-23 · process-spawn-preview** — `spawn/1` starts a function in a new, isolated, lightweight process
  and returns its PID (depth in [`67-actor-model-concurrency`](./67-actor-model-concurrency.md)).
...
← Previous: [65 · CSP-Style Concurrency](./65-csp-style-concurrency.md) · Next: [67 · Actor-Model Concurrency](./67-actor-model-concurrency.md) →
