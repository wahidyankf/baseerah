...
| `repo-governance/workflows/pr/pr-review-maker-fixer-cycle.md` _(New file)_        | New workflow doc defining the sequential N-cycle `pr-review-maker` → `pr-review-fixer` loop, the per-cycle mechanics, the CI-green-between-cycles gate, the done-definition, and the AI-attribution rule. Referenced from `plan-execution.md` Step 8. Placed under a **new** `repo-governance/workflows/pr/` directory (sibling to `plan/`, `ci/`, `ui/` — none named `pr/` exists yet [Repo-grounded]).                                                                                                                                                                                                                                                              |
...
`repo-governance/workflows/pr/pr-review-maker-fixer-cycle.md`.
...
4. **[Unverified] Review-bot identity + attribution.** The two review agents should post under a
   distinct **bot / GitHub App identity** (not a human PAT), with an attribution footer per Anthropic's
   own convention (e.g. a `— generated by AI` line on every comment/reply). Confirm the bot identity
   and token provisioning at execution time; do NOT reuse a personal token. This intersects the repo's
   Git Identity Guardrail (agents never set git identity) — the bot identity is a GitHub App/CI
   credential, not a `git config user.*` change.
