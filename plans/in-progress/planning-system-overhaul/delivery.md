...
- [ ] Install dependencies in root worktree: `npm install`
      — acceptance: exits 0, `node_modules/` synchronized
- [ ] Converge polyglot toolchain in root worktree: `npm run doctor -- --fix`
      — acceptance: exits 0 with no unresolved drift
- [ ] Verify markdown tooling: `npm run lint:md -- --help`
      — acceptance: help text displayed, no "command not found"
- [ ] Run baseline markdown lint: `npm run lint:md`
      — acceptance: baseline pass/fail count recorded; all preexisting violations documented
- [ ] Resolve all preexisting markdown lint violations before proceeding
...
- [ ] Edit `repo-governance/workflows/plan/plan-execution.md`: in the **Step 0 opening
...
- [ ] Edit `repo-governance/workflows/plan/plan-execution.md`: in **Step 0 Orchestrator action
...
- [ ] Edit `repo-governance/workflows/plan/plan-execution.md`: remove the `"On failure"` note
...
- [ ] Edit `repo-governance/workflows/plan/plan-execution.md`: update the **"Why this is a hard
...
- [ ] Edit `repo-governance/workflows/plan/plan-execution.md`: in **Step 1b**, add a reference
...
- [ ] Edit `repo-governance/development/workflow/test-driven-development.md`: after the
...
- [ ] Edit `repo-governance/development/workflow/test-driven-development.md`: in the **"Applying
...
- [ ] Edit `.claude/agents/plan-maker.md`: in the **"Planning Workflow"** section, renumber
...
- [ ] Edit `.claude/agents/plan-maker.md`: insert **Step 1: Grill the User (Mandatory —
...
- [ ] Edit `.claude/agents/plan-maker.md`: append **Step 8: Grill the User (Mandatory —
...
- [ ] Edit `.claude/agents/plan-maker.md`: update the **delivery checklist template** to always
...
- [ ] Edit `AGENTS.md`: in the plan-maker agent reference, add a note that plan-maker mandates
...
- [ ] Edit `AGENTS.md`: in the Workflows section (or nearby), add a reference to
      `plan-establishment` workflow
      — acceptance: `grep -n "plan-establishment" AGENTS.md` returns at least one line

- [ ] Edit `AGENTS.md`: in the AI Agents section, add `repo-setup-manager` to the appropriate
...
- [ ] Create `repo-governance/workflows/plan/plan-establishment.md` with the complete content
...
- [ ] Edit `repo-governance/workflows/plan/README.md`: add a plan-establishment entry to the
...
- [ ] Create `.claude/agents/repo-setup-manager.md` with the complete content specified in
...
- [ ] Edit `.markdownlintignore`: append `plans/done/` and `archived/` (with a comment line
...
- [ ] Edit `.markdownlint-cli2.jsonc`: add `"plans/done/**"` and `"archived/**"` to the
      `ignores` array
      — acceptance: `grep -n '"plans/done' .markdownlint-cli2.jsonc` returns one line; `grep -n '"archived/' .markdownlint-cli2.jsonc`
      returns one line

- [ ] Edit `repo-governance/development/quality/markdown.md`: add a new **Archive Exclusion**
...
- [ ] Run `npm run generate:bindings` [Repo-grounded] from repo root to sync
...
- [ ] Lint all markdown: `npm run lint:md`
      — acceptance: exits 0, no violations reported
- [ ] Format check markdown: `npm run format:md:check`
      — acceptance: exits 0, no formatting differences reported
- [ ] Fix ALL violations found above (including any preexisting issues unrelated to this plan)
      before pushing — root cause orientation principle applies
      — acceptance: no outstanding violations of any kind remain
- [ ] If any violations found, auto-fix and recheck: `npm run lint:md:fix && npm run format:md`
...
- [ ] Stage and commit Phase 1 changes (plan-execution.md):
...
- [ ] Stage and commit Phase 2 changes (test-driven-development.md):
...
- [ ] Stage and commit Phase 5 changes (plan-establishment.md + README.md):
...
- [ ] Stage and commit Phase 6 changes (repo-setup-manager.md):
...
- [ ] Stage and commit Phase 8 changes (bindings sync only):
