...
- [ ] [AI] **RED**: add a `.feature` scenario (+ step def) asserting a `repo-config.yml` **schema-parity gate** (Decision 10): given the three repos' configs, the gate passes when they share an identical key set (values differing) and fails when a required key is missing or an unknown key is present — command: `cargo test -p rhino-cli` — acceptance: new scenario fails (no schema-parity check exists yet).
...
- [ ] [AI] **GREEN**: implement the schema-parity check (a rhino-cli subcommand/validator that asserts `repo-config.yml`'s key set matches the canonical key set) and wire it into `.husky/pre-push` + the PR-gate specs job — command: `cargo test -p rhino-cli` — acceptance: new scenario passes; `nx run rhino-cli:specs:behavior:coverage` still exits 0; the gate is present in `.husky/pre-push`.
  - _Suggested executor: `swe-rust-dev`_
- [ ] [AI] **REFACTOR**: tidy the schema-parity validator (naming, error message lists the offending key) — command: `cargo clippy -p rhino-cli -- -D warnings` — acceptance: clippy passes; all tests green.
...
- [ ] [AI] Canonicalize the `repo-config.yml` header comment block (restore the `env-injection` line; one wording). Acceptance: `grep -c 'env-injection' repo-config.yml` returns `1`; the canonicalized header comment block is recorded verbatim in `audit/repo-config-header.md` as the copy-source for Phases 3–4.
...
- [ ] [AI] Config/targets/specs parity: `for p in $(npx nx show projects --json | jq -r '.[]'); do npx nx show project "$p" --json | jq -e '.namedInputs.specs' >/dev/null || echo "MISSING: $p"; done` prints nothing in all 3 repos (29/29 public, 26/26 primer, 8/8 infra — including all 4 contracts projects `organiclever-contracts`, `ose-contracts`, `crud-contracts`, `coralpolyp-contracts`); mandatory-target loop (same `nx show projects` enumeration) clean in all 3; `repo-config.yml` schema + header + harness list identical (`diff` the header comment block + top-level keys) and the schema-parity gate passes in all 3; no orphan spec dir. Acceptance: all green.
