...
| 2     | crane-be skeleton (health + HTTP + hexagonal + fake adapter) | Deployable service with fake PDF→md adapter, TDD                                |
| 3     | crane-be real PDF→md adapter + NATS subscriber               | Real PdfPig/Tesseract adapter via lib; NATS request/reply                       |
| 4     | organiclever-be messaging context                            | NATS client, crane clients, JetStream demo, env + drift guard                   |
| 5     | ose-app-be messaging context                                 | Same as Phase 4 for ose-app-be                                                  |
| 6     | Production Dockerfiles + sqlx::migrate! + compose NATS       | Three prod Dockerfiles; migrate on boot; NATS in compose                        |
| 7     | GHCR affected-aware publish workflow                         | Public images for all three backends/services                                   |
| 8     | Specs completeness + spec-coverage + docs                    | Spec sets complete; conventions and architecture docs updated                   |
| 9     | Final Quality Gate + Commit + Push + CI verify               | All gates green locally and in CI                                               |
