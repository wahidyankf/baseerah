...
- [ ] [HUMAN] Verify each app-web **staging** deployment serves the `stag-*-app-web` build (200) at its private staging URL — keep the URL private; do not paste it into this checklist or any committed file. **PROGRESS 2026-06-15** — staging Previews were skipped entirely (the `vercel.json` `ignoreCommand` only allowed the prod branch); fixed in `c4bc603d0` to also build `stag-*-app-web`. After re-push, both staging `WEB_BASE_URL`s flipped from **404** to **401** (deployed + Deployment-Protection on — expected, since unauthenticated requests are blocked). **Remaining for operator**: confirm `200` via an authenticated browser session (or the bypass below). Agent cannot reach the protected URL.
