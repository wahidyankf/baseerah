...
- **Relationship to `DD-15`**: where this course names real software, the same precedent from #19
  applies — permissively licensed examples (e.g. Apache Fineract, which documents Islamic-banking
  product configurability) are named directly; copyleft projects are described behaviourally only, per
  [tech-docs §Licensing](../../tech-docs.md#licensing-and-ip-compliance-a8).

## Concepts

1. **co-01 · contract-type-aware-posting** — extending #19's posting engine so each entry carries an
...
2. **co-02 · substance-validation-at-posting-time** — structurally checking a transaction's
...
3. **co-03 · zakah-fund-isolation** — architecting #22's Zakah obligation as a structurally separate
   fund/ledger, never a sub-account of the general tax-liability structure.
4. **co-04 · sukuk-holder-register** — a subledger tracking sukuk-holder positions and their
   asset/usufruct-based distributions, architecturally distinct from a conventional bondholder
   liability register.
5. **co-05 · sharia-board-audit-trail** — architecting the system so a Sharia supervisory board's
...
6. **co-06 · dual-manifest-shared-architecture** — the same posting-engine and reconciliation
...
7. **co-07 · reconciliation-across-sharia-subledgers** — extending #19's reconciliation architecture so
...
- **ex-03 · design-a-substance-check** — design (on paper) a posting-time check that would have caught
  ...
  verify the check's criteria matches #21's co-08 asset-risk-transfer-timing test. (co-02, `form-vs-
substance`)
...
- **Apache Fineract** (Apache-2.0) — a real, permissively licensed reference implementation whose
  configurable product framework has documented Islamic-banking product support; named nominatively per
  `DD-15`, never quoted from.
