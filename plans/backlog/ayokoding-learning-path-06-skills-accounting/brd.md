...
- **Three stage signals emitted** (observable): each of the three signals is recorded with all five
  fields and a real merged commit SHA, as a human/audit-readable record of the handoff. Plan 07's own
  gates verify readiness independently (via `test -d` checks against `origin/main`) and never read
  this record.
...
| **Plan 02's doc-level prerequisite-omission rule reads as forbidding this plan's manifest.**                                                  | Recorded as OI-4; plan 02 has already published a dated ruling resolving it (`tech-docs.md §"Link-don't-walk: prerequisite omission is permitted (OI-4 ruling, 2026-07-21)"`, ruling Direction A permitted and naming this plan's OI-4 as the trigger). The implemented `checkPrerequisiteConsistency` already permitted it before the ruling; Phase 0 confirms and records the ruling rather than negotiating a new one.                                                                                                        |
...
| **The two skills plans collide** on a shared manifest directory or the skills structural index.                                               | Ownership is scoped to exactly one manifest **data** file per plan (`manifests/skills/accounting.yaml` here) plus that manifest's own co-located unit test — no test file is shared across plans, so there is no cross-plan merge seam — and neither skills plan creates any `_index.md`; A3 assigns every structural index to plan 01. The Phase 6 ownership check asserts this plan's **commit** footprint, scoped to its own merged PRs rather than to a commit range, so a sibling plan merging concurrently cannot fail it. |
