...
- a **`paths/` content home** whose grid landing grows to fit the current path roster, populated as
  each ships, plus the five deeper structural indexes underneath it (`careers/`, its three arcs, and
  `skills/`) so no URL segment inside `paths/` is ever a dead one (amendment A3);
...
`apps-ayokoding-www-general-fixer` (the two hub-file rewrites) [Repo-grounded — each has a definition
...
- **Every URL segment under `paths/` renders, none 404 (observable, amendment A3)**: all six
  structural indexes exist —
  `find apps/ayokoding-www/content/en/learn/paths -name _index.md | wc -l` returns **6** — and each
  carries a title and at least one sentence of body, verified before any of the three downstream
  manifest-owning plans (05, 06, 07) has published a card into it. Falsifiable in both directions: the
  count is **1** before this plan's Phase 1 (`paths/_index.md` alone) and would be **5** or fewer if
  any arc or category index were missed.
...
- **Authoring or composing any path manifest** — every file under
  ...
  `ayokoding-learning-path-05-manifests`. This plan creates **every structural index under
  `<PATHS>`** — `paths/_index.md`, `paths/careers/_index.md`, the three
  `paths/careers/<arc>/_index.md` (`interview-ready`, `immediately-effective`,
  `fundamentally-strong`), and `paths/skills/_index.md` — six files total (amendment A3,
  2026-07-21). It publishes no manifest and populates no path card in any of them.

  **Why centralise all six here.** `paths/skills/_index.md` sits on a cross-plan seam: the two
  sibling `skills/` plans (`ayokoding-learning-path-06-skills-accounting` and
  `ayokoding-learning-path-07-skills-erp`) both populate cards into it, and assigning the file to
  either would leave it half-owned by the other. Assigning **every** structural index under
  `<PATHS>` to this plan — the plan that already owns the section's IA layer and its other two
  bucket homes — removes that seam instead of choosing a side. A **structural index** (a bucket:
...
| A missing structural index under `paths/` (`paths/careers/`, an arc, or `paths/skills/`) is a dead URL segment (R7) — `paths/careers/_index.md` and `paths/skills/_index.md` sit on a shared seam three sibling plans build toward. | All five deeper indexes are created **in the same Phase 1 step** as `paths/_index.md` (amendment A3, DD-49), each with a title and a sentence of body so it renders acceptably empty rather than 404s or a synthesized `weight: 0` node; Phase 1 gate asserts all six exist. The empty-state **design** is `ayokoding-learning-path-03-navigation-ui`'s, not this plan's — see [tech-docs.md DD-49](./tech-docs.md#design-decisions).                                |
