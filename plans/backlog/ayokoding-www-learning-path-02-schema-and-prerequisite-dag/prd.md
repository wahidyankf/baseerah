...
until `ayokoding-www-learning-path-03-navigation-ui` carries its own copy.
...
- As a **downstream plan author** (`ayokoding-www-learning-path-03-navigation-ui`), I want the five
...
- As a **downstream plan author** (`ayokoding-www-learning-path-04-course-authoring`), I want the
...
is rendered by `ayokoding-www-learning-path-03-navigation-ui`. Those scenarios are **owned by that
...
- _A legacy fundamentally-strong URL redirects to the canonical course URL_ — underpinned by
  ...
  `ayokoding-www-learning-path-01-url-restructure`**, not by the navigation plan.
...
- The course-prerequisite frontmatter contract `prerequisites: [course-id, ...]` — **canonical
  ...
  `ayokoding-www-learning-path-01-url-restructure`'s `tech-docs.md` as the Wave-1 parallel-safe shared
  contract.
...
- The `course-paths` Gherkin companion under
  ...
  RED (step bindings land in `ayokoding-www-learning-path-03-navigation-ui`).
- Custody of the `syllabus/` detail layer (128 files), including the archival repoint of 34
  cross-plan inbound links.

**Out-of-scope features:**

- Any `.yaml` manifest data file — owned by `ayokoding-www-learning-path-05-manifests`.
- Any `shell/` component, the `?path=` route wiring, the path rail, the path banner, the paths hub,
  and every mockup render — owned by `ayokoding-www-learning-path-03-navigation-ui`.
- Any course body, and any edit to a file under `syllabus/` — owned by
  `ayokoding-www-learning-path-04-course-authoring` (bodies) and by nobody (the corpus is frozen).
- The `<COURSES>_index.md` and `<PATHS>_index.md` content homes, the `legacy/` bucket, and every
  redirect module — owned by `ayokoding-www-learning-path-01-url-restructure`.
- Writing the actual `prerequisites:` values into the 37 re-homed course bundles — this plan defines
  the shape; `ayokoding-www-learning-path-01-url-restructure` writes the values.
...
Moved to **`ayokoding-www-learning-path-04-course-authoring`**, which owns every course body and every
...
- **Contract drift between the two Wave-1 plans** — `ayokoding-www-learning-path-01-url-restructure`
  ...
  `ayokoding-www-learning-path-03-navigation-ui`. Mitigated by reproducing the contract verbatim in both
...
- **Specs coverage delta is untraceable** — the `course-paths` Gherkin lands with no step bindings
  ...
  `ayokoding-www-learning-path-03-navigation-ui` as the plan that closes it, so the delta has a named
