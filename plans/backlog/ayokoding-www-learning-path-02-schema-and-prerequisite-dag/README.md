...
with `ayokoding-www-learning-path-01-url-restructure`.
...
| `apps/ayokoding-www/src/features/course-paths/manifests/`           | The `<MANIFESTS>` directory and its `README.md` (empty of `.yaml` files — those belong to `ayokoding-www-learning-path-05-manifests`) |
...
| The `prerequisites: [course-id, ...]` frontmatter contract          | **Canonical here.** `ayokoding-www-learning-path-01-url-restructure` writes the field; this plan defines its shape                    |
...
- The `<COURSES>_index.md` / `<PATHS>_index.md` content homes → `ayokoding-www-learning-path-01-url-restructure`.
- The UI design funnel, every mockup render, and every `shell/` component →
  `ayokoding-www-learning-path-03-navigation-ui`.
- Every `.yaml` manifest file and every step that creates, appends to, reorders, or re-verifies one →
  `ayokoding-www-learning-path-05-manifests`.
- Every course body under `<COURSES>` → `ayokoding-www-learning-path-04-course-authoring`.
...
- `syllabus/courses/<course-id>.md` (121 files) is the **literal source of truth for authoring each
  course body**, consumed by `ayokoding-www-learning-path-04-course-authoring`. Authoring "from a fresh
...
- `syllabus/paths/manifest-*.md` (4 files) are the **authoritative human-readable manifest
  orderings**, consumed by `ayokoding-www-learning-path-05-manifests`. Each YAML manifest's
...
flowchart LR
    P2["ayokoding-www-learning-path-<br/>02-schema-and-prerequisite-dag<br/>(this plan · custodian)"]:::owner
    CORPUS[("syllabus/<br/>121 course specs<br/>4 path orderings<br/>3 READMEs")]:::corpus
    P4["ayokoding-www-learning-path-<br/>04-course-authoring"]:::consumer
    P5["ayokoding-www-learning-path-<br/>05-manifests"]:::consumer
...
`grep -rn "DD-3[4-9]" plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`, which
...
| ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `ayokoding-www-learning-path-03-navigation-ui`    | `apps/ayokoding-www/src/features/course-paths/core/` — `schemas.ts`, `path-nav.ts`, `path-context.ts`, `prerequisites.ts`, `manifest-integrity.ts` | every shell component and the route wiring import from these pure modules             |
| `ayokoding-www-learning-path-03-navigation-ui`    | `apps/ayokoding-www/src/features/course-paths/manifests/` + its `README.md`                                                                        | the manifest repository loads `**/*.yaml` from this directory                         |
| `ayokoding-www-learning-path-04-course-authoring` | `syllabus/courses/<course-id>.md` — 121 settled per-course spec files                                                                              | each course body is authored **from** its spec file, never from a fresh judgment call |
| `ayokoding-www-learning-path-04-course-authoring` | the `prerequisites: [course-id, ...]` frontmatter contract                                                                                         | every net-new course `_index.md` declares it                                          |
| `ayokoding-www-learning-path-05-manifests`        | `syllabus/paths/manifest-*.md` — the four authoritative human-readable orderings                                                                   | each YAML manifest's `courseOrder` is transcribed from its mirror                     |
| `ayokoding-www-learning-path-05-manifests`        | `checkManifestIntegrity` + `checkPrerequisiteConsistency`                                                                                          | every manifest phase gate runs these                                                  |
...
- `ayokoding-www-learning-path-03-navigation-ui` — Wave 2. Cannot start until this plan's PR is merged;
  every `shell/` component imports the five `core/` modules, and `manifest-repository.ts` cannot
  validate without `schemas.ts`.
- `ayokoding-www-learning-path-04-course-authoring` — Wave 2. Cannot start until this plan's PR is
...
- `ayokoding-www-learning-path-05-manifests` — Wave 3, **transitively**. Every `courseOrder` is
...
`ayokoding-www-learning-path-01-url-restructure` writes `prerequisites: [course-id, ...]` frontmatter
...
build, surfacing only inside `ayokoding-www-learning-path-03-navigation-ui`), which is precisely why the
...
plan READMEs rather than linked. `ayokoding-www-learning-path-05-manifests` is the canonical owner for
...
(pure core + schema); its "UI" half is `ayokoding-www-learning-path-03-navigation-ui`; its content-home
half is `ayokoding-www-learning-path-01-url-restructure`. Everything from the MVP onward lives in
`ayokoding-www-learning-path-05-manifests` and `ayokoding-www-learning-path-04-course-authoring`. Do not
...
`ayokoding-www-learning-path-05-manifests` references both. This plan is the canonical owner of both.
...
`worktrees/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`, open a draft PR per phase
