...
half; `ayokoding-www-learning-path-03-navigation-ui` creates its `shell/` half.
...
└── shell/                     # NOT this plan — ayokoding-www-learning-path-03-navigation-ui
```
...
- **Prerequisites are surfaced on the page.** The canonical course page renders its declared
  ...
  `ayokoding-www-learning-path-03-navigation-ui`'s; the resolver behind it is this plan's.)
...
> **Canonical owner: this plan.** `ayokoding-www-learning-path-01-url-restructure` **writes** this field
...
`ayokoding-www-learning-path-01-url-restructure` writes a shape this plan's resolver does not parse —
...
`ayokoding-www-learning-path-03-navigation-ui`, as 37 course pages rendering an empty prerequisite list
...
`ayokoding-www-learning-path-03-navigation-ui`) parses each YAML file through it; a manifest that does
...
manifest to run them against — `ayokoding-www-learning-path-05-manifests` does. The `Published` state is
reached only once `ayokoding-www-learning-path-03-navigation-ui` has a renderer for it.
...
(three structural buckets) are owned by `ayokoding-www-learning-path-01-url-restructure`; DD-4 (graceful
canonical fallback) is owned by `ayokoding-www-learning-path-03-navigation-ui`; DD-15 and DD-27 (build
...
3. **Cross-plan references use the full relative path** —
   `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/<rest>` while both plans sit
...
| ------------------------------------------------------------- | ------------------------------------------------- |
| `tech-docs.md#course-library-catalog`                         | `ayokoding-www-learning-path-04-course-authoring` |
| `tech-docs.md#path-manifests` (and its four per-path anchors) | `ayokoding-www-learning-path-05-manifests`        |
| `tech-docs.md#path-aware-navigation-ui-ayokoding-www`         | `ayokoding-www-learning-path-03-navigation-ui`    |
| `tech-docs.md#smoothness-architecture-per-path`               | `ayokoding-www-learning-path-05-manifests`        |
| `prd.md#new-course--capstone-specifications`                  | `ayokoding-www-learning-path-04-course-authoring` |
...
Moved to **`ayokoding-www-learning-path-04-course-authoring`**. The catalog enumerates the 127-course
...
Moved to **`ayokoding-www-learning-path-05-manifests`**, which owns every manifest file and every
...
Ordering owned by `ayokoding-www-learning-path-05-manifests`; mirror at
...
Moved to **`ayokoding-www-learning-path-03-navigation-ui`**, which owns the `shell/` half of the
...
Moved to **`ayokoding-www-learning-path-05-manifests`**, which runs the per-path smoothness audits. The
...
`ayokoding-www-learning-path-03-navigation-ui`; Screen 4 (legacy landing) is owned by
`ayokoding-www-learning-path-01-url-restructure`. Neither set of artefacts is carried here, and no
...
components and route wiring built by `ayokoding-www-learning-path-03-navigation-ui`. So
...
| `plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/` | Unchanged  | Custodied; **no delivery step edits it**                                                                  |
...
- **No YAML parser is needed here.** `<MANIFESTS>` ships empty of `.yaml` files; the parser is
  `manifest-repository.ts`'s concern, in `ayokoding-www-learning-path-03-navigation-ui`.
...
- **Integration** (`test:integration`) — **not exercised by this plan**. Loading manifests from disk
  is `manifest-repository.ts`'s job, in `ayokoding-www-learning-path-03-navigation-ui`. The affected
...
- **`specs/` Gherkin companion** — authored RED under `<SPECS>`, consumed by
  `specs:behavior:coverage`; step bindings land in `ayokoding-www-learning-path-03-navigation-ui`.
