...
Worktree path: `worktrees/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`

Optional manual pre-provisioning (run from repo root):

```bash
claude --worktree ayokoding-www-learning-path-02-schema-and-prerequisite-dag
```
...
ayokoding-www-learning-path-02-schema-and-prerequisite-dag/<phase-slug>`), authors its work there,
...
1. [AI] Sync the worktree to latest `origin/main` and branch:
   `git fetch origin && git checkout main && git pull && git checkout -b
ayokoding-www-learning-path-02-schema-and-prerequisite-dag/<phase-slug>`.
...
- **Downstream (`blocks`)**: `ayokoding-www-learning-path-03-navigation-ui` (Wave 2),
  `ayokoding-www-learning-path-04-course-authoring` (Wave 2), and — transitively —
  `ayokoding-www-learning-path-05-manifests` (Wave 3).
- **Wave-1 sibling, soft coupling, NOT a blocking edge**:
  `ayokoding-www-learning-path-01-url-restructure`. It writes `prerequisites:` frontmatter into 37
...
- **This plan runs in parallel with `ayokoding-www-learning-path-01-url-restructure`** (the other Wave-1
...
- [ ] [AI] **Confirm the `syllabus/` corpus is intact and untouched** —
      `find plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus -type f | wc -l`
...
- [ ] [AI] Confirm `learnings.md` exists in the plan folder with its H1 —
      `test -f plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/learnings.md`
      — acceptance: returns 0 and the file's first content line is
      `# Learnings: ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
- [ ] [AI] Create the evidence folder: `mkdir -p plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/evidence`
...
> `ayokoding-www-learning-path-01-url-restructure`; the design-funnel steps (`:244-309`) belong to
> `ayokoding-www-learning-path-03-navigation-ui`. Phase 1 was a **three-way** split, not two.
...
- [ ] [AI] Verify the canonical contract is stated in this plan's
      ...
      — command:
      `grep -qF "The prerequisite frontmatter contract (canonical here)" plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/tech-docs.md`
...
- [ ] [AI] **Verify the Wave-1 sibling's copy agrees** — read
      `plans/backlog/ayokoding-www-learning-path-01-url-restructure/tech-docs.md` and compare its
...
--format=%H -- plans/backlog/ayokoding-www-learning-path-01-url-restructure`)
...
- [ ] [AI] Author `apps/ayokoding-www/src/features/course-paths/manifests/README.md` _(new file)_
      ...
      owned by `ayokoding-www-learning-path-05-manifests` and by no other plan; (c) that this plan
      creates the directory and nothing else in it
      — command:
      `test -f apps/ayokoding-www/src/features/course-paths/manifests/README.md && grep -qF "ayokoding-www-learning-path-05-manifests" apps/ayokoding-www/src/features/course-paths/manifests/README.md`
...
- [ ] [AI] The six-clause contract-agreement comparison against
      `ayokoding-www-learning-path-01-url-restructure` is recorded in
...
- [ ] [AI] Record the resulting coverage delta and name its closing plan explicitly in
      ...
      `ayokoding-www-learning-path-03-navigation-ui`; this delta closes there, not here"_
...
- [ ] [AI] **RED** — write failing unit tests in
  ...
  **These four scenarios are owned by `ayokoding-www-learning-path-03-navigation-ui`'s `prd.md`**, not
...
- [ ] [AI] **RED** — write failing unit tests in
  ...
  **Gherkin (underpins) →** "A course deep-linked without path context renders the canonical view";
  ...
  `ayokoding-www-learning-path-03-navigation-ui`'s `prd.md`**; reproduced here for the RED signal.
...
- [ ] [AI] **RED** — extend
  ...
  **Gherkin (underpins) →** "A path landing page lists its courses in manifest order"; "The
  ...
  course URL". **The first two are owned by `ayokoding-www-learning-path-03-navigation-ui`; the third by
  `ayokoding-www-learning-path-01-url-restructure`.** Reproduced here for the RED signal.
...
- [ ] [AI] `course-paths` Gherkin authored under `<SPECS>`; the
      ...
      `ayokoding-www-learning-path-03-navigation-ui` as its closing plan.
...
> (`ayokoding-www-learning-path-03-navigation-ui` and
> `ayokoding-www-learning-path-04-course-authoring`) have their start precondition from this plan
> satisfied.
...
> to `ayokoding-www-learning-path-05-manifests` and `ayokoding-www-learning-path-01-url-restructure`.
...
- [ ] [AI] Capture one screenshot per locale per breakpoint via `browser_take_screenshot`, saved to
      ...
      `en`/`id` × 375/768/1280)
      — acceptance: `find plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/evidence -name 'phase-4-no-regression-*.png' | wc -l`
...
- [ ] [AI] **Record the Rule-15 exemption explicitly, with its reason**, in this checklist and in
      ...
      `ayokoding-www-learning-path-03-navigation-ui`."_
...
- [ ] [AI] Confirm no plan PR is still open —
      `gh pr list --search "ayokoding-www-learning-path-02-schema-and-prerequisite-dag" --state open`
...
- [ ] [AI] Confirm the downstream handoff signal holds on integrated `main` —
      ...
      `ayokoding-www-learning-path-03-navigation-ui` and
      `ayokoding-www-learning-path-04-course-authoring` check before they start.
...
> `ayokoding-www-learning-path-05-manifests` and `ayokoding-www-learning-path-04-course-authoring` finish.
...
- [ ] [AI] Verify the manual no-regression evidence is committed —
      `find plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/evidence -name 'phase-4-no-regression-*.png' | wc -l`
...
- [ ] [AI] Verify the Rule-15 exemption and Rule-16 non-applicability are on the record —
      `test -f plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/evidence/phase-4-rule-15-exemption.txt`
...
- [ ] [AI] Verify the `syllabus/` corpus is byte-intact —
      `find plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus -type f | wc -l`
      returns **128** AND
      `git diff --stat origin/main -- plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus`
      prints nothing
...
- [ ] [AI] Record the pre-move inbound-link inventory to
      ...
      `grep -rn "ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus" plans/backlog plans/in-progress`
...
- [ ] [AI] Move the plan folder using today's completion date:
      `git mv plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag plans/done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag`
      (substitute today's date; the `evidence/` and `syllabus/` subfolders move with it)
      — acceptance: `test -d plans/done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus`
      returns 0 and `test -d plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag`
      returns non-zero.
- [ ] [AI] **In the same commit**, repoint every inbound cross-plan link in the other four plan
      folders from the sibling form
      `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/…`
      to the archived form
      `../../done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/…`
      ...
      — acceptance: `grep -rn "\.\./ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus" plans/backlog plans/in-progress`
      prints **nothing** and exits 1, AND
      `grep -rn "done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus" plans/backlog plans/in-progress`
...
- [ ] [AI] Update `plans/backlog/README.md` — remove this plan's entry
      — acceptance: `grep -qF "ayokoding-www-learning-path-02-schema-and-prerequisite-dag" plans/backlog/README.md`
      exits **1**.
- [ ] [AI] Update `plans/done/README.md` — add this plan's entry with today's completion date
      — acceptance: `grep -qF "ayokoding-www-learning-path-02-schema-and-prerequisite-dag" plans/done/README.md`
      exits **0**.
...
- [ ] [AI] Commit the archival **and the repoint together**:
      `chore(plans): move ayokoding-www-learning-path-02-schema-and-prerequisite-dag to done`
...
- [ ] [AI] The plan folder is under `plans/done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`
...
- [ ] [AI] After the archival PR merges, prompt the user before deleting
      `worktrees/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`.
...
This plan is created in `plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/`.
When work starts it is promoted to
`plans/in-progress/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/` — a pure move, no date
prefix on either stage. The `git mv` in Phase 7 then archives from wherever it sits to
`plans/done/YYYY-MM-DD__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/` using the
