...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`; the content homes, the `legacy/`
relocation, and both redirect modules are **owned by** `ayokoding-www-learning-path-01-url-restructure`;
the real manifests are **owned by** `ayokoding-www-learning-path-05-manifests`.
...
```text
apps/ayokoding-www/src/features/course-paths/
├── core/                      # PURE — no IO — OWNED BY ayokoding-www-learning-path-02-schema-and-prerequisite-dag
...
`ayokoding-www-learning-path-05-manifests`; this plan's `manifest-repository.ts` globs whatever directory
...
- **Content homes are not created here.** `content/en/learn/paths/_index.md` and
  ...
  `ayokoding-www-learning-path-01-url-restructure`; this plan builds the renderers that those routes mount
...
`ayokoding-www-learning-path-01-url-restructure`. **This plan's share** is narrower and purely a rendering
...
> **Q-E**, owned by `ayokoding-www-learning-path-01-url-restructure`. Its ruling changes what that plan's
...
The four real manifests ship in the Wave-3 plan `ayokoding-www-learning-path-05-manifests`, which depends
...
> (Screens 0-3), and **6** by `ayokoding-www-learning-path-01-url-restructure` (Screen 4). Every
...
blocks `ayokoding-www-learning-path-05-manifests`.

### Consumed from sibling plans (not restated)

| Decision    | Subject                                            | Owning plan                                                  |
| ----------- | -------------------------------------------------- | ------------------------------------------------------------ |
| DD-1        | Order lives outside the body                       | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` |
| DD-3        | Path-aware nav via `?path=` client context         | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` |
| DD-24       | AI path links, does not include, SWE prerequisites | `ayokoding-www-learning-path-05-manifests`                   |
| DD-40–DD-45 | Three-bucket IA, `legacy/` relocation, redirects   | `ayokoding-www-learning-path-01-url-restructure`             |
...
- **Hard plan prerequisites**: `ayokoding-www-learning-path-01-url-restructure` **and**
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`, both merged to `main`. The first supplies
...
- **Downstream consumer**: `ayokoding-www-learning-path-05-manifests` cannot publish a manifest that
  nothing renders.
