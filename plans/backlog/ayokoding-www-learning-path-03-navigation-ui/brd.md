...
- **It has exactly two upstream artefacts and one downstream consumer.** It needs the pure
  `course-paths/core/` modules (from `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`) and
  the `courses/` + `paths/` content homes plus the re-home redirect table (from
  `ayokoding-www-learning-path-01-url-restructure`); it hands its components to
  `ayokoding-www-learning-path-05-manifests`. Nothing else touches it.
...
- **Publishing any real path manifest.** Every rendering behaviour here is proven against a fixture
  manifest. All four real manifests belong to `ayokoding-www-learning-path-05-manifests`.
- **Authoring or editing any course body.** That is `ayokoding-www-learning-path-04-course-authoring`'s
  scope.
...
- **Writing the pure `course-paths/core/` modules or the `PathManifest` schema.** Those belong to
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`; this plan imports them.
...
- **Screen 4 (the legacy-bucket landing and page banner).** Its funnel section, its six renders, and
  its pending Q-D-dependent selection belong to `ayokoding-www-learning-path-01-url-restructure`.
...
| The re-home redirect assertion in this plan's e2e can never go green.                                     | The per-course redirect table is owned and shipped by `ayokoding-www-learning-path-01-url-restructure` **before** this plan starts (its Wave-1 merge is a hard start precondition); this plan asserts the redirect as a regression guard, not as its own Gherkin.   |
