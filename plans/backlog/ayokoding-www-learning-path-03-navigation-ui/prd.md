...
| 4      | Legacy-bucket landing and page banner         | **Not this plan** — owned by `ayokoding-www-learning-path-01-url-restructure`                |
...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`; this plan imports it and never edits it.
Every rendering behaviour is proven against a **fixture manifest**, because the four real manifests are
published by the Wave-3 plan `ayokoding-www-learning-path-05-manifests`.
...
`ayokoding-www-learning-path-01-url-restructure`; see
...
`ayokoding-www-learning-path-01-url-restructure`. That plan carries Screen 4's funnel prose, its six
...
| 4 Legacy landing | `legacy-landing-option-a-*` | `legacy-landing-option-b-*` | mobile / tablet / desktop | `ayokoding-www-learning-path-01-url-restructure` — all 6 pending there |
...
> **6** in `ayokoding-www-learning-path-01-url-restructure` (Screen 4). A reader auditing DD-47 against
...
- **Grid capacity**: the layout has room for **all four** paths from day one and is populated as each
  manifest ships in `ayokoding-www-learning-path-05-manifests`. That plan populates cards; it does not
  re-invent the grid.
...
`ayokoding-www-learning-path-05-manifests`. Each `Given` is therefore rewritten to name the **fixture
...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`, the "every re-homed course declares its
prerequisites" scenario goes to `ayokoding-www-learning-path-01-url-restructure`, and the **rendering**
...
`ayokoding-www-learning-path-05-manifests`, which is the first place all three real manifests exist.
Likewise, the legacy-redirect scenario belongs wholly to
`ayokoding-www-learning-path-01-url-restructure`; this plan asserts the redirect only as an e2e regression
guard.
...
- Publishing any real path manifest, or growing one (`ayokoding-www-learning-path-05-manifests`).
- Authoring or editing any course body (`ayokoding-www-learning-path-04-course-authoring`).
- Creating the `courses/` or `paths/` content homes, relocating the `legacy/` bucket, or writing either
  redirect module (`ayokoding-www-learning-path-01-url-restructure`).
- The pure `course-paths/core/` modules, the `PathManifest` zod schema, the `<MANIFESTS>` directory, and
  the whole `syllabus/` detail layer (`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`).
