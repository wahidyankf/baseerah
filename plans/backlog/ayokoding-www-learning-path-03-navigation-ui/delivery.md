...
**Hard prerequisite**: `ayokoding-www-learning-path-01-url-restructure` **and**
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag` must both be merged to `main` before Phase 1
...
Worktree path: `worktrees/ayokoding-www-learning-path-03-navigation-ui/`

Optional manual pre-provisioning (run from repo root):

```bash
claude --worktree ayokoding-www-learning-path-03-navigation-ui
```
...
ayokoding-www-learning-path-03-navigation-ui/<phase-slug>`), authors its work there, commits, pushes that
...
1. [AI] Sync the worktree to latest `origin/main` and branch:
   `git fetch origin && git checkout main && git pull && git checkout -b
ayokoding-www-learning-path-03-navigation-ui/<phase-slug>`.
...
- `<PLAN>` = `plans/in-progress/ayokoding-www-learning-path-03-navigation-ui/` (this plan's folder — while
...
2. **Cross-plan link filter (catches a stale link into a sibling plan's archived location)**:
   ...
   cargo run --release --manifest-path apps/rhino-cli/Cargo.toml -- md links validate \
     --exclude plans/done \
     --exclude apps/ayokoding-www/content \
     --exclude apps/ose-www/content 2>&1 | grep -F "ayokoding-www-learning-path-03-navigation-ui"
   ```
...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`, and its Phases 5-12 belong to the
...
- [ ] [AI] **Upstream precondition 1** — confirm `ayokoding-www-learning-path-01-url-restructure` has
...
- [ ] [AI] **Upstream precondition 2** — confirm
      `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` has merged:
...
- [ ] [AI] Confirm the two upstream plans are archived rather than merely branch-merged:
      `test -d plans/done && ls plans/done | grep -o -- "ayokoding-www-learning-path-01-url-restructure" | wc -l`
      returns **1**, and the same form for
      `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` returns **1** — acceptance: both return
...
> `ayokoding-www-learning-path-01-url-restructure`. DD-47's total of 30 is a two-plan total; see the
...
- [ ] [AI] **All 24 of this plan's hi-fi renders exist** —
      ...
      Screen 4's remaining 6 renders belong to `ayokoding-www-learning-path-01-url-restructure`; **24 is
...
- [ ] [AI] **No-forked-body acceptance clause (not Gherkin here)** — assert over the **two** fixture
      ...
      shared course with no body duplication") belongs to `ayokoding-www-learning-path-05-manifests`, the
...
- [ ] [AI] Commit and push to `origin ayokoding-www-learning-path-03-navigation-ui/<phase-slug>` (the PR
...
- [ ] [AI] **Legacy-redirect regression guard (not owned Gherkin)** — assert that one already-shipped
      ...
      Gherkin belongs wholly to `ayokoding-www-learning-path-01-url-restructure`; this is a regression
...
- [ ] [AI] Commit and push to `origin ayokoding-www-learning-path-03-navigation-ui/<phase-slug>`.
...
> scope is complete**; `ayokoding-www-learning-path-05-manifests` is unblocked. Safe to stop. To resume:
...
- [ ] [AI] Confirm no plan PR is still open — `gh pr list --search "ayokoding-www-learning-path-03-navigation-ui" --state open`
...
- [ ] [AI] **Cross-plan link gate (BF-8)** — run **both** commands 1 and 2 in
      ...
      This is the check that catches a stale `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/…`
...
- [ ] [AI] **Verify this plan's design-funnel artefacts are complete (DD-46 / DD-47)** —
      ...
      **24 is the complete deliverable for this plan.** DD-47's total of 30 is a two-plan total: the
      remaining 6 (Screen 4) belong to `ayokoding-www-learning-path-01-url-restructure`. Do NOT copy that
...
- [ ] [AI] Move: `git mv plans/in-progress/ayokoding-www-learning-path-03-navigation-ui/
plans/done/YYYY-MM-DD__ayokoding-www-learning-path-03-navigation-ui/` using today's completion date (the
...
- [ ] [AI] Commit the archival:
      `chore(plans): move ayokoding-www-learning-path-03-navigation-ui to done`.
...
- [ ] [AI] Plan folder is under `plans/done/YYYY-MM-DD__ayokoding-www-learning-path-03-navigation-ui/`; all
...
This plan is authored in `plans/backlog/ayokoding-www-learning-path-03-navigation-ui/`. When work starts it
is promoted to `plans/in-progress/ayokoding-www-learning-path-03-navigation-ui/` (pure move, no date
prefix); the `git mv` in Phase 8 then archives it to
`plans/done/YYYY-MM-DD__ayokoding-www-learning-path-03-navigation-ui/` using the completion date. Substitute
