# Coding Interview (By Example, Python (agnostic))

**Course ID**: `coding-interview` · **Format**: By Example · **Language**: Python (agnostic). **NEW**
— interview course.
...
structures (that is `data-structures-and-algorithms-essentials`) or algorithms (that is
`advanced-algorithms`) — it references those forward and drills the _interview skin_ over them.
Examples are Python; every pattern is language-agnostic.
...
- **Prior topics**: `data-structures-and-algorithms-essentials` and
  `advanced-algorithms` (arrays, hash maps, trees, graphs, sorting, recursion, DP,
  Big-O); `just-enough-python` for the example language.
...
> Pre-authoring `web-researcher` sweep pending (per this plan's Anti-Hallucination verification recipe). Items below are stable-concept or
...
- The structured "clarify → example → plan+complexity → code → verify" loop displaced the older "just
  ...
  pressure. This module hands the interview skin forward to `take-home-and-live-coding` (async and pairing formats) and up to the [Phase 1 mock
  loop capstone](./capstone-interview-loop.md).
