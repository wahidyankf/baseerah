# Software Product Engineering (Annotated-concept, — (concept, no code)) — carried-over course

**Course ID**: `software-product-engineering` · **Format**: Annotated-concept · **Language**: — (concept, no code).

**Scope note**: **carried-over course** — this migration changes only its packaging into the shared
two-path library; the subject content is kept from the sibling plan and is not re-authored here. This
is a **pointer file**, not a full worked-example page. Order is NOT a property of this course — see
"In which paths" below.

**Short summary**: Turning engineering into shipped products

## Authored content — source of truth

The full worked content for `software-product-engineering` (Concepts `co-NN`, Worked examples `ex-NN`, and the intra-course
Capstone spec) lives in the sibling plan and is the **single source of truth**:

- [`32-software-product-engineering.md`](../../../in-progress/fundamentally-strong-software-engineer/syllabus/32-software-product-engineering.md)

See [overview.md](./overview.md) for the shared-library architecture and the per-course file template,
and [README.md](./README.md) for the full course catalog (this course's format, language, and short
summary are listed there verbatim).

## In which paths

- `job-seeking/software-engineer` — Phase 3 · Deepening (shallow → deep) — Quality / product / delivery.
- `fundamentally-strong/software-engineer` — Stage 4 · Systems, data, architecture, security & ops depth (shallow → deep) — Quality / product / delivery.

---

← Back to [README.md — course library catalog](./README.md)
