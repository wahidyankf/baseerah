# Browser Automation with CDP (By Example, Python — CDP client)

**Course ID**: `browser-automation-with-cdp` · **Format**: By Example · **Language**: Python (CDP
client). **NEW** — a harness tool and the `remotebrowser` substrate.
...
- **Prior topics**: `async-python-and-fastapi-services`
  (async I/O, WebSocket-shaped concurrency), `networking-essentials` (HTTP,
  WebSockets), and `just-enough-python`.
...
- Browser automation evolved from brittle screen-scraping and Selenium/WebDriver toward CDP-based tools
  ...
  agents over MCP. This module feeds `agent-tools-and-mcp` (exposing the
  browser as a tool) and the [coding-agent capstone](./capstone-build-your-own-coding-agent.md)'s
