# Detection Engineering & SIEM Operations (By Example, XML/rules + config + Python)

**Course ID**: `detection-engineering-and-siem-operations` · **Format**: By Example · **Language**:
XML/rules + config + Python. **NEW** — detection-engineering **principles**; Wazuh's XML ruleset is the
worked example, not the subject. **Prereq**: `defensive-security`. Distinct from the concept-level
`defensive-security` course (two altitudes, never merged).
...
**false-positive tuning**, **dashboards**, and **alert triage**. Where `defensive-security` teaches
detection _as a concept_ (blue-team, monitoring, IR at the idea level), this course has the reader
_operate a SIEM and write rules_. A Wazuh XML ruleset + decoders are the concrete worked example; the
transferable skill is detection engineering, applicable to any SIEM.
...
- **Prior topics**: `defensive-security` (blue-team concepts, monitoring, IR),
  `security-essentials`, `offensive-security` (to know what attacks
  look like), and `just-enough-python` (for log-processing + rule-testing scripts).
- **Tools & environment**: a macOS/Linux terminal; a SIEM to operate (Wazuh as the worked example,
  self-hostable per `self-hosting-essentials`) — pinned CVE-clean at authoring; sample log
...
- **Assumed knowledge**: reading logs; regular expressions; XML basics; the attacker techniques from
  `offensive-security`; blue-team concepts from `defensive-security`.
...
21. **co-21 · incident-escalation** — a triaged true positive escalates into the incident-response
    process (from `defensive-security`).
...
23. **co-23 · metrics-and-coverage** — measuring detection coverage, FP rate, and time-to-detect drives
    improvement.
24. **co-24 · adversary-emulation-for-detection** — replaying attacker techniques (from `offensive-security`) validates
...
- Security monitoring evolved from grepping logs by hand, to static signature IDS, to SIEMs correlating
  ...
  concept-level `defensive-security` and feeds the
  [pentest-engine capstone](./capstone-build-your-own-pentest-engine.md) (which must evade + be
...
16. **ex-34 · escalate-to-incident** — escalate a triaged TP into the IR process (from `defensive-security`) — verify the
    handoff. (co-21)
...
1. **ex-37 · adversary-emulation-replay** — replay an attack sequence (from `offensive-security`) + confirm the
...
13. **ex-49 · incident-from-detection-to-response** — a full flow: detection fires → triage → enrich →
    escalate → IR (`defensive-security`) — verify each stage. (co-19, co-20, co-21)
...
- **Ordered steps**:
  ...
  3. Map each detection to an ATT&CK technique + build a coverage/alert dashboard. Verify the mapping +
     the dashboard.
  4. Emulate an attack (from `offensive-security`) + replay it through the pack; verify the kill-chain alerts fire and
