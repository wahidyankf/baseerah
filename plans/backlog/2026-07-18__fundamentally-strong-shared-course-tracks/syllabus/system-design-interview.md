# System-Design Interview (Annotated-concept, — (concept, no code))

**Course ID**: `system-design-interview` · **Format**: Annotated-concept · **Language**: — (concept,
no code). **NEW** — interview course.
...
drills at mid/senior/staff altitude. This is deliberately **NOT** the depth course `system-design`
(which teaches designing real systems for scale) — "interview format" and "genuine design depth" are
two separate courses by design, kept as distinct catalog entries. This course teaches the _performance
and communication_ of the round; it references the depth course forward. No runnable code — prose,
worked design scenarios, and WCAG-accessible Mermaid diagrams.
...
- **Prior topics**: `object-oriented-programming-essentials`, `object-oriented-design-and-patterns`, and
  `sql-essentials` for the building blocks; a working mental model of HTTP, caches,
...
- The structured round replaced open-ended "design Twitter" chats because unstructured design
  ...
  communication; the genuine engineering depth lives in `software-architecture`, `system-design`, and `distributed-systems`, which this round draws on but does not replace. It feeds the [Phase 1 mock loop
  capstone](./capstone-interview-loop.md).
