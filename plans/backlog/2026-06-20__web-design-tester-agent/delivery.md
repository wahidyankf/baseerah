...
- [ ] [AI] Confirm the ose-public primary checkout is on `main` and synced with `origin/main`
...
- [ ] [AI] Install dependencies: `npm install` — acceptance: exits 0, `node_modules/` synchronized
- [ ] [AI] Converge the toolchain: `npm run doctor -- --fix` — acceptance: exits
      0 with no unresolved drift
- [ ] [AI] Establish the binding/markdown baseline: `npm run validate:sync` and
...
- [ ] [AI] `npm run validate:sync`, `npm run harness:bindings-validation`, and `npm run lint:md`
...
- [ ] [AI] Re-read the two sibling tester agent files as structural templates:
...
- [ ] [AI] Create `.claude/agents/web-design-tester.md` with frontmatter: `name: web-design-tester`,
...
- [ ] [AI] Write the **Why This Agent Exists** + **mental model** sections: design-team advocate
...
- [ ] [AI] Write the **`swe-ui-checker` boundary** section (HARD): web-design-tester = live
...
- [ ] [AI] Write the **five ground-truth sources** section: (1) committed plan-folder mockup assets
...
- [ ] [AI] Write the **locale + evidence** section: test ALL supported locales (discovered from app
...
- [ ] [AI] Write the **Output** section modelled on the siblings: backlog plan at
...
- [ ] [AI] Write **Non-Destructive Constraint**, **Relationship to Other Agents** (feeds `plan-maker`,
...
- [ ] [AI] Run markdown gate on the new file: `npm run lint:md` — acceptance: exits 0

### Phase 1 Gate

> All checks below must pass before starting Phase 2.

- [ ] [AI] `test -f .claude/agents/web-design-tester.md` — expected: file exists
- [ ] [AI] `grep -Eq "^name: web-design-tester$" .claude/agents/web-design-tester.md && grep -Eq "^model: sonnet$" .claude/agents/web-design-tester.md && grep -Eq "^color: green$" .claude/agents/web-design-tester.md`
      — expected: exit 0 (metadata correct)
- [ ] [AI] `grep -q "swe-ui-checker" .claude/agents/web-design-tester.md` — expected: boundary pinned
- [ ] [AI] `grep -q "libs/web-ui" .claude/agents/web-design-tester.md && grep -qi "not cramped" .claude/agents/web-design-tester.md && grep -q "DWT-" .claude/agents/web-design-tester.md`
...
- [ ] [AI] Edit `.claude/agents/web-exploratory-tester.md`: in its **Relationship to Other Agents**
      ...
      ≥ 1
- [ ] [AI] Edit `.claude/agents/web-usability-tester.md`: in its relationship/"distinct from" section,
...
- [ ] [AI] Confirm `web-design-tester.md` reciprocally names BOTH siblings in its own relationship
...
- [ ] [AI] Run markdown gate: `npm run lint:md` — acceptance: exits 0

### Phase 1b Gate

> All checks below must pass before starting Phase 2.

- [ ] [AI] Each of the three tester agent files names the other two (mutual cross-reference):
...
- [ ] [AI] Edit `repo-governance/conventions/structure/agent-naming.md`: add `web-design-tester` to
      ...
      ≥ 2
- [ ] [AI] Edit `.claude/agents/README.md`: add a `web-design-tester` bullet to the `### 🧪 Testing`
...
- [ ] [AI] Edit `AGENTS.md`: add a **Testing** line to the agent catalog block (the
...
- [ ] [AI] Rename the workflow file to seat the design lens:
...
- [ ] [AI] Edit the renamed workflow file: update frontmatter `name`/`title`/`goal`/`termination` to
      ...
      ≥ 2
- [ ] [AI] Edit `repo-governance/workflows/README.md`: update the workflow table row label + cell and
      ...
      reads three testers
- [ ] [AI] Edit `repo-governance/workflows/web/README.md`: update the Purpose, Scope, and Workflows
...
- [ ] [AI] Fix any internal links that referenced the old workflow filename:
...
- [ ] [AI] Run markdown + link gates: `npm run lint:md` and
...
- [ ] [AI] `grep -rl "web-design-tester" repo-governance/conventions/structure/agent-naming.md .claude/agents/README.md AGENTS.md repo-governance/workflows/README.md repo-governance/workflows/web/README.md`
      — expected: all five files listed (every surface registers the agent)
- [ ] [AI] `test -f repo-governance/workflows/web/web-ux-test-fixing-planning.md && ! test -f repo-governance/workflows/web/web-exploratory-and-usability-test-fixing-planning.md`
      — expected: exit 0 (rename complete, old file gone)
- [ ] [AI] `grep -rn "web-exploratory-and-usability-test-fixing-planning" repo-governance/ .claude/ AGENTS.md`
      — expected: no output (no stale references)
- [ ] [AI] `npm run lint:md` and `npx nx run rhino-cli:links:validation` — expected: both exit 0
...
- [ ] [AI] Edit `repo-governance/development/quality/user-facing-delivery-hardening.md` Rule 15: change
...
- [ ] [AI] Update the same file's **Examples** (PASS line), **Tools and Automation** bullets (add
...
- [ ] [AI] Edit `AGENTS.md`: update the hardening bullet that currently names a "near-end
...
- [ ] [AI] Edit `repo-governance/workflows/plan/plan-execution.md`: at the near-end / finalization gate
...
- [ ] [AI] Edit `.claude/agents/plan-maker.md`: where it emits the Rule-15 near-end retest step for
...
- [ ] [AI] Edit `.claude/agents/plan-checker.md`: where it flags a missing/single-locale Rule-15
...
- [ ] [AI] Edit `.claude/agents/plan-execution-checker.md`: where it verifies the Rule-15 round ran
...
- [ ] [AI] Scope clarification: the rule binds web-UI **feature-change** plans (browser-rendered apps),
...
- [ ] [AI] Run markdown + link gates: `npm run lint:md` and `npx nx run rhino-cli:links:validation`
...
- [ ] [AI] All three tester names appear in Rule 15 + Tools and Automation of
      ...
      — expected: exit 0
- [ ] [AI] `grep -rl "web-design-tester\|web-ux-test-fixing-planning" AGENTS.md repo-governance/workflows/plan/plan-execution.md .claude/agents/plan-maker.md .claude/agents/plan-checker.md .claude/agents/plan-execution-checker.md`
...
- [ ] [AI] Regenerate all secondary bindings: `npm run generate:bindings`
      — acceptance: exits 0 and `.opencode/agents/web-design-tester.md` now exists
      (`test -f .opencode/agents/web-design-tester.md`)
- [ ] [AI] Validate sync parity: `npm run validate:sync` — acceptance: exits 0 (no mirror drift)
- [ ] [AI] Validate harness bindings: `npm run harness:bindings-validation` — acceptance: exits 0
- [ ] [AI] Re-run markdown gate after generation: `npm run lint:md` — acceptance: exits 0

### Phase 3 Gate

> All checks below must pass before starting Phase 4.

- [ ] [AI] `test -f .opencode/agents/web-design-tester.md` — expected: OpenCode mirror generated
- [ ] [AI] `npm run validate:sync` — expected: exits 0
- [ ] [AI] `npm run harness:bindings-validation` — expected: exits 0
- [ ] [AI] `npm run lint:md` — expected: exits 0
...
- [ ] [AI] Run affected checks: `npx nx affected -t typecheck lint test:quick`
      — acceptance: exits 0 (rhino-cli may be affected via binding generation)
- [ ] [AI] Run binding + markdown + link gates:
...
- [ ] [AI] Fix ALL failures found — including preexisting issues not caused by this change (root-cause
...
- [ ] [AI] Stage explicit paths only (never `git add -A` — sibling repos carry WIP):
...
- [ ] [AI] Commit and push to origin main: `git push origin HEAD:main` (Trunk Based Development;
...
- [ ] [AI] `git log --oneline -1` shows the web-design-tester commit on `main`
- [ ] [AI] All triggered GitHub Actions are green (`gh run list --branch main --limit 5` shows
...
- [ ] [AI] Confirm `prd.md` Gherkin scenarios for ose-public are satisfied: agent file + metadata,
...
- [ ] [AI] Re-run the full ose-public verification battery:
...
- [ ] [AI] `npm run validate:sync && npm run harness:bindings-validation` — expected: both exit 0
...
- [ ] [AI] In ose-primer on `main`, sync with `origin/main` and confirm clean baseline
...
- [ ] [AI] Author `.claude/agents/web-design-tester.md` topic-identically to ose-public's, **localized**:
...
- [ ] [AI] Apply the same registration surfaces (agent-naming convention, agents README, AGENTS.md,
...
- [ ] [AI] Regenerate bindings + validate using ose-primer's actual script names (confirm via its
      `package.json`) — acceptance: the OpenCode mirror exists and sync/harness validators exit 0
- [ ] [AI] Run ose-primer local quality gates; fix all failures (incl. preexisting) — acceptance: zero
      failures
- [ ] [AI] Stage explicit paths, commit (`feat(agents): add web-design-tester completing the live-site
advocate triad`), and push to ose-primer `origin main` — acceptance: push succeeds
- [ ] [AI] Monitor ose-primer CI to green (poll ~3 min; no `gh run watch`) — acceptance: all checks pass

### Phase 6 Gate

> All checks below must pass before starting Phase 7.

- [ ] [AI] In ose-primer: the design-tester commit is on `main` and all GitHub Actions are green
- [ ] [AI] `grep -q "libs/ts-ui" .claude/agents/web-design-tester.md` and the OpenCode mirror exists
...
- [ ] [AI] In the ose-infra `~/ose-projects/ose-infra` checkout on `main`, sync with `origin/main` and
...
- [ ] [AI] Author `.claude/agents/web-design-tester.md` topic-identically, **localized** (`libs/ts-ui`;
      ose-infra app/lib names) — acceptance: `grep -c "libs/ts-ui"` ≥ 1 and `libs/web-ui` absent
- [ ] [AI] Apply the same registration surfaces present in ose-infra (verify each exists first); if a
...
- [ ] [AI] Regenerate bindings + validate using ose-infra's actual script names — acceptance: OpenCode
      mirror exists and validators exit 0
- [ ] [AI] Run ose-infra local quality gates; fix all failures (incl. preexisting) — acceptance: zero
      failures
- [ ] [AI] Stage explicit paths, commit (`feat(agents): add web-design-tester completing the live-site
advocate triad`), and push to ose-infra `origin main` from the `~/ose-projects/ose-infra` checkout
      — acceptance: push succeeds
- [ ] [AI] Monitor ose-infra CI to green (poll ~3 min; no `gh run watch`) — acceptance: all checks pass

### Phase 7 Gate

> All checks below must pass before archival.

- [ ] [AI] In ose-infra: the design-tester commit is on `main` and all GitHub Actions are green
- [ ] [AI] `grep -q "libs/ts-ui" .claude/agents/web-design-tester.md` in the ose-infra checkout and its
...
- [ ] [AI] Run `repo-rules-maker` in **ose-public**: scope it to the web-design-tester registration +
...
- [ ] [AI] Stage explicit paths, commit (`chore(governance): repo-rules-maker consistency sweep for
...
- [ ] [AI] Run `repo-rules-maker` in **ose-primer** (localized surfaces); apply fixes; commit + push
      `origin main`; CI green — acceptance: no unresolved findings; pushed; CI green (or no-op if clean)
- [ ] [AI] Run `repo-rules-maker` in **ose-infra** (in `~/ose-projects/ose-infra` on `main`); apply
...
- [ ] [AI] `repo-rules-maker` reports zero unresolved consistency findings (for the changed surfaces)
      in all three repos
- [ ] [AI] Each repo's `main` is pushed and CI-green after the sweep (or the sweep was a verified no-op
...
- [ ] [AI] Confirm three-repo parity: each of ose-public, ose-primer, ose-infra has
...
- [ ] [AI] Confirm every `prd.md` Gherkin scenario is satisfied across the three repos — acceptance:
...
- [ ] [AI] Note: no manual UI/API behavioral assertions apply — this plan ships no UI/API code (it
...
- [ ] [AI] Move and date-prefix the plan (use today's completion date, NOT the creation date):
...
- [ ] [AI] Update `plans/in-progress/README.md` — remove the plan entry — acceptance: entry gone
- [ ] [AI] Update `plans/done/README.md` — add the plan entry with completion date — acceptance: entry present
...
- [ ] [AI] Commit the archival: `chore(plans): move web-design-tester-agent to done`
- [ ] [AI] Commit and push to origin main: `git push origin HEAD:main` — acceptance: push succeeds

### Phase 8 Gate

> All checks below must complete successfully before closing the plan.

- [ ] [AI] `test -d plans/done/*web-design-tester-agent` — expected: archived
- [ ] [AI] `git log --oneline -1` shows the archival commit on `main` and CI is green
