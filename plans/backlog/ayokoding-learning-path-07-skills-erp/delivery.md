...
- `<PLANDIR>` = `plans/backlog/ayokoding-learning-path-07-skills-erp/` **while this plan is in
  `plans/backlog/`**, and `plans/in-progress/ayokoding-learning-path-07-skills-erp/` **once the plan
  is promoted for execution**. Shell variable assignment captures a value at assignment time, not a
  live reference, so reassigning `PLANDIR` alone does **not** update `SYL`/`SYLPATHS` — the Shell
  constants block below detects the current promotion stage and derives `PLANDIR`, `SYL`, and
  `SYLPATHS` **together, in one command**, every time that block runs. Paste the whole block again
  (not just a `PLANDIR=` line) in any new shell session, including one opened after promotion. Every
  `<SYL>`/`<SYLPATHS>`-based acceptance clause in Phases 1-10 depends on that block having run in the
...
- `<SYLPATHS>` = `<PLANDIR>syllabus/paths/` — holds exactly one file,
  `manifest-skills-enterprise-resource-planning.md` (DD-22)
- `<SPECS>` = `specs/apps/ayokoding/behavior/ayokoding-www/gherkin/course-paths/`
- Path id: `skills/enterprise-resource-planning` · URL: `/en/learn/paths/skills/enterprise-resource-planning`

### Shell constants and course id lists (export once per shell; every acceptance clause below uses them)

**These are real, pasteable shell assignments — every acceptance clause below is verbatim-runnable,
never an angle-bracket placeholder.** `<` and `>` are shell redirection operators, so a literal
`test -f <ERPMAN>` is a syntax error if pasted; the variables below remove that hazard entirely.
`PLANDIR`, `SYL`, and `SYLPATHS` are **detected and derived together, in the same command**, every
time the block below runs — it prefers `plans/in-progress/` when that copy exists, falling back to
`plans/backlog/`, and re-derives `SYL`/`SYLPATHS` from whichever one it picked. Re-paste the whole
block (not just a `PLANDIR=` line) in any new shell session, including one opened after this plan
promotes from `plans/backlog/` to `plans/in-progress/` — every other constant is
promotion-independent and safe to keep across sessions.

The course-id lists are shell ARRAYS, not space-separated strings, and every loop below iterates
`"${NAME[@]}"`. The interactive shell in this repo is **zsh**, which does **not** word-split an
unquoted parameter expansion, so `for id in $ERP_ALL` (a space-separated **string**) iterates exactly
**once**, over the whole string, and every derived count would silently read `1` instead of `20`. The
quoted-array form `for id in "${ERP_ALL[@]}"` iterates correctly under both zsh and bash, which is why
every loop below uses it.

```bash
# Path constants (PLANDIR/SYL/SYLPATHS are detected + derived together every time this block runs)
COURSES=apps/ayokoding-www/content/en/learn/courses/
PATHS=apps/ayokoding-www/content/en/learn/paths/
ERPLANDING="${PATHS}skills/enterprise-resource-planning/_index.md"
FEAT=apps/ayokoding-www/src/features/course-paths/
MANIFESTS="${FEAT}manifests/"
ERPMAN="${MANIFESTS}skills/enterprise-resource-planning.yaml"
if test -d plans/in-progress/ayokoding-learning-path-07-skills-erp/; then
  PLANDIR=plans/in-progress/ayokoding-learning-path-07-skills-erp/
elif test -d plans/backlog/ayokoding-learning-path-07-skills-erp/; then
...
SWE_LIB=(backend-essentials api-design event-driven-architecture networking-essentials
  security-essentials data-engineering analytics-and-experimentation project-management)
```

Counts: `ERP_WAVE_A` = 10, `ERP_WAVE_B` = 5, `ERP_WAVE_C` = 3, `ERP_WAVE_D` = 2, `ERP_ALL` = 20,
`SWE_LIB` = 8. Verify the arrays loaded before trusting any clause below:
...
No id in `ERP_ALL` is a substring of another, so a plain `grep -F -q "$id"` against `"${ERPMAN}"` is
...
> anywhere in this file. (`--glob` is supported, but **only as `--glob=VALUE`** — the space-separated
> `--glob VALUE` dies with `ugrep: missing argument for --glob`, which `2>/dev/null` would turn into a
> silent zero. `--exclude-dir`, where it appears, avoids the question.) `find -newermt` is GNU syntax
...
- [ ] [AI] **Confirm `PLANDIR`** — this plan folder is expected to be promoted from `plans/backlog/`
      to `plans/in-progress/` before execution begins (Plans Organization Convention lifecycle
      stages). Run the same detect-and-derive command as the Shell constants block above — it sets
      `PLANDIR`, `SYL`, and `SYLPATHS` **together, in one command**, so `SYL`/`SYLPATHS` can never go
      stale relative to a freshly-detected `PLANDIR`:
      `if test -d plans/in-progress/ayokoding-learning-path-07-skills-erp/; then PLANDIR=plans/in-progress/ayokoding-learning-path-07-skills-erp/; elif test -d plans/backlog/ayokoding-learning-path-07-skills-erp/; then PLANDIR=plans/backlog/ayokoding-learning-path-07-skills-erp/; else unset PLANDIR; fi; SYL="${PLANDIR}syllabus/courses/"; SYLPATHS="${PLANDIR}syllabus/paths/"`
      — acceptance: `test -f "${PLANDIR}delivery.md"` exits 0 (falsifiable: fails closed if neither
      `plans/backlog/ayokoding-learning-path-07-skills-erp/` nor
      `plans/in-progress/ayokoding-learning-path-07-skills-erp/` exists — `PLANDIR` stays unset and
      `"${PLANDIR}delivery.md"` resolves to a bare, non-existent `delivery.md`), AND — whenever
      `plans/in-progress/ayokoding-learning-path-07-skills-erp/delivery.md` exists —
      `test "${PLANDIR}" = "plans/in-progress/ayokoding-learning-path-07-skills-erp/"` also exits 0
      (falsifiable: fails if the detection wrongly kept the stale `plans/backlog/` copy instead of the
      promoted one).
...
- [ ] [AI] Resolve the **Indonesian PSAK numbering** conflict ("PSAK 59 / SIFAS 101-109" versus "PSAK
      101-110") against IAI's published list, **and** re-verify the three-jurisdictional-model claim
      (AAOIFI/Bahrain, PSAK Syariah/Indonesia, MFRS + BNM Shariah Governance Policy 2019/Malaysia,
      including the "not on AAOIFI's mandatory-adoption list" claim for Malaysia — currently
      `[Unverified]`, see [tech-docs §Load-bearing for #19](./tech-docs.md#load-bearing-for-19--there-is-no-single-sharia-accounting-standard))
      against a primary AAOIFI, DSN-MUI, or BNM source — acceptance: either the resolved PSAK series
      and the re-verified three-model claim are each recorded with a cited URL and access date in
      `"${SYL}sharia-compliant-erp-design.md"`, or each unresolved item keeps its `[Unverified]` label
      and the spec forbids writing that item as unqualified fact. Falsifiable both ways: a spec that
      states a numbered PSAK standard or a jurisdictional adoption fact with no citation and no
      `[Unverified]` label fails this check.
  - _Suggested executor: `web-researcher`_
...
- [ ] [AI] **No category prefix on course ids (DD-21)** — the eight accounting prerequisites and the
      ...
      category — acceptance: a mechanical double loop over both prefixes and all sixteen cross-domain
      ids —
      `for prefix in skills careers; do for id in "${ACCT_GATE_B[@]}" "${ACCT_GATE_C[@]}" "${ACCT_GATE_D[@]}" "${SWE_LIB[@]}"; do grep -rF -l "$prefix/$id" "${SYL}" 2>/dev/null; done; done | wc -l`
      returns **0**. Falsifiable both ways: writing one prefixed course id (e.g.
      `skills/financial-statements-and-close-cycle`) into any spec makes the count **1**.
...
- [ ] [AI] `erp-integration-patterns` (By Example) — every integration-surface claim (IDoc status,
      ...
      `grep -F -q 'accuracy note' "${COURSES}erp-integration-patterns/overview.md"` exits 0 (exits 2
      before this step, since the file does not exist yet).
  - _Suggested executor: `apps-ayokoding-www-by-example-maker`_
- [ ] [AI] `erp-implementation-methodology` (Annotated-concept) — states its scope boundary against
      ...
      (exits 2 before this step, since the file does not exist yet, and exits 1 if the boundary line
      is later dropped from an existing file).
  - _Suggested executor: `apps-ayokoding-www-annotated-concept-maker`_
...
- [ ] [AI] **GREEN** — author `"${ERPMAN}"` _(new file)_ with `pathId: skills/enterprise-resource-planning`,
      `arc: immediately-effective`, a `title`, and a `description`, with `courseOrder` initially
      **empty** (`courseOrder: []` — populated in cycle 2.2b below) — command:
      `npx nx run ayokoding-www:test:unit`
      — acceptance: the pathId/arc assertion passes, AND
      `grep -F -q 'arc: immediately-effective' "${ERPMAN}"` exits 0 (exits 2 before this step, since
      the file does not exist yet).
- [ ] [AI] **REFACTOR** — align the YAML's key order and comment style with the schema plan's
      documented example — command: `npx nx run ayokoding-www:test:unit && npx nx run ayokoding-www:lint`
      — acceptance: both exit 0 and the arc/pathId assertions still pass.

### 2.2b · TDD cycle — publish the manifest with the ten Wave-A ids

- [ ] [AI] **RED** — extend the manifest unit test with an assertion that `courseOrder` contains
      exactly the ten Wave-A ids **in ramp order** and passes `checkManifestIntegrity` +
      `checkPrerequisiteConsistency` — command: `npx nx run ayokoding-www:test:unit`
      — acceptance: the new assertion **fails** because `courseOrder` is still empty (from cycle
      2.2a's GREEN step).
...
- [ ] [AI] **GREEN** — populate `"${ERPMAN}"`'s `courseOrder` with exactly the ten Wave-A ids **in ramp
      order** (`erp-foundations-and-history`, `erp-conceptual-data-model`, `erp-platform-landscape`,
...
- [ ] [AI] **REFACTOR** — assert the structural invariant that `courseOrder` is the file's **only**
...
- [ ] [AI] Author `"${ERPLANDING}"` _(new file — this is the **path landing**, not a structural index;
      ...
      **No `courseOrder` in the landing** — the ordered list renders from the loaded manifest —
      acceptance: `grep -F -q 'courseOrder' "${ERPLANDING}"` exits **1** (and exits 0 if one is
      mistakenly added), AND every boundary is stated with a **course-id-anchored** marker, never a
      bare ordinal that could misresolve against a partially-grown manifest: boundary 1 is already
      **reached** (Wave A ships this phase); boundaries 2 and 3 are **listed as coming**, not yet
      reached (their `Reached after <course-id>` form only appears once Phases 3 and 5 land them) —
      `for s in 'Reached after capstone-stand-up-and-integrate-an-open-source-erp' 'Coming after erp-integration-patterns' 'Coming after capstone-build-a-minimal-erp-core' 'silently corrupts state'; do grep -F -q "$s" "${ERPLANDING}" || echo "MISSING $s"; done | wc -l`
      returns **0**, AND the not-yet-reached boundaries' `Reached after` forms are absent —
      `for s in 'Reached after erp-integration-patterns' 'Reached after capstone-build-a-minimal-erp-core'; do grep -F -q "$s" "${ERPLANDING}" && echo "PRESENT $s"; done | wc -l`
      returns **0** (each flips to present only once Phases 3 and 5 land their boundary), AND
...
- [ ] [AI] **RED** — add `"${SPECS}skills-erp-path.feature"` _(new file)_ and a failing e2e step in
      `apps/ayokoding-www-fe-e2e/src/steps/skills-erp-path.steps.ts` _(new file, created by this plan,
      pairing 1:1 with its own `skills-erp-path.feature`)_ asserting that the ERP landing states boundary 1's
...
- [ ] [AI] `grep -F -q 'courseOrder' "${ERPLANDING}"` exits **1**; the landing's L-1/L-2/L-4 string
      checks each return **0** missing; boundary 1 shows `Reached after
capstone-stand-up-and-integrate-an-open-source-erp`; boundaries 2 and 3 show only their `Coming
after <course-id>` (not-yet-reached) form.
...
- [ ] [AI] `record-to-report-systems` (By Example) — the hard cross-domain edge; the body declares
      ...
      **and** `grep -F -q 'financial-statements-and-close-cycle' "${COURSES}record-to-report-systems/_index.md"`
      exits 0 (exits 2 before this step, since the file does not exist yet).
  - _Suggested executor: `apps-ayokoding-www-by-example-maker`_
...
- [ ] [AI] **GREEN** — update `"${ERPLANDING}"` so boundary 2 flips from **listed** to **reached**:
      replace its `Coming after erp-integration-patterns` marker with `Reached after
erp-integration-patterns`, and bind the assertion — command:
      `npx nx run ayokoding-www:specs:behavior:coverage && npx nx run ayokoding-www-fe-e2e:test:e2e`
      — acceptance: both exit 0, AND `grep -F -q 'Reached after erp-integration-patterns' "${ERPLANDING}"`
      exits 0 (exits 1 before this step), AND `grep -F -q 'Coming after erp-integration-patterns' "${ERPLANDING}"`
      exits **1** (exits 0 before this step) — the listed form is fully replaced, not merely
      supplemented.
...
- [ ] [AI] `grep -F -q 'Reached after erp-integration-patterns' "${ERPLANDING}"` exits 0; `grep -F -q
'Coming after erp-integration-patterns' "${ERPLANDING}"` exits **1** — boundary 2 is fully
      transitioned, not merely supplemented.
...
- [ ] [AI] `erp-security-and-controls` (Annotated-concept) — states its scope boundary against
      `it-governance-grc`: RBAC/SoD and COSO-SOX specifics only (DD-10); declares
      `audit-controls-and-compliance` — acceptance: all seven steps complete; zero
      CRITICAL/HIGH/MEDIUM; **and**
      `grep -F -q 'it-governance-grc' "${COURSES}erp-security-and-controls/overview.md"` exits 0
      (exits 2 before this step, since the file does not exist yet); **and**
      `grep -F -q 'audit-controls-and-compliance' "${COURSES}erp-security-and-controls/_index.md"`
      exits 0 (exits 2 before this step, since the file does not exist yet).
  - _Suggested executor: `apps-ayokoding-www-annotated-concept-maker`_
...
- [ ] [AI] `sharia-compliant-erp-design` (Annotated-concept) — the engineering lesson is
      **jurisdictional pluggability**: the chart of accounts, recognition rules, and disclosure set are
      ...
      the Bank Negara Malaysia Shariah Governance Policy 2019 — and states, hedged as `[Unverified]`
      pending the Phase-1.2 resolution (see
      [tech-docs §Load-bearing for #19](./tech-docs.md#load-bearing-for-19--there-is-no-single-sharia-accounting-standard)),
      that **Malaysia is not on AAOIFI's mandatory-adoption list**. Any numbered PSAK standard, and
      this Malaysia claim, obey the same Phase-1.2 resolution —
...
- [ ] [AI] **RED** — add a failing e2e assertion for boundary 3 on the landing — command:
  ...
  **Gherkin (binds) →** "A reader reaches full competence after the twentieth course"
...
- [ ] [AI] **GREEN** — update `"${ERPLANDING}"` so boundary 3 flips from **listed** to **reached**:
      replace its `Coming after capstone-build-a-minimal-erp-core` marker with `Reached after
capstone-build-a-minimal-erp-core`, and bind the assertion — command:
      `npx nx run ayokoding-www:specs:behavior:coverage && npx nx run ayokoding-www-fe-e2e:test:e2e`
      — acceptance: both exit 0, AND `grep -F -q 'Reached after capstone-build-a-minimal-erp-core' "${ERPLANDING}"`
      exits 0 (exits 1 before this step), AND `grep -F -q 'Coming after capstone-build-a-minimal-erp-core' "${ERPLANDING}"`
      exits **1** (exits 0 before this step).
...
- [ ] [AI] `grep -F -q 'Reached after capstone-build-a-minimal-erp-core' "${ERPLANDING}"` exits 0;
      `grep -F -q 'Coming after capstone-build-a-minimal-erp-core' "${ERPLANDING}"` exits **1** —
      boundary 3 is fully transitioned, not merely supplemented.
- [ ] [AI] `npx nx run ayokoding-www:test:unit` + `:build` + `:specs:behavior:coverage` **and**
      `npx nx run ayokoding-www-fe-e2e:test:e2e` exit 0.
- [ ] [AI] Draft PR opened; 3-cycle PR-Review complete; CI green; PR `[AI]`-merged; deployed.
...
- [ ] [AI] **DD-14 live-network scan** — confirm no code sample in courses #4, #10, or #20 depends on
      a live network call to a third-party ERP vendor's cloud SaaS (containerised or fixtured ERP
      only, per DD-14) — command:
      `grep -rIoE 'https?://[A-Za-z0-9.-]*\.(sap|dynamics|netsuite|odoo)\.com' "${COURSES}capstone-stand-up-and-integrate-an-open-source-erp" "${COURSES}erp-integration-patterns" "${COURSES}capstone-build-a-minimal-erp-core" 2>/dev/null | wc -l`
      — acceptance: returns **0**. Falsifiable both ways: pasting one line containing
      `https://api.sap.com/...` (or the Dynamics/NetSuite/Odoo cloud equivalent) into any of the three
      course bundles makes the count **1** or more.
...
- [ ] [AI] **Ownership boundary check** — confirm this plan wrote nothing outside its own surface:
      ...
      returns **0**, AND no structural or shared-catalog `_index.md` was **created** by this plan
      (only populated) —
      `git diff --name-only --diff-filter=A origin/main...HEAD | grep -E 'paths/(_index|skills/_index|careers/)|courses/_index\.md' | wc -l`
      returns **0** — acceptance: all three return **0**. Falsifiable both ways: touching a careers
      manifest, creating `paths/skills/_index.md`, or creating `courses/_index.md` makes the
...
- [ ] [AI] The DD-14 live-network scan returns **0** across courses #4, #10, and #20.
