...
| A capstone body's sample code depends on a live third-party ERP and breaks CI.                                | DD-14 forbids a live-network dependency in any code sample across all three touchpoints — #4, #10, and #20: the #4 and #20 capstones each stand up a containerised or fixtured ERP, and #10's integration examples run on recorded fixtures, so the third-party API stays subject matter rather than a build dependency.                                        |
...
| Cross-plan references break when a sibling plan migrates across the plan lifecycle folders.                   | Sibling plans are referenced by **name in code spans, not links** — not because a sibling folder is missing, but because a relative link authored at planning time can break as any plan (this one included) migrates `plans/backlog/` → `plans/in-progress/` → `plans/done/`; the archival phase re-runs a scoped link validation over this plan's own folder. |
