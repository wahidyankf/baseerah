...
| `content-url.ts`'s new optional `pathId` parameter regresses existing learn pages.                                                                                                                                                                                        | The change is purely additive (one optional `pathId` param appending a query string; the `/{locale}/c/{slug}` URL shape the function already emits is preserved byte-for-byte), so the seven existing `content-url` assertions stay **unchanged** and keep passing; Phase 4 runs a live no-regression sweep across **both** supported locales at three breakpoints with committed screenshot evidence.                                                                                                                                                                    |
