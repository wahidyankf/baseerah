...
1. **It unblocks two plans at once.** Both Wave-2 plans consume this plan's output — the UI plan
   imports the six pure `core/` modules; the authoring plan authors against the `syllabus/` specs
...
- **The `course-paths` Gherkin companion exists** (observable): the feature files land under
  `specs/apps/ayokoding/behavior/ayokoding-www/gherkin/course-paths/`, every scenario in them is
  tagged `@wip` (the validator's own step-binding-deferral exemption [Repo-grounded —
  `apps/rhino-cli/src/application/behavior_coverage/validator.rs`: "`@wip` scenarios are fully
  exempt"]), and `npx nx run ayokoding-www:specs:behavior:coverage` **exits 0** both before and after
  the new domain lands.
- **The `syllabus/` corpus is intact and linkable** (observable): the folder holds exactly 128 files
  at archival — with exactly one recorded content exception (the R3 custody exception, 2026-07-21) —
  and the pre-push-hook form of `md links validate` prints `All links valid! No broken links found.`
  after the archival repoint.
- **The schema is category-agnostic by construction** (observable, R2/R8): `PathManifest` validates
  both a 3-segment `careers/<arc>/<role>` fixture and a 2-segment `skills/<subject>` fixture without
  either fixture asserting a specific segment count, and rejects a fixture whose first segment is
  neither `careers` nor `skills`. Every fixture — careers and skills alike — carries a required `arc`
  field. Verified by unit test (`schemas.test.ts`, Phase 1.2).
- **No rendered surface regresses** (observable): after `content-url.ts` gains the optional `pathId`
  param — the URL shape it already emits, `/{locale}/c/{slug}` (so a course is
  `/en/c/learn/courses/<course-id>`), being preserved byte-for-byte — existing learn pages in
  **both** supported locales (`en` and `id` [Repo-grounded — `SUPPORTED_LOCALES` in
...
test:integration test:e2e specs:behavior:coverage` exits 0, and
  `npx nx run ayokoding-www:build` exits 0. Note that `ayokoding-www`'s `test:integration` and
  `test:e2e` targets are `echo` no-ops [Repo-grounded — `apps/ayokoding-www/project.json`], so they
  are run for completeness only and carry **no** evidential weight; the actual regression evidence
  for `content-url.ts` is `content-url.test.ts` plus the Phase 4 Playwright sweep.
...
- **Content-editing any file under `syllabus/`.** The corpus's curriculum content is custodied here,
  not authored here, **with exactly one recorded content exception** — the R3 custody exception
  (2026-07-21 ruling), which corrects the fourth path's manifest mirror from an SWE→AI transition
  framing to a from-scratch framing. (A separate, non-content mechanical `careers/`-prefix string
  correction to existing path-id text, required by R1/R2, is not a content exception.) See
  [tech-docs.md §Custody rules](./tech-docs.md#custody-rules-binding).
- **Owning, authoring, or scoping the `skills/` URL category.** All **four** `skills/` path landings,
  four skills manifests (amendment A10), and the entire ERP + accounting course corpus belong
  end-to-end to **two** sibling plans (R4): `ayokoding-learning-path-06-skills-accounting` owns
  `skills/conventional-accounting` and `skills/sharia-accounting`, and
  `ayokoding-learning-path-07-skills-erp` owns `skills/conventional-erp` and `skills/sharia-erp`.
  This plan's schema and resolvers are built category-agnostic (R2) so all four can consume them
  without a rewrite, but this plan authors none of the skills content itself.
...
| **The `syllabus/` corpus is copied into a consuming plan instead of linked, forking 120 specs.**                                                                                                                                                                                                                                                                                                                     | The custody rule is stated in this plan's `README.md` and restated as a `> **Cross-plan source of truth**` blockquote in each sibling plan's README. A copy is a boundary violation, not a convenience.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **This plan archives (Wave 1) while every sibling plan folder still links into `syllabus/`, breaking every one of those links.**                                                                                                                                                                                                                                                                                     | The `Plan Archival` phase carries a reciprocal repoint step that rewrites every inbound cross-plan link in the **same commit** as the `git mv`, then runs the pre-push hook's own form of `md links validate`. The exact inventory is measured at execution time (Phase 7.2 records `N_BEFORE`) rather than hardcoded, and the sweep covers **every** sibling folder under `plans/backlog` and `plans/in-progress`. Without it the next push from any surviving plan fails for an unrelated reason.                                                                                                                                                                                                                                                                      |
...
| `content-url.ts`'s new optional `pathId` parameter regresses existing learn pages.                                                                                                                                                                                                                                                                                                                                   | The change is purely additive (one optional `pathId` param appending a query string; **this plan** does not move the `/{locale}/c/{slug}` shape the function already emits), so the seven existing `content-url` assertions stay **unchanged** by _this_ plan's edit and keep passing. Note the `/c/` segment itself is **not** guaranteed to survive Wave 1 — the sibling `ayokoding-learning-path-01-url-restructure` deletes it, and whichever plan merges second rebases and re-baselines (see `delivery.md`'s shared-seam note); Phase 4 runs a live no-regression sweep across **both** supported locales at three breakpoints with committed screenshot evidence.                                                                                                 |
...
| **The `pathId` schema hardcodes a fixed segment count (e.g. always 3), silently breaking the future `skills/` category (2-segment ids) or forcing a rewrite when it lands.**                                                                                                                                                                                                                                         | `.refine()` validates only the first segment (`careers`/`skills`) plus a **minimum** of one further segment — never a **fixed** segment count and never an upper bound (R2). Proven by a unit-test fixture trio (2-segment skills, 3-segment careers, 4-segment `careers/a/b/c`, all valid) with no fixture asserting a specific depth (R2/R8, Phase 1.2).                                                                                                                                                                                                                                                                                                                                                                                                               |
| **A skills path is modelled as "arc-less" because its URL omits the arc segment, corrupting the data model for a field the URL grammar simply doesn't surface.**                                                                                                                                                                                                                                                     | `arc` is a required `PathManifest` field independent of URL grammar (R8); a skills manifest still records `arc: immediately-effective` even though its URL never shows it. Verified by the same Phase 1.2 unit-test fixtures.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **The one-time R3 custody exception is mistaken for an open door to further `syllabus/` edits, eroding the custody guarantee downstream plans rely on.**                                                                                                                                                                                                                                                             | The exception is recorded once, by name, with its own binding custody rule (rule 2) stating it is a single, reasoned, dated exception — not a precedent. See [tech-docs.md §Custody rules](./tech-docs.md#custody-rules-binding).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **A manifest that legitimately links a prerequisite (link-don't-walk) and a manifest that simply forgot one are indistinguishable to `checkPrerequisiteConsistency` — the check can never fail in the direction of a genuinely missing prerequisite (OI-4, 2026-07-21).**                                                                                                                                            | By design, not a gap: completeness is an editorial judgment requiring audience context the function never has, exactly like every other smoothness property (tech-docs.md §Smoothness Architecture). The function's informational `linkedPrerequisites` output plus the manifest's own "linked, not included" prose give a **human reviewer** the two artefacts to diff; the plan does not pretend a machine check can replace that review. See [tech-docs.md §Link-don't-walk](./tech-docs.md#link-dont-walk-prerequisite-omission-is-permitted-oi-4-ruling-2026-07-21).                                                                                                                                                                                                |
| **A per-course syllabus spec (or its future revision) reproduces copyrighted structure — a well-known book's chapter progression, a paid course's module sequence, or an external curriculum's outline — instead of being independently authored (programme [`A8`](../ayokoding-learning-path-programme.md#programme-decisions-r--a) / [`A12`](../ayokoding-learning-path-programme.md#programme-decisions-r--a)).** | Every syllabus in this corpus is authored from domain reasoning first; external research is used only to test coverage, never adopted as structure (A12's binding order of operations). `A8` is cited, not restated, in [tech-docs.md §Design Decisions](./tech-docs.md#design-decisions), alongside the pre-existing narrower `DD-15` (technology-choice licensing) inherited from the corpus's original authoring plan — the two are complementary, not duplicate. No retroactive audit of the existing corpus (120 syllabus spec files, carrying 127 course ids once the seven DD-20 capstones embedded in host-topic files are counted) is in this plan's delivery; see this plan's `delivery.md` for the going-forward check on any syllabus this plan still edits. |
