...
- **Owning, authoring, or scoping the `skills/` URL category.** Both `skills/` path landings, both
  skills manifests, and the entire ERP + accounting course corpus belong end-to-end to **two**
  sibling plans (R4): `ayokoding-learning-path-06-skills-accounting` owns `skills/accounting`, and
  `ayokoding-learning-path-07-skills-erp` owns `skills/enterprise-resource-planning`. This plan's
  schema and resolvers are built category-agnostic (R2) so both can consume them without a rewrite,
  but this plan authors none of the skills content itself.
