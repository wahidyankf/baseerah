...
> `plans/backlog/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/`. Do not copy
> them; do not author from any other source. Every course body in this plan is authored **from** its
> [`syllabus/courses/<course-id>.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses/README.md)
...
> [`ayokoding-www-learning-path-05-manifests`](../ayokoding-www-learning-path-05-manifests/README.md).
...
| `PLAN`              | `ayokoding-www-learning-path-04-course-authoring`                                 |
...
`worktrees/ayokoding-www-learning-path-04-course-authoring/`, open a draft PR per phase against `main`,
...
| ------------- | ------------------------------------------------------------ | ---------------------------------------------------------------- |
| **blockedBy** | `ayokoding-www-learning-path-01-url-restructure`             | hard — populated flat `courses/` namespace + `courses/_index.md` |
| **blockedBy** | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | hard — `syllabus/courses/` specs + the `prerequisites` contract  |
| **blocks**    | `ayokoding-www-learning-path-05-manifests`                   | hard — 90 authored bodies + the band-completion signals          |
| _(sibling)_   | `ayokoding-www-learning-path-03-navigation-ui`               | none — same wave, independent surface, no shared file            |
...
**This plan never edits a manifest file.** Every file under
`apps/ayokoding-www/src/features/course-paths/manifests/` is owned by
`ayokoding-www-learning-path-05-manifests`. When a band lands, this plan records a
...
| ------------------------------------------------------------ | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ayokoding-www-learning-path-01-url-restructure`             | 37 re-homed course bundles occupying the flat `courses/` namespace | the 23-new-slug collision check is vacuous against an empty namespace, and a native-authored slug could silently collide with a not-yet-moved re-home slug |
| `ayokoding-www-learning-path-01-url-restructure`             | `apps/ayokoding-www/content/en/learn/courses/_index.md`            | every authored course is listed in this catalog landing                                                                                                    |
| `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | `syllabus/courses/<course-id>.md` — 121 settled spec files         | each body is authored **from** its spec (`co-NN` concepts, `ex-NN` examples, prerequisite chain, capstone spec), never from a fresh judgment call          |
| `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | the `prerequisites: [course-id, ...]` frontmatter contract         | every net-new `_index.md` declares it                                                                                                                      |
...
3. `test -d apps/ayokoding-www/content/en/learn/courses` returns 0 and the directory holds the
   **37** re-homed bundles.
4. `test -f plans/<stage>/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses/README.md`
...
| ------------------------------------------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `ayokoding-www-learning-path-05-manifests` | 90 authored course bundles under `<COURSES>`                                   | manifest integrity fails on any `courseOrder` ID with no resolving bundle |
| `ayokoding-www-learning-path-05-manifests` | the six net-new AI course bodies                                               | the fourth path's spine references exactly these six                      |
| `ayokoding-www-learning-path-05-manifests` | the locked evals forward-link / D9 citation / D11 concept contracts            | the four-path blast-radius statement cites them                           |
| `ayokoding-www-learning-path-05-manifests` | one **band-completion signal** per band, recorded in this plan's `delivery.md` | triggers the corresponding manifest-growth step there                     |
...
> authored bodies** (37 re-homed bundles arrive from `ayokoding-www-learning-path-01-url-restructure`;
...
point. The canonical owner for citation purposes is `ayokoding-www-learning-path-05-manifests` (its
...
- **DL-6 · Library source & catalog (baseline 121 → 127, course surgery now permitted — amended
  ...
  - _Amendment split across plans_: DL-6 is amended by **DL-15**, which lands in
    ...
    [`ayokoding-www-learning-path-01-url-restructure`](../ayokoding-www-learning-path-01-url-restructure/README.md);
...
- **DL-14 · Seven orphaned inter-topic capstones promoted to first-class library courses (baseline
  ...
  - _Split note_: this plan authors six of the seven natively (Band 8). `capstone-solid-core` is
    already live on disk and is re-homed by `ayokoding-www-learning-path-01-url-restructure`, not
    authored here.
...
1. **It ships no screen and no component.** Every artefact is a markdown page bundle under
   ...
   `ayokoding-www-learning-path-03-navigation-ui`, which carries the mandatory retest.
...
- **Cross-plan**:
  [`syllabus/` source of truth](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/README.md)
  ·
  [`syllabus/courses/` catalog](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses/README.md)
  ·
  [`syllabus/paths/` manifests](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/README.md)
  · [manifest plan](../ayokoding-www-learning-path-05-manifests/README.md)
  · [URL-restructure plan](../ayokoding-www-learning-path-01-url-restructure/README.md)
  · [navigation-UI plan](../ayokoding-www-learning-path-03-navigation-ui/README.md)
  · [schema plan](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/README.md)
