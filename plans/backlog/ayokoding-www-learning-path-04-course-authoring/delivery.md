...
> [`ayokoding-www-learning-path-05-manifests`](../ayokoding-www-learning-path-05-manifests/README.md). This
...
> [`../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/README.md).
...
Worktree path: `worktrees/ayokoding-www-learning-path-04-course-authoring/`

Optional manual pre-provisioning (run from repo root):

```bash
claude --worktree ayokoding-www-learning-path-04-course-authoring
```
...
(`git fetch origin && git checkout main && git pull && git checkout -b ayokoding-www-learning-path-04-course-authoring/<phase-slug>`),
...
| --------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| **blockedBy**   | `ayokoding-www-learning-path-01-url-restructure`             | **Hard.** Creates the flat `<COURSES>` bucket + `<COURSES>_index.md` + the 37 re-homed bundles this plan joins. |
| **blockedBy**   | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | **Hard.** Owns `syllabus/` (every authoring source spec) and the `prerequisites` frontmatter contract.          |
| **blocks**      | `ayokoding-www-learning-path-05-manifests`                   | The manifest plan's `courseOrder` IDs resolve only after this plan's bands land; it consumes the band signals.  |
| **independent** | `ayokoding-www-learning-path-03-navigation-ui`               | Same Wave 2. Touches `<FEAT>` app code only; this plan touches content only. No shared file.                    |
...
- `<SYLLABUS>` = `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/` (cross-plan authoring source of truth — **never copied**)
...
- [ ] [AI] **Verify blocking plan #2 merged** — the cross-plan syllabus layer is on `origin/main`:
      `test -d plans/done/*__ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses || test -d plans/in-progress/ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses`
...
- [ ] [AI] **Record the authored-body baseline (the falsifiable-both-ways anchor for archival)** —
      ...
      `ayokoding-www-learning-path-05-manifests`, never here.
- [ ] [AI] Confirm `learnings.md` exists in the plan folder with its H1 — command:
      ...
      `# Learnings: ayokoding-www-learning-path-04-course-authoring`.
- [ ] [AI] **Cross-plan link gate (BF-8)** — confirm every `../ayokoding-www-learning-path-*` reference
  ...
  cargo run --release --manifest-path apps/rhino-cli/Cargo.toml -- md links validate \
    ...
    --exclude apps/ose-www/content 2>&1 | grep -F "ayokoding-www-learning-path-04-course-authoring"
  ```

  — acceptance: the `grep` finds **no** matching line (exits 1). Falsifiable both ways: adding one
  bad `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/nope.md` link makes the
...
> in the cross-plan
> [`syllabus/courses/`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/courses/README.md) —
...
- [ ] [AI] **State the four-path blast radius (DD-28 binding rule)** — for the evals extraction, the D9
      ...
      performed by `ayokoding-www-learning-path-05-manifests`.
...
- [ ] [AI] **Verify zero manifest files were touched by this entire plan** —
      `git log origin/main --name-only --pretty=format: --grep 'ayokoding-www-learning-path-04-course-authoring' | grep -F 'src/features/course-paths/manifests/' | sort -u | wc -l`
...
> [`ayokoding-www-learning-path-03-navigation-ui`](../ayokoding-www-learning-path-03-navigation-ui/README.md);
...
- [ ] [AI] Confirm no plan PR is still open:
      `gh pr list --search "ayokoding-www-learning-path-04-course-authoring" --state open --json number --jq 'length'`
...
- [ ] [AI] **Notify the downstream manifest plan** — confirm all ten band-completion signals are
      ...
      [`ayokoding-www-learning-path-05-manifests`](../ayokoding-www-learning-path-05-manifests/delivery.md)
      — acceptance: `git show origin/main:plans/*/ayokoding-www-learning-path-04-course-authoring/delivery.md | grep -F 'MERGED_COMMIT:' | wc -l`
      returns **10**.
...
- [ ] [AI] **Verify this plan's authored-body assertion** —
      ...
      `ayokoding-www-learning-path-05-manifests`'s terminal assertion (90 authored here + 37 re-homed by
...
- [ ] [AI] Move: `git mv plans/in-progress/ayokoding-www-learning-path-04-course-authoring/ plans/done/YYYY-MM-DD__ayokoding-www-learning-path-04-course-authoring/`
...
- [ ] [AI] Commit the archival:
      `chore(plans): move ayokoding-www-learning-path-04-course-authoring to done`.
...
- [ ] [AI] Plan folder is under `plans/done/YYYY-MM-DD__ayokoding-www-learning-path-04-course-authoring/`;
...
This plan is created in `plans/backlog/ayokoding-www-learning-path-04-course-authoring/`. When work
starts it is promoted to `plans/in-progress/ayokoding-www-learning-path-04-course-authoring/` (no date
prefix on either); the `git mv` in Phase 16 then archives it to
`plans/done/YYYY-MM-DD__ayokoding-www-learning-path-04-course-authoring/` using the completion date.
</content>
