...
> [`ayokoding-www-learning-path-05-manifests`](../ayokoding-www-learning-path-05-manifests/README.md). A
...
[`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/README.md).
This plan is its single largest consumer and **never copies it**.

- Every reference uses the **full cross-plan relative path**:
  `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/<rest>`. The source plan's
...
introduce one bad `../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/` link and the
...
[`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/tech-docs.md).
...
- **DD-20 · Seven inter-topic capstones promoted to first-class catalog/manifest entries (2026-07-19
  ...
  **Ruling: promote all seven** to the [Course Library Catalog](#course-library-catalog), all three
  ...
  - _Split note_: this plan authors six of the seven natively (Band 8). `capstone-solid-core` is
    re-homed by `ayokoding-www-learning-path-01-url-restructure`; the manifest placements are performed
    by `ayokoding-www-learning-path-05-manifests`.
...
- **DD-28 · Course surgery (update / merge / split / create) now permitted; six net-new AI courses
  ...
  - **Amendment pair split across plans (binding — read both halves).** DD-28 is the **amendment**;
    ...
    [`ayokoding-www-learning-path-05-manifests`](../ayokoding-www-learning-path-05-manifests/tech-docs.md#design-decisions).
...
| ----- | ---------------------------------------------------------------- | ------------------------------------------------------------- |
| DD-2  | One canonical body + URL per course; re-home with redirects      | `ayokoding-www-learning-path-01-url-restructure`              |
| DD-6  | Every course declares `prerequisites` → a prerequisite DAG       | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`  |
| DD-7  | Omit-or-create; per-path framing is a callout, never a body fork | `ayokoding-www-learning-path-05-manifests` (amended by DD-28) |
| DD-16 | Prerequisite-consistency is the audited smoothness property      | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`  |
| DD-21 | The AI path teaches building AI systems, not driving them        | `ayokoding-www-learning-path-05-manifests`                    |
| DD-22 | Convergence amended: paths converge per role, not globally       | `ayokoding-www-learning-path-05-manifests`                    |
| DD-24 | Fourth path's entry point: linked, not included, prerequisites   | `ayokoding-www-learning-path-05-manifests`                    |
| DD-33 | Fourth path's manifest WALKS the AI/harness cluster; spine is 15 | `ayokoding-www-learning-path-05-manifests`                    |
...
> `ayokoding-www-learning-path-01-url-restructure`. The 127 total is the manifest plan's terminal
...
`ayokoding-www-learning-path-05-manifests`. `prerequisites` are the course's own DAG edges (`—` = entry
...
(33 E + 4 Ecap) are re-homed by `ayokoding-www-learning-path-01-url-restructure`. 90 + 37 = 127.
...
[`ayokoding-www-learning-path-03-navigation-ui`](../ayokoding-www-learning-path-03-navigation-ui/prd.md);
Screen 4's funnel is owned by
[`ayokoding-www-learning-path-01-url-restructure`](../ayokoding-www-learning-path-01-url-restructure/prd.md).
...
| ------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------- |
| `ayokoding-www-learning-path-01-url-restructure` merged             | hard, plan | populated flat `courses/` namespace + `courses/_index.md`            |
| `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` merged | hard, plan | `syllabus/courses/` specs + the `prerequisites` frontmatter contract |
