...
> [navigation-UI plan](../ayokoding-www-learning-path-03-navigation-ui/prd.md) and the
> [manifest plan](../ayokoding-www-learning-path-05-manifests/prd.md).
...
re-homed by `ayokoding-www-learning-path-01-url-restructure`, not authored here) — alongside the 61
...
specs exist at
[`syllabus/courses/`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/README.md)
...
- **Any manifest file** under `<MANIFESTS>` — creating, appending to, reordering, or re-verifying.
  Owned by `ayokoding-www-learning-path-05-manifests`. Binding invariant.
...
- **Any `course-paths` feature code** (`core/` or `shell/`) — owned by the schema and navigation-UI
  plans.
- **Any redirect module or rule** — owned by `ayokoding-www-learning-path-01-url-restructure`.
- **The 33 shipped topics and the 4 existing capstones** (including `capstone-solid-core`) — re-homed,
  not authored, by `ayokoding-www-learning-path-01-url-restructure`.
