...
- **Prerequisite edges get invented.** A body that declares a prerequisite the spec never named adds
  ...
  **not** surface here. It surfaces much later, in
  `ayokoding-www-learning-path-05-manifests`, as a manifest-integrity failure with no traceable link
...
`ayokoding-www-learning-path-03-navigation-ui`), data/schema author (owned by
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`), path composer (owned by
`ayokoding-www-learning-path-05-manifests`), IA/URL owner (owned by
...
- **Editing any manifest file.** Binding invariant — owned by
  `ayokoding-www-learning-path-05-manifests`. Not a scope preference; a correctness requirement.
- **Building any part of the navigation UI.** The path rail, banner, breadcrumb, prerequisite-list
  component, path landings, and paths hub belong to `ayokoding-www-learning-path-03-navigation-ui`.
- **Re-homing the 33 shipped topics or the 4 existing capstones.** Those bodies already exist; moving
  them is `ayokoding-www-learning-path-01-url-restructure`'s work. This plan authors only what has no
...
- **Defining the `prerequisites` frontmatter contract.** This plan **consumes** the contract;
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` owns its canonical shape.
