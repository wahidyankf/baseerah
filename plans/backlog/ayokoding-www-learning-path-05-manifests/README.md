...
`ayokoding-www-learning-path-04-course-authoring` owns course **bodies only** and **never edits a
manifest**.
...
which is why this plan's hard prerequisite is **both** Wave-2 plans (`ayokoding-www-learning-path-03-navigation-ui`
**and** `ayokoding-www-learning-path-04-course-authoring`) rather than the navigation plan alone.

Its two mechanical consequences:

- **Five steps are absorbed** from `ayokoding-www-learning-path-04-course-authoring` into this plan —
...
- Any course **body** — authored by `ayokoding-www-learning-path-04-course-authoring`.
- The `PathManifest` zod schema, the pure `course-paths` core modules, the `<MANIFESTS>` directory
  ...
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
- Every rendering component (`path-landing.tsx`, `path-card.tsx`, `path-rail.tsx`,
  `manifest-repository.ts`, `?path=` wiring) — owned by `ayokoding-www-learning-path-03-navigation-ui`.
- The flat `courses/` namespace, the `legacy/` bucket, and both redirect modules — owned by
  `ayokoding-www-learning-path-01-url-restructure`.

## Where this plan sits

```mermaid
%% Wave and dependency position of ayokoding-www-learning-path-05-manifests within the five-way split.
...
| ----------- | ------------------------------------------------------------ | --------------------------------------- |
| `blockedBy` | `ayokoding-www-learning-path-03-navigation-ui`               | hard — merged to `origin/main` first    |
| `blockedBy` | `ayokoding-www-learning-path-04-course-authoring`            | hard — merged to `origin/main` first    |
| `blockedBy` | `ayokoding-www-learning-path-01-url-restructure`             | transitive, via both Wave-2 plans       |
| `blockedBy` | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | transitive, via both Wave-2 plans       |
...
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| `ayokoding-www-learning-path-03-navigation-ui`                              | `<FEAT>shell/path-landing.tsx` + `path-card.tsx`                                                                                                       | a published manifest with no renderer is invisible; every landing phase gate asserts a rendered ordered list |
| `ayokoding-www-learning-path-03-navigation-ui`                              | `<FEAT>shell/manifest-repository.ts`                                                                                                                   | a YAML manifest is inert until this loads and validates it at build time                                     |
| `ayokoding-www-learning-path-03-navigation-ui`                              | `?path=` route wiring + prev/next + breadcrumb                                                                                                         | every path-walk e2e in this plan exercises them                                                              |
| `ayokoding-www-learning-path-04-course-authoring`                           | 90 authored course bundles under `<COURSES>` (61 transferred topics + 10 new courses + 8 capstones + 5 Band-9 interview bodies + 6 net-new AI courses) | manifest integrity fails on any `courseOrder` ID with no resolving bundle                                    |
| `ayokoding-www-learning-path-04-course-authoring`                           | the six net-new AI course bodies                                                                                                                       | the fourth path's six-course spine references exactly these                                                  |
| `ayokoding-www-learning-path-04-course-authoring`                           | the locked evals / D9-citation / D11-concept contract record                                                                                           | the four-path blast-radius statement cites it                                                                |
| `ayokoding-www-learning-path-01-url-restructure` _(transitive)_             | 37 re-homed course bundles + `<PATHS>_index.md`                                                                                                        | the first manifest's `courseOrder` is exactly these IDs; the hub renders under `<PATHS>`                     |
| `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` _(transitive)_ | `PathManifest` zod schema, integrity gates, `syllabus/paths/manifest-*.md` mirrors                                                                     | each `courseOrder` is transcribed from its mirror and validated by the gates                                 |

**Start precondition (checkable — all five must hold):**

1. PR for `ayokoding-www-learning-path-03-navigation-ui` is **merged to `origin/main`**.
2. PR for `ayokoding-www-learning-path-04-course-authoring` is **merged to `origin/main`**.
...
**Split mapping of that order.** Group A is delivered by `ayokoding-www-learning-path-01-url-restructure`,
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag` and
`ayokoding-www-learning-path-03-navigation-ui` (Waves 1–2). Every remaining step — MVP manifest, AI-path
...
backfill **bodies** are delivered by `ayokoding-www-learning-path-04-course-authoring` in Wave 2, ahead of
...
> **DL-6 lands in `ayokoding-www-learning-path-04-course-authoring`, and DL-15 amends it.** Because the
...
the interview-ready path in its `Given`, spanning `ayokoding-www-learning-path-03-navigation-ui` and this
...
`worktrees/ayokoding-www-learning-path-05-manifests/`, open a draft PR per phase against `main`, run the
...
- [Syllabus (cross-plan, read-only)](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/README.md) —
  the per-course and per-path detail layer, owned by
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`. The four
  [`paths/` manifest mirrors](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/README.md)
