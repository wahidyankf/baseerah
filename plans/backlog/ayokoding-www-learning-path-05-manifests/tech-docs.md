...
> [`syllabus/paths/manifest-*.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/README.md)
...
| ------------------------- | ------------------------------------------------------------ | ------------------------------- |
| URL / IA                  | `ayokoding-www-learning-path-01-url-restructure`             | consumes (`courses/`, `paths/`) |
| Schema / core / integrity | `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` | consumes (zod, gates, syllabus) |
| Rendering / route wiring  | `ayokoding-www-learning-path-03-navigation-ui`               | consumes (components, `?path=`) |
| Course bodies             | `ayokoding-www-learning-path-04-course-authoring`            | consumes (127 bundles)          |
...
| `apps/ayokoding-www/content/en/learn/paths/_index.md`                                     | content | card population only — the file itself is `ayokoding-www-learning-path-01-url-restructure`'s |
...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`. They are restated here because every
...
`ayokoding-www-learning-path-03-navigation-ui` — a manifest published against no repository is never
...
`ayokoding-www-learning-path-04-course-authoring` that opens Phase 5.
...
[`syllabus/paths/` mirrors](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/README.md)
...
Mirror:
[`manifest-interview-ready-software-engineer.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/manifest-interview-ready-software-engineer.md).
...
[`manifest-immediately-effective-software-engineer.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/manifest-immediately-effective-software-engineer.md).
...
[`manifest-fundamentally-strong-software-engineer.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/manifest-fundamentally-strong-software-engineer.md).
...
[`manifest-immediately-effective-software-engineer-to-ai-engineer.md`](../ayokoding-www-learning-path-02-schema-and-prerequisite-dag/syllabus/paths/manifest-immediately-effective-software-engineer-to-ai-engineer.md),
...
- **DD-7 · Omit-or-create; per-path framing is a callout, never a body fork (amended 2026-07-20 by
  ...
  - **DD-28 lives in a different plan.** Read it at
    [`ayokoding-www-learning-path-04-course-authoring` §Design Decisions](../ayokoding-www-learning-path-04-course-authoring/tech-docs.md#design-decisions).
...
- **DD-13 · Harness-engineering cluster as a marquee build-your-own track** (manifest half). The five
  ...
  - **Resolved by DD-33**: it walks. The course **bodies** for this cluster are owned by
    `ayokoding-www-learning-path-04-course-authoring`; only their manifest placement is this plan's.
...
- **DD-33 · Fourth path's manifest WALKS the AI/harness cluster; spine is 15 courses, not 6 (D13,
  ...
  **Build-order consequence**: the 9 harness-cluster course bodies are authored by
  `ayokoding-www-learning-path-04-course-authoring` **after** this plan's AI-path manifest phase. That
...
courses; catalog 121 → 127), lands in **`ayokoding-www-learning-path-04-course-authoring`**. The amended
...
2. This plan's DD-7 carries a working cross-plan link to DD-28 at
   [`../ayokoding-www-learning-path-04-course-authoring/tech-docs.md#design-decisions`](../ayokoding-www-learning-path-04-course-authoring/tech-docs.md#design-decisions).
...
- **No net-new component.** Every rendering component the four landings and the hub use —
  ...
  `prerequisite-list.tsx` — is built and owned by `ayokoding-www-learning-path-03-navigation-ui`. This
...
- **No net-new screen.** The two screens this plan populates are Screen 1 (paths hub) and Screen 2
  ...
  citation, and the responsive strategy — owned by `ayokoding-www-learning-path-03-navigation-ui`'s
...
| `plans/backlog/ayokoding-www-learning-path-05-manifests/evidence/`                                                   | created                                   | 7          |

All paths are marked `_New file_` except `paths/_index.md`, which is created by
`ayokoding-www-learning-path-01-url-restructure` and only **populated** here. None of the manifest paths
...
- **Runtime / build** — no net-new npm dependency. The YAML loader, the zod schema, and the integrity
  gates are delivered by `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
