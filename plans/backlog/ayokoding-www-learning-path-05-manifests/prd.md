...
`/en/c/learn/paths/<path-id>`, and a card in the paths hub's 2×2 grid.
...
- **`immediately-effective/software-engineer-to-ai-engineer`** — the **immediately-effective** arc
  applied to a **role transition**: assumes an already-working software engineer; prerequisite
  SWE-fundamentals courses are **linked, not included**; teaches **building** AI systems (models,
  agents, evals, inference serving), not driving them (`agentic-coding` stays a separate, unrelated
  axis). Its `courseOrder` **walks** the nine-course AI/harness cluster directly (DD-33).
...
- As an **experienced engineer re-entering the market**, I want a published interview-ready manifest
  ...
  soonest.
- As an **already-working software engineer**, I want an AI-engineering path whose manifest **links**
  rather than re-lists the SWE fundamentals I already have, so that I get straight to building AI
  systems without walking material I have mastered.
- As an **already-working software engineer**, I want that path's `courseOrder` to actually **walk**
  the agent-building courses rather than linking past them, so that the path teaches what its own
  scope promises.
...
gate. `ayokoding-www-learning-path-03-navigation-ui` keeps a fixture-level substitute as a checklist
...
waves: its legacy-browse half is `ayokoding-www-learning-path-01-url-restructure`'s narrowed scenario,
...
`ayokoding-www-learning-path-03-navigation-ui`'s `PathCourseLinks`, but its **content** enumerates every
...
- Four thin landing anchors under `apps/ayokoding-www/content/en/learn/paths/<path-id>/_index.md` —
  prose, SEO metadata, fast-path affordance, and (for the AI path) the outbound canonical-page links
  to its linked-not-included prerequisites. **No `courseOrder` in any landing.**
- Population of the paths-hub 2×2 grid: one card per published path, added as each manifest ships, so
  the hub is coherent at every intermediate state.
...
- All manifest growth as backfill lands: Bands 1–8 into the three software-engineer manifests, Band 9
  into interview-ready and fundamentally-strong only, and the AI path's growth from its six-course
  smoke-test spine to its full 15-course composition.
- The four-path blast-radius statement for every course surgery that touches a manifest this plan
  owns.

### Out of scope

- **Any course body.** Owned by `ayokoding-www-learning-path-04-course-authoring`.
- **Any rendering component or route wiring.** Owned by `ayokoding-www-learning-path-03-navigation-ui`.
- **The `PathManifest` zod schema, the pure core modules, the `<MANIFESTS>` directory scaffold, and
  the `syllabus/` detail layer.** Owned by `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
- **URLs, redirects, the flat `courses/` namespace, the `legacy/` bucket, and the learn-section IA.**
  Owned by `ayokoding-www-learning-path-01-url-restructure`.
- **A fifth path.** The four-path composition is locked; a fifth would be its own plan.
...
`ayokoding-www-learning-path-03-navigation-ui` — Screen 1 (paths hub) and Screen 2 (path landing) — each
...
- **Growth applied to the wrong manifest**: Band 9's interview-technique courses are appended to
  `immediately-effective/software-engineer`, which omits that band by design. Mitigated by an
...
- **Split amendment fragmentation**: DD-7 lands here while its amendment DD-28 lands in
  `ayokoding-www-learning-path-04-course-authoring`, so a reader of either plan alone inherits a stale
...
- **Hub incoherence mid-flight**: the 2×2 grid shows empty or placeholder cards between manifest
  phases. Mitigated by populating exactly one card per shipped path at each phase and asserting the
  populated count at that phase's gate.
