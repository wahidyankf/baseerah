...
| `repo-rules-checker` body grows past comfortable size (currently ~51K [Repo-grounded — `ls` size]); maintenance gets harder | _Judgment call:_ low-medium | Agent file becomes unwieldy; future edits riskier                                                                 | Evaluate splitting `repo-rules-checker` if total body crosses an agreed threshold during execution; may move the new Step 8b into its own dedicated agent if size becomes a problem        |
| `npm run generate:bindings` fails to sync the modified agent body                                                           | Low                         | `.opencode/agents/repo-rules-checker.md` falls out of sync                                                        | Run `npx nx run rhino-cli:cross-vendor:parity-validation` post-sync; gate prevents merge while drift remains                                                                               |
