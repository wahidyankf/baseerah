...
- **Every re-homed course declares its prerequisites** (observable): all 37 re-homed `_index.md`
  ...
  in the shape owned by `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
...
- **Building the `course-paths` feature** — the pure core belongs to
  `ayokoding-www-learning-path-02-schema-and-prerequisite-dag` and the rendering layer to
...
- **Authoring or composing any path manifest** — every file under
  ...
  `ayokoding-www-learning-path-05-manifests`. This plan creates the `paths/_index.md` **content home**
...
- **Authoring any course body** — including the 61 transferred topics, the net-new courses, and the
  capstones. Those belong to `ayokoding-www-learning-path-04-course-authoring`.
- **Implementing the Screen 0 landing hero** — the hero funnel and its implementation belong to
  `ayokoding-www-learning-path-03-navigation-ui`. This plan owns Screen 4 only.
