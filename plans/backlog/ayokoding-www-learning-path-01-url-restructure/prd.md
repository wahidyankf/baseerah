...
documented by `ayokoding-www-learning-path-03-navigation-ui` in its `prd.md` §Learner Journey, the plan
...
`ayokoding-www-learning-path-03-navigation-ui` — that plan carries their funnel record and their 24
...
`ayokoding-www-learning-path-03-navigation-ui` in its R7 Prior-Art Findings section; the finding relevant
...
| ------ | ---------------- | ----------------- | ------- | ------------------------------------------------ |
| 0–3    | see scheme above | 2 × 3 each        | **24**  | `ayokoding-www-learning-path-03-navigation-ui`   |
| 4      | `legacy-landing` | 2 × 3             | **6**   | `ayokoding-www-learning-path-01-url-restructure` |

> **Cross-plan note on DD-47.** DD-47 mandates **30** renders (5 screens × 2 options × 3 viewports)
> across two plans — **6 here** and **24 in `ayokoding-www-learning-path-03-navigation-ui`**. A reader
...
> half above, and `ayokoding-www-learning-path-05-manifests` gains the both-routes scenario at its
...
- Adding `prerequisites: [course-id, ...]` frontmatter to each re-homed course `_index.md`, in the
  shape owned by `ayokoding-www-learning-path-02-schema-and-prerequisite-dag`.
...
- **Navigation regresses for non-path readers**: the relocation is one of two ways the sidebar can
  ...
  `ayokoding-www-learning-path-03-navigation-ui`, which owns that component and that risk.
