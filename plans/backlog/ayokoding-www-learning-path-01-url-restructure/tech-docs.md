...
`ayokoding-www-learning-path-04-course-authoring` — the bucket is deliberately under-filled at first, not
broken.
...
`ayokoding-www-learning-path-02-schema-and-prerequisite-dag`**, which builds the resolver that parses it.
...
is authored by `ayokoding-www-learning-path-05-manifests`, so this plan's own Phase-2 work re-points the
...
- **DD-20 (re-home half) · `capstone-solid-core` is a first-class catalog entry and is in the re-home
  ...
  `ayokoding-www-learning-path-04-course-authoring`, which owns DD-20 in full. The **seventh**,
...
- **DD-43 · `fundamentally-strong/` stays on per-course redirects; `learn-three-bucket.ts` carries no
  ...
  `ayokoding-www-learning-path-03-navigation-ui`. It is **moved into this plan**, because the negative
...
`ayokoding-www-learning-path-03-navigation-ui` and `ayokoding-www-learning-path-04-course-authoring` each
...
- Each `courses/<course-id>` whose subject is covered by a legacy page names that page as
  **superseded** in its own `overview.md`, so promotion is recorded where the survivor lives. For the
  ...
  `ayokoding-www-learning-path-04-course-authoring`'s.
...
`ayokoding-www-learning-path-05-manifests`, which merges after this plan. This plan therefore **preserves
the three files and their redirect targets** rather than deleting them, and hands the fold-in forward.
`ayokoding-www-learning-path-03-navigation-ui` is blocked on this ruling too — it determines what its
