...
- [x] [AI] Run `npm install && npm run doctor -- --scope minimal` — acceptance: exits 0.
- [x] [AI] Establish a green baseline for both in-scope suites against docker-compose (current `BASE_URL`
...
- [x] [AI] `git grep -nE '\bBASE_URL\b' -- apps/ose-be-e2e apps/organiclever-be-e2e` — acceptance: 2 hits
...
- [x] [AI] Confirm the failing-state assertion: `git grep -nE 'process\.env\.API_BASE_URL' -- apps/ose-be-e2e apps/organiclever-be-e2e`
      returns **no hits** (the new variable is not read yet) — acceptance: zero matches (RED).

### 1b — GREEN (apply the rename in lockstep)

- [x] [AI] Edit `apps/ose-be-e2e/playwright.config.ts`: `process.env.BASE_URL` → `process.env.API_BASE_URL`
...
- [x] [AI] Edit `apps/organiclever-be-e2e/playwright.config.ts`: same rename (keep `|| "http://localhost:8202"`)
      — acceptance: new name present, old name absent in that file.
- [x] [AI] Edit `.github/workflows/_reusable-app-test-local-deploy-stag.yml` "Run BE E2E tests" step:
...
- [x] [AI] Edit `apps/ose-be-e2e/README.md` and `apps/organiclever-be-e2e/README.md`: env-var table +
...
- [x] [AI] Edit `env-injection.yaml`: add the `API_BASE_URL` `ci-harness` entry (class `var`, environments
...
- [x] [AI] Run `actionlint .github/workflows/_reusable-app-test-local-deploy-stag.yml` — acceptance: zero errors.
- [x] [AI] Run `cargo run --release --quiet --manifest-path apps/rhino-cli/Cargo.toml -- env validate` —
      acceptance: exits 0 (env-injection drift guard passes with the new key).
- [x] [AI] Re-run both suites against docker-compose with `API_BASE_URL` set (via the local-stack harness or
...
- [x] [AI] Run `npx nx run rhino-cli:links:validation` and `npm run lint:md` — acceptance: both exit 0.
- [x] [AI] Run `npx nx affected -t typecheck lint test:quick specs:coverage` — acceptance: exits 0, no
      errors reported. Fix ALL failures found, including any preexisting issues not caused by this rename.

### 1d — Commit + push

- [x] [AI] Stage explicit paths (`apps/ose-be-e2e/playwright.config.ts apps/organiclever-be-e2e/playwright.config.ts .github/workflows/_reusable-app-test-local-deploy-stag.yml apps/ose-be-e2e/README.md apps/organiclever-be-e2e/README.md env-injection.yaml`)
...
- [x] [AI] Push to `origin main` and verify CI green (commons-quality-gate + actions + any app-affected
...
- [x] [AI] `git grep -nE '\bBASE_URL\b' -- apps/ose-be-e2e apps/organiclever-be-e2e` — acceptance: zero matches.
- [x] [AI] `git grep -nE '\bBASE_URL\b' -- apps/ose-www-be-e2e apps/ayokoding-www-be-e2e apps/organiclever-www-be-e2e`
      — acceptance: exactly 3 matches (www suites untouched).
- [x] [AI] `grep -n 'API_BASE_URL' .github/workflows/_reusable-app-test-local-deploy-stag.yml` — acceptance: matches.
- [x] [AI] `git grep -nE 'https?://' -- apps/ose-be-e2e/README.md apps/organiclever-be-e2e/README.md | grep -i stag`
...
- [x] [AI] After Phase 1 lands and CI is green, move plan to `done/`:
...
- [x] [AI] Remove this plan's row from `plans/in-progress/README.md`.
- [x] [AI] Add an entry to `plans/done/README.md` (note the deferred Phase 2).
- [x] [AI] Commit `chore(plans): move rename-be-e2e-api-base-url to done`.
