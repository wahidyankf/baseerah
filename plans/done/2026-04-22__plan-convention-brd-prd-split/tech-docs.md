...
- **`WebSearch`** — in-context search for a narrow, single-claim query. Escalate to `web-researcher` if verification requires 2 or more `WebSearch` calls or 3 or more `WebFetch` calls.
