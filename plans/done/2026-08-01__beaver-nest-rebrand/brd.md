...
on 2026-08-01 — this is a snapshot, not an immutable target; [delivery.md Phase
0](./delivery.md#phase-0-environment-setup-and-baseline) re-records the count at execution time and
surfaces any material drift to the maintainer before proceeding) rather than a slow drip of partial
renames that leave the repository in a permanently inconsistent state (some files say Baseerah, some
say BeaverNest, nobody is sure which is current).
...
- **Observable fact**: after this plan's Repo-Wide Residual Sweep phase, `git grep -liE "baseerah"`
  scoped to the whole repository except `plans/done/2026-07-31__baseerah-repo-reset/**` and every
  file the historical-citation preservation rule (see
  [tech-docs.md §Decision Log](./tech-docs.md#decision-log) Decision 6) legitimately keeps citing
  `baseerah-repo-reset` returns zero files.
...
- **No new deploy target is provisioned.** The existing dormant-deployer posture (deployer agents
  ...
  [baseerah-first-deploy idea](../../ideas/q2-not-urgent-important/beaver-nest-first-deploy.md) (renamed to
...
| Risk                                                                                                                                                                                | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A missed reference leaves a stray "Baseerah" string somewhere a future reader trips over it                                                                                         | The Repo-Wide Residual Sweep phase (Phase 16) runs a repo-wide case-insensitive grep, cross-checked against a rule (not a hardcoded list) that recognizes any file legitimately citing the archived `baseerah-repo-reset` plan-id as expected residual, plus an `md links validate` gate to catch any broken historical-citation link — any other hit is a defect fixed before the phase gate passes                                                                                                                                    |
...
| The GHCR hard cutover (Q9) breaks something that silently depended on the old image name                                                                                            | `[Repo-grounded]`: `gh api repos/wahidyankf/baseerah/environments` returns exactly two auto-created Environment objects (`baseerah-app-local`, `baseerah-app-staging`, both with empty `protection_rules` and no secrets — artifacts of earlier workflow runs, not deliberate config), and no `stag-*`/`prod-*` branch exists in `git branch -r` today (verified 2026-08-01) — nothing live consumes the old image name, so the cutover has no rollback cost; Phase 13 deletes both stale Environment objects after the workflow rename |
