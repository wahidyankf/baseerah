...
**Applies to**: All Agent-tool spawns, whether background or foreground. Both content-producing makers (e.g., `docs-tutorial-maker`) and meta-agents (e.g., `repo-rules-maker`) count toward N. Total simultaneous background Agent-tool invocations is the metric, not agent type.
