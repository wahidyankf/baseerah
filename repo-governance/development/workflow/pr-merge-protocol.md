...
- Environment branch deployments managed by CI (e.g., `prod-beaver-nest-fe`), which are governed by their own documented CI workflows.
...
### PASS: Correct agent behavior -- plan declares a `[HUMAN]` gate
...
### FAIL: Incorrect agent behavior -- merging through a declared `[HUMAN]` gate
