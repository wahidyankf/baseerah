...
AI agents working on worktrees routinely touch apps across many languages: `beaver-nest-be` (F#/Giraffe), `rhino-cli` (Rust CLI), `beaver-nest-fe` (TypeScript), and more. The probability that a new worktree session will need a toolchain that has drifted is high, and the cost of discovering the drift mid-task — through an obscure Gradle, Cargo, `mix`, or `dotnet` error — is much higher than the cost of running `npm run doctor -- --fix` deliberately upfront.
...
itself currently has no Elixir projects; its own F# apps, e.g. `beaver-nest-be`, use
