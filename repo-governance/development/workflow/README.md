...
- [Large-Scale Rename Plan Link Management](./large-scale-rename-plan-link-management.md) - Rules for keeping markdown links and path-referencing strings consistent through a multi-phase repo-wide identifier rename plan, given that the pre-push `md links validate` hook checks the whole tree regardless of which phase owns a given file: repoint repo-wide inbound links in the same commit as every `git mv`, revert an outbound link's target text if the phase renaming it runs before the target itself moves, and enumerate every pre-push-only check (not just the one motivating a deferral) before collapsing a push cadence across phases
