...
- [Dependency Bump Stability & Safety Policy](./workflow/dependency-bump-policy.md) - Three-path decision tree (LTS, 60-day soak, security waiver) governing every dependency bump across the polyglot monorepo. Mandates exact pinning, CVE clearance, cutoff-date computation in writing, and waiver documentation for Path C overrides
...
- [Subagent Orchestration Convention](./agents/subagent-orchestration.md) - Concurrency cap (default 3 simultaneous Agent-tool spawns) and 3-minute stuck-detection polling for background subagents. Prevents Claude API rate-limit hits and ensures stalled agents are detected and relaunched via TaskStop + relaunch
