...
| `test:integration`        | Backend: real configured production database with direct code calls (no HTTP); others: in-process mocking (MSW, Godog); `echo` placeholder where no real integration tests exist                                                                                                                             | All projects (echo where N/A)      |
...
| `test:integration` | Service-level tests use the app's real configured production database; frontend integration uses MSW/PGlite. `echo "no integration tests"` where no real integration test applies                |
...
| ------------------------ | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| Configured production DB | API backends (`beaver-nest-be`, once scaffolded)                 | Real app-selected database; calls application code directly (no HTTP layer); runs all shared Gherkin scenarios; fresh isolated database per run | No        |
...
**API backends** expose `test:integration` against an isolated instance of their configured production database, run migrations, and execute all shared Gherkin scenarios by calling application service/repository functions directly — no HTTP layer. A containerized database may use `docker compose -f docker-compose.integration.yml up --abort-on-container-exit --build` with database and test-runner services. SQLite may instead use a fresh test-owned database file. `Dockerfile.integration`, when used, contains the language runtime and test execution. Coverage is NOT measured at the integration level — coverage comes from `test:unit` only.
...
**API backend `test:integration` with docker-compose**: When an API backend's configured production
database is containerized, `test:integration` runs `docker compose -f docker-compose.integration.yml down -v && docker compose -f docker-compose.integration.yml up --abort-on-container-exit --build`.
Its `docker-compose.integration.yml` defines a database service with a healthcheck and a test-runner
service that waits for that database. The test runner runs migrations, optionally loads seed data, then
executes all shared Gherkin scenarios by calling application service/repository functions directly — no
HTTP layer. The specs volume is mounted read-only at `../../specs:/specs:ro`. After tests complete,
`docker-compose` tears down all containers and volumes. A SQLite backend instead creates and removes a
fresh test-owned database file without Docker.
...
| `test:integration`        | No     | Backends use their configured real production database (non-deterministic state). Default `cache: false` in `nx.json`. Projects using in-process mocking only (MSW, Godog) may override to `cache: true` in their `project.json` |
...
- **Using a real database in unit tests**: Unit tests must use mocked repositories or in-memory implementations — never a real database. Real configured production databases belong in backend integration tests or E2E tests
...
- **Enabling cache on database-backed `test:integration`**: Integration tests that use a configured real production database must have `cache: false` — stale results when database state matters. Only in-process-mocking integration tests (MSW, Godog) may enable caching
