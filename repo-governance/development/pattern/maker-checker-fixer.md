...
```markdown
User: "Add a new tutorial to docs/tutorials/ about retrieval-augmented generation (RAG)"

Maker Agent (docs-tutorial-maker):

1. Creates docs/tutorials/rag-fundamentals.md
2. Updates docs/tutorials/README.md (navigation/index)
3. Ensures cross-references from related how-to/reference docs are correct
4. Follows Diátaxis tutorial conventions (learning-oriented, hands-on)
5. Uses accessible colors in diagrams
6. Validates all internal links
7. Delivers complete, ready-to-publish content
```
...
```markdown
User: "Check the new RAG tutorial for quality issues"

Checker Agent (docs-tutorial-checker):

1. Reads docs/tutorials/rag-fundamentals.md
2. Validates frontmatter (required fields, category, subcategory)
3. Checks content structure (heading hierarchy, link format)
4. Validates Diátaxis tutorial conventions (learning-oriented, hands-on)
...
User: "Create a RAG fundamentals tutorial in docs/tutorials/"
...
| ------------- | -------- | ------- | -------------------------------------------- | ------------------------------------- |
| 🟦 **Blue**   | Writers  | Maker   | Has `Write` (creates new files)              | docs-tutorial-maker, readme-maker     |
| 🟩 **Green**  | Checkers | Checker | Has `Write`, `Bash` (no `Edit`)              | docs-tutorial-checker, readme-checker |
...
See [AI Agents Convention - Agent Color Categorization](../agents/ai-agents.md#agent-color-categorization) for complete details, including the [Platform Binding Color Translation](../agents/ai-agents.md#platform-binding-color-translation) subsection that documents how named colors map to platform binding color tokens via `rhino-cli harness bindings generate`.
