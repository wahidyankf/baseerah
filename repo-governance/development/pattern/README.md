...
- [Hexagonal Architecture — CLI Apps](./hexagonal-architecture-cli.md) - CLI specialization: `commands/` as inbound adapter, the canonical directory layout for `rhino-cli` (the sole surviving Rust CLI; the F# CLI layout pattern is retained for historical reference), layer responsibilities, and forbidden imports
