...
Note: Components still written with `React.forwardRef` and `@radix-ui/react-slot` should migrate to the `React.ComponentProps` + `radix-ui` pattern described above when next updated.
