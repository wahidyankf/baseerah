...
1. **Match by project/app name**: If the checklist item names a specific app (e.g., `beaver-nest-be`), use the agent for that app's language (e.g., `swe-fsharp-dev`). Refer to [CLAUDE.md](../../../CLAUDE.md) for the full app list and their tech stacks.
...
- Run the workflow's strictly sequential N-cycle loop (default **N = 3**): each cycle, the nine
...
- If status is `pass` (zero findings):
  ...
  **`worktree-to-pr` / `main-to-pr` (`*-to-pr` modes)** — archival-in-PR: the plan-folder move lands
  ...
  7. **Run or complete the PR-Review Maker→Fixer Cycle** against the PR (see the gate above) — because
     each cycle's reviewer pipeline (nine specialists → `pr-review-synthesis-maker`) reviews the full
