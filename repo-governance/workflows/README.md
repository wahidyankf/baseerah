...
| [PR-Review Maker→Fixer Cycle](pr/pr-review-quality-gate.md)     | Run a strictly sequential N-cycle (default 3) reviewer pipeline (eight specialists → `pr-review-synthesis-maker`) → `pr-review-fixer` loop against a pull request — post line-anchored findings via the GitHub Reviews API, triage and resolve every thread, gate on CI-green between cycles — until the `*-to-pr` done-definition (N cycles complete, every comment answered, gates green, archival-in-PR committed) is satisfied. Mandatory pre-merge gate for the `worktree-to-pr` and `main-to-pr` delivery modes, invoked from Plan Execution Step 8. | pr-review-synthesis-maker, pr-review-fixer                                                            | Medium     |
...
- `scope` — top-level domain matching the parent directory (`ci`, `docs`, `plan`, `pr`, `repo`, `specs`, `ui`, `web`, `infra`, etc.).
- `qualifier` — zero or more refinement tokens (e.g., `rules`, `dependency-bump`, `harness-compatibility`).
...
- **plan**: Project planning documents — establishment, two-pager→backlog idea promotion, multi-repo parity planning (with and without composite execution), quality gate, and execution; `plan-ideas-grooming` grooms the idea corpus itself (`plans/ideas/`), sweeping and reorganizing it rather than authoring or executing a plan
