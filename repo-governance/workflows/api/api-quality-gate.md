...
inputs:
  - name: scope
    type: string
    description: 'Base URL or endpoint set to exercise, plus the contract to test against (e.g., "http://localhost:19320 with apps/beaver-nest-be/openapi.yaml")'
    required: true
  - name: mode
    type: enum
...
2. The appropriate `swe-*-dev` agent — chosen by the implementing language of the service under
   test (`swe-fsharp-dev` for `beaver-nest-be`, `swe-typescript-dev`, `swe-rust-dev`,
...
```
User: "Run API quality gate for http://localhost:19320 against apps/beaver-nest-be/openapi.yaml"
User: "Run API quality gate for the beaver-nest-be GraphQL endpoint"
```
