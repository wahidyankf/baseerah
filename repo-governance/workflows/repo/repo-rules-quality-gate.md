...
- FAIL: **Skips**: secondary platform binding agent directories (e.g., `.opencode/agents/`) — auto-generated from `.claude/agents/` via `npm run generate:bindings`. Validate via the sync script + `validate:cross-vendor-parity` Nx target, not this workflow.
