...
| Class                      | Rule                                        | Example                                                  |
| -------------------------- | ------------------------------------------- | -------------------------------------------------------- |
| App-defined value          | `SCREAMING_SNAKE`, per-app prefix           | `BEAVER_NEST_BE_PORT`, `BEAVER_NEST_BE_OPENROUTER_MODEL` |
...
The **per-app prefix** is the app's Nx project name upcased with `_` separators: `beaver-nest-be` →
`BEAVER_NEST_BE_`, `beaver-nest-fe` → `BEAVER_NEST_FE_`.
...
`BEAVER_NEST_FE_PORT` would break `nx dev beaver-nest-fe`. **Backend** ports are app-defined code, so they
**do** take the prefix (`BEAVER_NEST_BE_PORT`, `EXAMPLE_BE_PORT`). This is the single most
...
The default backup target is `~/<repo-root-basename>-env-backup/` (e.g. `~/beaver-nest-env-backup/`).
...
env-contract:
  surfaces:
    - root: apps/beaver-nest-be
      kind: app
      lang: fsharp
      allowlist: []
    - root: apps/beaver-nest-fe
      kind: app
      lang: typescript
...
| -------------------------- | --------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------- |
| App-runtime (server)       | `DATABASE_URL`, `BEAVER_NEST_BE_NATS_URL`                                   | **yes**            | local `.env.local` · GitHub Env (CI) · Vercel encrypted env · k3s secret                       |
...
key list. They load a gitignored local `.env` (e.g. `infra/dev/beaver-nest-app/.env`, already
...
guard still sees one source of truth. New stacks (e.g. `infra/dev/beaver-nest-fe/`) and stack
renames (e.g. `infra/dev/beaver-nest` → `infra/dev/beaver-nest-app`) follow this rule and keep
...
env-injection:
  apps:
    - app: beaver-nest-fe
      ...
      keys-from: apps/beaver-nest-fe/.env.example
    - app: beaver-nest-be
      runtime: { local-ci: compose, staging: k3s-coralpolyp }
      keys-from: apps/beaver-nest-be/.env.example
  ci-harness:
...
Course and teaching material sometimes ships an env file as a **worked example** — a self-hosting
kata that demonstrates a secret committed to a repo, for instance. Those files are published
curriculum, not real environment files, and blocking them stops agents from authoring, linting, or
even `git stash`-ing the course they belong to.
