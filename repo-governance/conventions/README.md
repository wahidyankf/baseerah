...
- [Documentation Linking Convention](./formatting/linking.md) - Standards for linking between documentation files across the repository, including relative path conventions, `.md` extension requirements, and cross-directory reference patterns
...
- [FP-Variant Multi-Language Convention](./writing/fp-variant-multi-language.md) - Bidirectional idiomatic-language rule for FP-variant by-example tutorials in beaver-nest-fe. Requires F# AND Clojure tabs with each language kept idiomatic to its own community. Defines idiomatic patterns for each language, cross-paradigm concept handling (closest native equivalent + annotation), and per-tab annotation density (1.0–2.25 ratio)
- [Indonesian Content Policy](./writing/indonesian-content-policy.md) - Policy defining when and how to create Indonesian content in beaver-nest-fe. Establishes English-first policy for technical tutorials, defines Indonesian content categories (unique content, strategic translations, discouraged mirrors), provides decision tree for language selection, and specifies agent behavior for content creation
...
- [Programming Language Documentation Separation](./structure/programming-language-docs-separation.md) - **Partially historical** — Rule 1 (repository-specific style-guide scope for `docs/explanation/`) remains valid; Rules 2-5 (cross-referencing with educational content on the removed ayokoding-www platform) are retained for reference only, pending any future content-education app. Applies to all programming languages (Java, Python, Golang, TypeScript, Elixir, Kotlin, Dart, Rust, Clojure, F#, C#)
...
Tutorial creation, structure, naming, and content standards applying to **all tutorial content** (docs/, and any future content-education app, anywhere). These conventions **build upon and extend** the writing conventions above.
...
- [Programming Language Content Standard](./tutorials/programming-language-content.md) - **Historical** — Full Set Tutorial Package architecture authored for programming-language education on the removed ayokoding-www platform. No current applicability (neither `beaver-nest-fe` nor `beaver-nest-be` is an educational-content platform); retained for reference in case a future content-education app is added. **See also**: [How to Add a Programming Language](../../docs/how-to/add-programming-language.md)
