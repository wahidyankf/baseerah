...
about intent, tradeoffs, and conventions in _any_ changed content, prose or code alike. **Default to
including a specialist, not skipping it, whenever applicability is ambiguous** — this filter trims two
structurally-gated disciplines only; it is not a general "is this specialist plausibly useful"
judgment call.
...
2. **Selected specialist set** — the empty set for `trivial`, the four-specialist `lite` set, or all
   nine specialists for `full` (minus any DD-10 skip).
...
- [PR Review Cycle Scout + Cycle-Number + Type-Soundness — README](https://github.com/wahidyankf/ose-public/blob/main/plans/done/2026-08-06__pr-review-cycle-scout-and-typesafety/README.md) - The plan that introduced this agent (plan folder lives only in `ose-public`)
