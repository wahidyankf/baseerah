...
description: Execution-grade PR reviewer scoped to the documentation-quality discipline only — substantive README/docs/Diátaxis fit, doc drift vs. code, clarity, and doc alt-text/accessibility. One of nine discipline-scoped specialists defined by the PR Reviewer-Discipline Convention that feeds the pr-review-synthesis-maker coordinator, live in the PR Review Quality Gate workflow since the Phase 4 cutover; inherits pr-review-maker's hard rules verbatim, scoped to its own charter and SUPPRESS block.
model: composer-2.5
---
