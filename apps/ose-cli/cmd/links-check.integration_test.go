...
var specsLinksCheckOseplatformDir = func() string {
	...
	return filepath.Join(filepath.Dir(f), "../../../specs/apps/ose-platform/behavior/cli/gherkin")
}()
...
type linksCheckOseSteps struct {
	originalWd string
	tmpDir     string
	contentDir string
	cmdErr     error
...
func (s *linksCheckOseSteps) before(_ context.Context, _ *godog.Scenario) (context.Context, error) {
	s.originalWd, _ = os.Getwd()
	s.tmpDir, _ = os.MkdirTemp("", "links-check-ose-*")
	verbose = false
	quiet = false
	output = "text"
	linksContentDir = "apps/ose-web/content"
...
func (s *linksCheckOseSteps) after(_ context.Context, _ *godog.Scenario, _ error) (context.Context, error) {
	_ = os.Chdir(s.originalWd)
	_ = os.RemoveAll(s.tmpDir)
	return context.Background(), nil
}

func (s *linksCheckOseSteps) captureRun() {
	r, w, _ := os.Pipe()
...
func (s *linksCheckOseSteps) makeContentDir() (string, error) {
...
func (s *linksCheckOseSteps) oseWebContentWhereAllInternalLinksResolveCorrectly() error {
...
func (s *linksCheckOseSteps) oseWebContentWithALinkPointingToANonExistentPage() error {
	contentDir, err := s.makeContentDir()
	if err != nil {
		return err
	}
...
func (s *linksCheckOseSteps) oseWebContentWithOnlyExternalHTTPSLinks() error {
	contentDir, err := s.makeContentDir()
	if err != nil {
		return err
	}
...
func (s *linksCheckOseSteps) theDeveloperRunsLinksCheck() error {
	linksContentDir = s.contentDir
	s.captureRun()
	return nil
}

func (s *linksCheckOseSteps) theDeveloperRunsLinksCheckWithJSONOutput() error {
	linksContentDir = s.contentDir
	output = "json"
	s.captureRun()
	return nil
...
func (s *linksCheckOseSteps) theOseLinksCommandExitsSuccessfully() error {
	if s.cmdErr != nil {
...
func (s *linksCheckOseSteps) theOseLinksCommandExitsWithAFailureCode() error {
	if s.cmdErr == nil {
...
func (s *linksCheckOseSteps) theOseLinksOutputIsValidJSON() error {
...
func InitializeLinksCheckOseplatformScenario(sc *godog.ScenarioContext) {
	s := &linksCheckOseSteps{}
	sc.Before(s.before)
	sc.After(s.after)

	sc.Step(`^ose-web content where all internal links resolve correctly$`, s.oseWebContentWhereAllInternalLinksResolveCorrectly)
	sc.Step(`^ose-web content with a link pointing to a non-existent page$`, s.oseWebContentWithALinkPointingToANonExistentPage)
	sc.Step(`^ose-web content with only external HTTPS links$`, s.oseWebContentWithOnlyExternalHTTPSLinks)
	...
	sc.Step(`^the command exits successfully$`, s.theOseLinksCommandExitsSuccessfully)
	sc.Step(`^the command exits with a failure code$`, s.theOseLinksCommandExitsWithAFailureCode)
	sc.Step(`^the output is valid JSON$`, s.theOseLinksOutputIsValidJSON)
}
