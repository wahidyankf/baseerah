...
let ``backup and restore reject invalid names before touching the filesystem`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-invalid-operation-name-" + Guid.NewGuid().ToString("N"))
...
let ``restore returns sanitized errors for missing and corrupt backup files`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-invalid-restore-" + Guid.NewGuid().ToString("N"))
...
let ``backup and restore reject a foreign-key-invalid SQLite backup`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-invalid-foreign-key-" + Guid.NewGuid().ToString("N"))
...
let ``backup refuses overwrite and restore installs a backup when no live database exists`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-no-live-restore-" + Guid.NewGuid().ToString("N"))
...
let ``restore installs a verified backup into a separate previously absent data directory`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-absent-live-directory-" + Guid.NewGuid().ToString("N"))
...
let ``SQLite connection-string metacharacters in literal directories cannot inject backup options`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-connection-literal-" + Guid.NewGuid().ToString("N"))
...
let ``backup and restore reject symbolic-link roots, files, and data-directory aliases`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-operation-links-" + Guid.NewGuid().ToString("N"))
...
let ``database operations fail closed while another operation owns the data-directory lock`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-operation-lock-" + Guid.NewGuid().ToString("N"))
...
let ``database operations reject a symbolic-link operation lock`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-operation-lock-link-" + Guid.NewGuid().ToString("N"))
    ...
    File.CreateSymbolicLink(Path.Combine(dataDirectoryPath, ".beaver-nest-operation.lock"), lockTargetPath)
    |> ignore
...
let ``database operations fail closed when configured data directory becomes a file`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-data-directory-file-" + Guid.NewGuid().ToString("N"))
...
let ``restore fails closed for a live service while online backup remains available`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-service-lock-" + Guid.NewGuid().ToString("N"))
...
let ``restore rejects symbolic-link database companions before replacing live data`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-companion-link-" + Guid.NewGuid().ToString("N"))
...
let ``restore rejects a symbolic-link live database without replacing the target`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-live-link-" + Guid.NewGuid().ToString("N"))
...
let ``embedded migrations are readable in order and readiness observes without creating a database`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-readiness-observation-" + Guid.NewGuid().ToString("N"))
