...
let ``database configuration refuses empty, root, home, repository, and nonpositive timeout values`` () =
    let invalidCases =
        [ "", 100
          ...
          Path.Combine(Path.GetTempPath(), "beaver-nest-timeout-" + Guid.NewGuid().ToString("N")), 0
          Path.Combine(Path.GetTempPath(), "beaver-nest-timeout-" + Guid.NewGuid().ToString("N")), -1 ]
...
let ``database configuration refuses an explicit symbolic-link component`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "beaver-nest-link-" + Guid.NewGuid().ToString("N"))
...
let ``database environment uses safe defaults and parses only positive timeouts`` () =
    ...
    Assert.Equal("/var/lib/beaver-nest", dataDirectory defaultConfiguration)
    Assert.Equal(5000, busyTimeoutMilliseconds defaultConfiguration)

    let explicitDirectory =
        Path.Combine(Path.GetTempPath(), "beaver-nest-env-" + Guid.NewGuid().ToString("N"))

    let explicitConfiguration =
        from (
            Map.ofList
                [ "BEAVER_NEST_BE_DATA_DIRECTORY", explicitDirectory
                  "BEAVER_NEST_BE_SQLITE_BUSY_TIMEOUT_MILLISECONDS", "75" ]
        )
    ...
    let fallbackTimeout =
        from (
            Map.ofList
                [ "BEAVER_NEST_BE_DATA_DIRECTORY", explicitDirectory
                  "BEAVER_NEST_BE_SQLITE_BUSY_TIMEOUT_MILLISECONDS", "not-a-number" ]
        )
        |> Result.defaultWith failwith
