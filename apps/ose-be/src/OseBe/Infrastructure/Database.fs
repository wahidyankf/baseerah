module OseAppBe.Infrastructure.Database
...
/// string. The migration scripts live as embedded resources in the OseAppBe
/// assembly (db/migrations/*.sql). Fails fast if any script cannot be applied.
...
/// Reads DATABASE_URL or fails fast. ose-app-be is a server backend and
