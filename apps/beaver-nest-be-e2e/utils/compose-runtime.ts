...
const backendService = "beaver-nest-be";
const projectName = process.env.BEAVER_NEST_BE_E2E_COMPOSE_PROJECT;
const composeFiles = [
  "-f",
  resolve(repositoryRoot, "infra/dev/beaver-nest-app/docker-compose.yml"),
  "-f",
  resolve(repositoryRoot, "infra/dev/beaver-nest-app/docker-compose.ci.yml"),
];
...
export async function runFsi(script: string, stopped = false): Promise<string> {
  ...
  const command = `printf '%s' '${encoded}' | base64 -d > /tmp/beaver-nest-e2e-probe.fsx\ndotnet fsi --exec /tmp/beaver-nest-e2e-probe.fsx\nrm -f /tmp/beaver-nest-e2e-probe.fsx`;
