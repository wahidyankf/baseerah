...
use connection = new SqliteConnection("Data Source=/var/lib/beaver-nest/beaver-nest.sqlite3;Mode=ReadOnly")
connection.Open()
...
let configuration = create "/var/lib/beaver-nest" 1000 |> Result.defaultWith failwith
...
When("the BeaverNest application starts against a disposable database", async () => {
  const result = await composeResult([
    ...
    "sh",
    "-ceu",
    `source=/tmp/beaver-nest-e2e-broken-source
rm -rf "$source"
mkdir -p "$source"
...
BEAVER_NEST_BE_DATA_DIRECTORY="$source/data" BEAVER_NEST_BE_HTTP_LISTEN_ADDRESS=127.0.0.1 BEAVER_NEST_BE_HTTP_LISTEN_PORT=19321 dotnet "$source/src/BeaverNestBe/bin/Debug/net10.0/BeaverNestBe.dll"`,
  ]);
...
Then("the migration failure is logged without exposing sensitive configuration", async () => {
  ...
  expect(output).not.toContain("/tmp/beaver-nest-e2e-broken-source/data");
});
