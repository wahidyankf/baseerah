...
const backupName = "beaver-nest-e2e-snapshot.sqlite3";
...
async function createValidatedBackup(): Promise<void> {
  await backendShell(`rm -f /var/backups/beaver-nest/${backupName}`);
  await runBackendCommand(["backup", "--name", backupName]);
  await backendShell(`test -f /var/backups/beaver-nest/${backupName}`);
  backupCreated = true;
}
...
Given("BeaverNest is ready with WAL enabled", async ({ request }) => {
  ...
  const output = await runFsi(databaseCheckScript("/var/lib/beaver-nest/beaver-nest.sqlite3"));
...
Then("integrity_check returns {string} for the backup", async ({}, result: string) => {
  const output = await runFsi(databaseCheckScript(`/var/backups/beaver-nest/${backupName}`));
...
Then("the replaced database is preserved at a recoverable path", async () => {
  ...
  await stoppedBackendShell(
    "test \"$(find /var/lib/beaver-nest -maxdepth 1 -name 'beaver-nest.sqlite3.replaced-*' | wc -l)\" -eq 1",
  );
});

Then("the restored migration journal is current", async () => {
  const output = await runFsi(databaseCheckScript("/var/lib/beaver-nest/beaver-nest.sqlite3"), true);
