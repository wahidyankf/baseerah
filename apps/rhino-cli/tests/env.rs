...
/// F# app surface with a matching injection manifest, used to prove that
/// `readEnvironment "KEY"` calls count as production environment reads.
...
#[given(
    "an F# app surface whose .env.example declares keys read through a pure environment-reader wrapper"
)]
