...
mod tests {
    ...
    fn non_domain_area_project_is_skipped() {
        let domain_areas = vec!["beaver-nest-be".to_string(), "example-be".to_string()];
        assert!(
    ...
    fn domain_area_project_is_eligible() {
        let domain_areas = vec!["beaver-nest-be".to_string(), "example-be".to_string()];
        assert!(
            is_eligible("beaver-nest-be", &domain_areas),
            "beaver-nest-be must be eligible for domain-coverage — listed in domain-areas"
        );
    }
}
